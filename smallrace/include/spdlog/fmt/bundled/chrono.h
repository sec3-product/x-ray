    ...
    auto format(const std::tm& tm, FormatContext& ctx) -> decltype(ctx.out()) {
        ...
        for (;;) {
            ...
            if (size >= tm_format.size() * 256) {
                // If the buffer is 256 times larger than the format string,
                // assume that `strftime` gives an empty result. There doesn't
                // seem to be a better way to distinguish the two cases:
...
inline std::chrono::duration<Rep, std::milli> get_milliseconds(std::chrono::duration<Rep, Period> d) {
    ...
    auto ms =
        mod(d.count() * static_cast<common_type>(Period::num) / static_cast<common_type>(Period::den) * 1000, 1000);
...
struct chrono_formatter {
    ...
    void write(Rep value, int width) {
        ...
        uint32_or_64_t<int> n = to_unsigned(to_nonnegative_int(value, (std::numeric_limits<int>::max)()));
...
public:
    ...
    auto format(const duration& d, FormatContext& ctx) -> decltype(ctx.out()) {
        ...
        internal::handle_dynamic_spec<internal::width_checker>(specs.width, width_ref, ctx, format_str.begin());
        internal::handle_dynamic_spec<internal::precision_checker>(precision, precision_ref, ctx, format_str.begin());
        if (begin == end || *begin == '}') {
