...
class MLIRGenImpl {
 ...
 private:
  ...
  void createExtraLLVMFuncOp(mlir::Location location, llvm::StringRef name,
                             mlir::Type llvmFnType) {
    OpBuilder::InsertionGuard insertGuard(builder);
    builder.setInsertionPointToStart(theModule.getBody());
    builder.create<LLVM::LLVMFuncOp>(location, name, llvmFnType);
  }
...
class MLIRGenImpl {
 private:
  ...
  std::string getCurrentNonAnonFunctionName() {
    ...
    if (size > 0) {
      for (int i = size - 1; i >= 0; i--) {
        ...
        if (!name.startswith(ANON_FUNC_NAME)) return name.str();
      }
  ...
  FlatSymbolRefAttr getOrInsertUserDefinedFunctionX(
    ...
    llvm::outs() << "-----adding UserDefinedSTFunction ------" << name << "\n";
  ...
  FlatSymbolRefAttr getOrInsertBuiltInParentVarFunctionX() {
    auto name = ST_BUILT_IN_MODEL_PARENT_VAR;
    auto *context = builder.getContext();
    if (theModule.lookupSymbol<FuncOp>(name))
      return SymbolRefAttr::get(name, context);
  ...
  FlatSymbolRefAttr getOrInsertBuiltInBlockParamFunctionX(
    ...
    auto name =
        ST_BUILT_IN_MODEL_BLOCK_PARAM + "." + std::to_string(ANON_FUNC_INDEX);
  ...
  FlatSymbolRefAttr getOrInsertBuiltInClassMetaFunctionX() {
    auto name = ST_BUILT_IN_CLASS_METADATA;
    auto *context = builder.getContext();
    if (theModule.lookupSymbol<FuncOp>(name))
      return SymbolRefAttr::get(name, context);
 ...
 public:
  ...
  mlir::Value mlirGen(CascadeFunctionCallAST &call) {
    ...
    // multiple calls
    // Transcript cr; show: 'Size ',	self sharedState size printString.
    llvm::outs() << "-----creating CascadeFunctionCallAST------"
                 << "\n";
    ...
    for (auto kwmsg : call.getKeywordMessagesStack()) {
      ...
      llvm::outs()
          << "-----creating CascadeFunctionCallAST KeywordMessage------callee: "
          << callee << "\n";
      ...
      auto mlocation = arg.getLoc();  // loc(kwmsg->loc());
      auto calleeRef =
    ...
    for (auto umsg : call.getUnaryMessagesStack()) {
      SmallVector<mlir::Value, 4> operands;
      operands.push_back(arg);
      auto mlocation = arg.getLoc();  // loc(call.loc());
      std::string callee = umsg;
      llvm::outs()
          << "-----creating CascadeFunctionCallAST UnaryMessage------callee: "
          << callee << "\n";
      callee = ST_BUILT_IN_NAME + callee;
      auto calleeRef =
          getOrInsertUserDefinedFunctionX(mlocation, callee, operands);
  ...
  mlir::Value mlirGen(KeywordFunctionCallAST &call) {
    llvm::outs() << "-----creating KeywordFunctionCallAST------callee: "
                 << call.getName() << "\n";
    // self sharedState: Dictionary new.
    ...
    // msg->binSend - Dictionary new.
    // call callee on expr
    llvm::outs() << "-----call kwcallee on expr------name: " << callee
  ...
  mlir::Value mlirGen(BinaryFunctionCallAST &call) {
    //  vector<UnaryMessageAST*> umessages;
    // vector<string> umessagesTails;
    // BinaryMessageAST* bmessages = NULL;
    llvm::outs() << "-----creating BinaryFunctionCallAST------"
                 << "\n";
    ...
    for (auto callee : call.umessage->unaryTails) {
      // call callee on expr
      llvm::outs() << "-----call callee on expr bin ------name: " << callee
                   << "\n";
    ...
    //'Size ',	self sharedState size printString.
    auto bmsg = call.bmessages;
    while (bmsg) {
      llvm::outs() << "-----binary message------opr: " << bmsg->opr << "\n";
      // UnaryMessageAST* operand
  ...
  mlir::Value mlirGen(UnaryMessageAST &call) {
    llvm::outs() << "-----creating UnaryMessageAST------"
                 << "\n";
    auto expr = call.expr;      // self
    auto arg = mlirGen(*expr);  // args can be complicated!
    for (auto callee : call.unaryTails) {
      // call callee on expr
      llvm::outs() << "-----call callee on expr unary------name: " << callee
                   << "\n";
  ...
  mlir::Value mlirGen(VariableExprAST &expr) {
    ...
    if (symbolTableStack.back()->count(expr.getName())) {
      ...
      llvm::outs() << "-----did not find in symbolTable------name: " << varName
                   << "\n";
      auto varLoc = loc(expr.loc());
  ...
  mlir::Value mlirGen(BlockExprAST &block) {
    std::string funcName = ANON_FUNC_NAME + getCurrentNonAnonFunctionName() +
    ...
    auto retBlock = getOrCreateGlobalStringX(locInfo, funcName, funcName);
    {
      ...
      auto tmp = builder.create<LLVM::CallOp>(locInfo, llvmI8PtrTy, blockRef,
                                              blockOperands);
      ...
      //   llvm::outs() << "block arg: " << arg.getName() << "\n";
      // }
  ...
  void mlirGen(ExprASTList &exprList) {
    ...
    for (int k = 0; k < size; k++) {
      ...
      {
        // llvm::outs() << "-----mlirGen ExprASTList expr------OTHER"
        //              << "\n";
        mlirGen(*expr);
      }
    }
  ...
  mlir::Value mlirGen(ExprAST &expr) {
    llvm::outs() << "ExprAST: " << expr.getKind() << " loc: " << loc(expr.loc())
                 << "\n";
  ...
  void declareLocalVariable(VarDeclExprAST &vardecl, llvm::StringRef funcName) {
    ...
    // expressions.
    llvm::outs() << "-----mlirGen FunctionAST ------VarDeclExprAST: " << varName
                 << "\n";
    // i8* st.newTemp("name")
  ...
  mlir::FuncOp mlirGenClassesMetadata(stx::ModuleAST &moduleAST) {
    auto funcName = ST_BUILT_IN_CLASS_METADATA + "$" + moduleAST.path;
    if (LOWER_BOUND_ID > 0)
