...
class ModuleLinker {
  ...
  GlobalValue *getLinkedToGlobal(const GlobalValue *SrcGV) {
    ...
    if (DGV->hasLocalLinkage()) return nullptr;
...
static GlobalValue::VisibilityTypes getMinVisibility(
    GlobalValue::VisibilityTypes A, GlobalValue::VisibilityTypes B) {
...
bool ModuleLinker::linkIfNeeded(GlobalValue &GV) {
  ...
  if (shouldLinkOnlyNeeded()) {
    ...
    if (!GV.hasAppendingLinkage()) {
      ...
      if (!DGV->isDeclaration()) return false;
    }
  }
  ...
  if (DGV && !GV.hasLocalLinkage() && !GV.hasAppendingLinkage()) {
    ...
    if (DGVar && SGVar) {
      ...
      if (DGVar->hasCommonLinkage() && SGVar->hasCommonLinkage()) {
        ...
        SGVar->setAlignment(MaybeAlign(Align));
        DGVar->setAlignment(MaybeAlign(Align));
      }
    }
  ...
  if (const Comdat *SC = GV.getComdat()) {
    ...
    if (!LinkFromSrc) return false;
  }

  bool LinkFromSrc = true;
  if (DGV && shouldLinkFromSource(LinkFromSrc, *DGV, GV)) return true;
...
void ModuleLinker::addLazyFor(GlobalValue &GV, const IRMover::ValueAdder &Add) {
  ...
  if (InternalizeCallback) Internalize.insert(GV.getName());
  Add(GV);
  ...
  for (GlobalValue *GV2 : LazyComdatMembers[SC]) {
    ...
    bool LinkFromSrc = true;
    if (DGV && shouldLinkFromSource(LinkFromSrc, *DGV, *GV2)) return;
    if (!LinkFromSrc) continue;
    if (InternalizeCallback) Internalize.insert(GV2->getName());
    Add(*GV2);
  }
}
...
void ModuleLinker::dropReplacedComdat(
  ...
  if (!C) return;
  if (!ReplacedDstComdats.count(C)) return;
  if (GV.use_empty()) {
    GV.eraseFromParent();
    return;
  }
...
bool ModuleLinker::run() {
  ...
  for (const auto &SMEC : SrcM->getComdatSymbolTable()) {
    ...
    if (ComdatsChosen.count(&C)) continue;
    Comdat::SelectionKind SK;
    bool LinkFromSrc;
    if (getComdatResult(&C, SK, LinkFromSrc)) return true;
  ...
  for (GlobalVariable &GV : SrcM->globals())
    if (linkIfNeeded(GV)) return true;

  for (Function &SF : *SrcM)
    if (linkIfNeeded(SF)) return true;

  for (GlobalAlias &GA : SrcM->aliases())
    if (linkIfNeeded(GA)) return true;
  ...
  for (unsigned I = 0; I < ValuesToLink.size(); ++I) {
    ...
    for (GlobalValue *GV2 : LazyComdatMembers[SC]) {
      GlobalValue *DGV = getLinkedToGlobal(GV2);
      bool LinkFromSrc = true;
      if (DGV && shouldLinkFromSource(LinkFromSrc, *DGV, *GV2)) return true;
      if (LinkFromSrc) ValuesToLink.insert(GV2);
    }
  }

  if (InternalizeCallback) {
    for (GlobalValue *GV : ValuesToLink) Internalize.insert(GV->getName());
  }
  ...
  if (Error E = Mover.move(
    ...
    handleAllErrors(std::move(E),
                    [&](ErrorInfoBase &EIB) { HasErrors = true; });
  }
  if (HasErrors) return true;

  if (InternalizeCallback) InternalizeCallback(DstM, Internalize);

  return false;
}

st::Linker::Linker(Module &M) : Mover(M) {}

bool st::Linker::linkInModule(
