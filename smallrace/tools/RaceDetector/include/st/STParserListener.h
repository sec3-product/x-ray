...
class STParserListener : public SmalltalkBaseListener {
 public:
 ...
 private:
      ...
      if (op->reference() && ctx->binarySend()->unarySend()->unaryTail()) {
        ...
        if (op->literal()->parsetimeLiteral()) {
          ...
          if (ctx->binarySend()->unarySend()->unaryTail()) {
            ...
            Location loc2({fileName, usend->getStart()->getLine() + line_base,
                           usend->getStart()->getCharPositionInLine()});
            auto name = usend->unaryTail()->unaryMessage()->getText();
            auto *expr = new CallExprAST(loc2, std::string(name), args, level);
            exprList.push_back(expr);
            std::cout << "adding LiteralExprAST CallExprAST: " << op->getText()
                      << " " << name << "\n";
          }
          ...
          if (op->reference()) {
            ...
            if (op->literal()->parsetimeLiteral()) {
              if (op->literal()->parsetimeLiteral()->number()) {
                ...
                auto *arg = new NumberExprAST(loc, std::stol(op->getText()));
                args.push_back(arg);
              } else {
                std::cout << "adding other parsetimeLiteral: " << op->getText()
                          << "\n";
                auto *arg = new LiteralExprAST(loc, op->getText());
                args.push_back(arg);
              }
  ...
  void enterKeywordMessage(SmalltalkParser::KeywordMessageContext *ctx) {
    std::cout << SPACE << "enterKeywordMessage: " << ctx->getStart()->getText()
              << "\n";
    indentMore();
  }
  ...
  void enterKeywordPair(SmalltalkParser::KeywordPairContext *ctx) {
    std::cout << SPACE
              << "enterKeywordPair: " << ctx->KEYWORD()->getSymbol()->toString()
              << "\n";
    indentMore();
  }
  ...
  void enterBinaryMessage(SmalltalkParser::BinaryMessageContext *ctx) {
    ...
    std::cout << SPACE << "enterBinaryMessage: " << ctx->getText() << " @level "
              << level << "\n";
    indentMore();
  }
