...
// FIXME: when LLVM support configuration when errors happens, delete the file
// and use LLVM's version
namespace st {
...
class Linker {
 ...
 public:
  ...
  bool linkInModule(
      std::unique_ptr<llvm::Module> Src, unsigned Flags = Flags::None,
      std::function<void(llvm::Module &, const llvm::StringSet<> &)>
          InternalizeCallback = {});

  static bool linkModules(
      llvm::Module &Dest, std::unique_ptr<llvm::Module> Src,
