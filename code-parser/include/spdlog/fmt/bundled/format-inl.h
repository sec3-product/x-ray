...
inline fmt::internal::null<> strerror_s(char*, std::size_t, ...) { return fmt::internal::null<>(); }
...
FMT_FUNC int count_digits<4>(internal::fallback_uintptr n) {
    // Assume little endian; pointer formatting is implementation-defined
    // anyway.
...
public:
    ...
    // Compute lower and upper boundaries (m^- and m^+ in the Grisu paper),
    // where a boundary is a value half way between the number and its
    // predecessor (lower) or successor (upper). The upper boundary is
    // normalized and lower has the same exponent but may be not normalized.
...
inline round_direction get_round_direction(uint64_t divisor, uint64_t remainder, uint64_t error) {
    ...
    if (remainder <= divisor - remainder && error * 2 <= divisor - remainder * 2) return down;
...
digits::result grisu_gen_digits(fp value, uint64_t error, int& exp, Handler& handler) {
    ...
    do {
        uint32_t digit = 0;
        // This optimization by miloyip reduces the number of integer divisions
...
FMT_API bool grisu_format(Double value, buffer<char>& buf, int precision, unsigned options, int& exp) {
    ...
    if (precision != -1) {
        ...
        fp_value.normalize();
        auto cached_pow = get_cached_power(min_exp - (fp_value.e + fp::significand_size), cached_exp10);
        fp_value = fp_value * cached_pow;
        fixed_handler handler{buf.data(), 0, precision, -cached_exp10, fixed};
        if (grisu_gen_digits(fp_value, 1, exp, handler) == digits::error) return false;
        buf.resize(to_unsigned(handler.size));
    } else {
        fp lower, upper;  // w^- and w^+ in the Grisu paper.
        fp_value.compute_boundaries(lower, upper);
        // Find a cached power of 10 such that multiplying upper by it will
        // bring the exponent in the range [min_exp, -32].
        ...
        if ((options & grisu_options::grisu3) != 0) {
            ...
            // Numbers outside of (lower, upper) definitely do not round to
            // value.
            ...
            if (n < buf.capacity()) {
                ...
                if (specs.type != 'a' && specs.type != 'A') {
                    ...
                    if (p < end && *p != 'e' && *p != 'E') {
                        decimal_point_pos = p;
                        if (!specs.type) {
                            // Keep only one trailing zero after the decimal
                            // point.
                            ++p;
                            if (*p == '0') ++p;
                ...
                buf.resize(n);
                break;  // The buffer is large enough - continue with
                        // formatting.
            }
            buf.reserve(n + 1);
        } else {
            // If result is negative we ask to increase the capacity by at least
            // 1, but as std::vector, the buffer grows exponentially.
