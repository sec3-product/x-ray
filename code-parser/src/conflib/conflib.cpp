...
static inline std::string BuildPath_(const std::vector<std::string> &stages,
                                     int last_stage) {
  std::string s;
...
static inline void AssignValue_(jsoncons::json &j, const std::string &key,
...
static inline void AssignValues_(jsoncons::json &j, const std::string &key,
...
[[maybe_unused]] static inline void ReplaceValue_(jsoncons::json &j,
  ...
  if (value == "true") {
    ...
    jsoncons::jsonpath::json_replace(j, jpath, jsoncons::json{value});
  }
}
 ...
 *
 */
static inline void InsertConfiguration_(jsoncons::json &j,
                                        const std::string &s) {
  size_t pos = s.find_first_of('=');
  bool is_array = (s[0] == '@');
    ...
    if (!r.empty()) {
      if (jtmp.empty()) {
        if (!is_array)
          ...
          jsoncons::jsonpath::json_replace(j, jpath, jsoncons::json{values});
      } else {
        if (r[0].is_object()) {
          jtmp.merge(std::move(r[0]));
        }
        jsoncons::jsonpath::json_replace(j, jpath, std::move(jtmp));
      }
...
ParseCmdlineArgs_(int argc, char *argv[],
...
Initialize(const std::map<std::string, std::string> &short_args_map,
           bool use_env_cmdline_opts, int argc, char *argv[]) {
  std::string conf_path;

  // load the default configuration file
  char *home_dir = std::getenv(ENV_INSTALLATION_DIRECTORY_.c_str());
  if (home_dir != nullptr) {
    conf_path = std::string(home_dir) + OS_PATHSEP_ + "conf" + OS_PATHSEP_ +
                DEFAULT_CONFIGURATION_NAME_;
  } else {
...
[[maybe_unused]] static inline std::vector<std::string>
ParseArray_(jsoncons::json &j) {
