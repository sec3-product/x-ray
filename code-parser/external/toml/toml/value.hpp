...
[[noreturn]] inline void throw_bad_cast(const std::string& funcname,
                                        value_t actual, const Value& v) {
  throw type_error(
      detail::format_underline(
...
template <typename Value>
[[noreturn]] void throw_key_not_found_error(const Value& v, const key& ky) {
  ...
  // or "the parsed file is empty" if there is no content at all (0 bytes in
  // file).
  ...
  if (loc.line() == 1 && loc.region() == 0) {
    ...
    throw std::out_of_range(format_underline(
        concat_to_string("key \"", ky, "\" not found in the top-level table"),
    ...
    throw std::out_of_range(
        format_underline(concat_to_string("key \"", ky, "\" not found"),
                         {{loc, "in this table"}}));
  }
}
...
class basic_value {
 ...
 public:
  ...
  basic_value(basic_value&& v)
      : type_(v.type()),
        region_info_(std::move(v.region_info_)),
  ...
  template <typename T,
            typename std::enable_if<
                detail::conjunction<
                    std::is_integral<T>,
                    detail::negation<std::is_same<T, boolean>>>::value,
  ...
  template <
      typename T,
      typename std::enable_if<
          detail::conjunction<detail::negation<std::is_same<T, array_type>>,
  ...
  basic_value(const T& list)
      : type_(value_t::array),
        region_info_(std::make_shared<region_base>(region_base{})) {
    static_assert(
        std::is_convertible<typename T::value_type, value_type>::value,
  ...
  basic_value& operator=(
      std::initializer_list<std::pair<key, basic_value>> list) {
    this->cleanup();
    this->type_ = value_t::table;
    this->region_info_ = std::make_shared<region_base>(region_base{});
  ...
  template <
      typename Map,
      typename std::enable_if<
          detail::conjunction<detail::negation<std::is_same<Map, table_type>>,
                              detail::is_map<Map>>::value,
          std::nullptr_t>::type = nullptr>
  ...
  template <typename T,
            typename std::enable_if<detail::has_into_toml_method<T>::value,
                                    std::nullptr_t>::type = nullptr>
  ...
  basic_value(toml::string s, detail::region reg, std::vector<std::string> cm)
      : type_(value_t::string),
        region_info_(std::make_shared<detail::region>(std::move(reg))),
        comments_(std::move(cm)) {
    assigner(this->string_, std::move(s));
  ...
  basic_value(std::pair<T, detail::region> parse_result,
      ...
      : basic_value(std::move(parse_result.first),
                    std::move(parse_result.second), std::move(com)) {}
  ...
  offset_datetime const& as_offset_datetime(
      const std::nothrow_t&) const& noexcept {
    return this->offset_datetime_;
  }
  local_datetime const& as_local_datetime(
      const std::nothrow_t&) const& noexcept {
    return this->local_datetime_;
  }
  local_date const& as_local_date(const std::nothrow_t&) const& noexcept {
    return this->local_date_;
  }
  local_time const& as_local_time(const std::nothrow_t&) const& noexcept {
  ...
  local_date& as_local_date(const std::nothrow_t&) & noexcept {
    return this->local_date_;
  }
  local_time& as_local_time(const std::nothrow_t&) & noexcept {
    return this->local_time_;
  }
  ...
  value_type& at(const std::size_t idx) {
    ...
    if (this->as_array(std::nothrow).size() <= idx) {
      throw std::out_of_range(detail::format_underline(
          ...
          {{this->location(),
            concat_to_string("the length is ",
...
using value = basic_value<TOML11_DEFAULT_COMMENT_STRATEGY, std::unordered_map,
                          std::vector>;
...
template <typename C, template <typename...> class T,
          template <typename...> class A>
inline bool operator==(const basic_value<C, T, A>& lhs,
                       const basic_value<C, T, A>& rhs) {
...
inline std::string format_error(
    const std::string& err_msg, const basic_value<C, T, A>& v,
    const std::string& comment, std::vector<std::string> hints = {},
...
template <typename C, template <typename...> class T,
          template <typename...> class A>
inline std::string format_error(
    const std::string& err_msg, const toml::basic_value<C, T, A>& v1,
    const std::string& comment1, const toml::basic_value<C, T, A>& v2,
    const std::string& comment2, std::vector<std::string> hints = {},
    const bool colorize = TOML11_ERROR_MESSAGE_COLORIZED) {
  return detail::format_underline(
      err_msg, {{v1.location(), comment1}, {v2.location(), comment2}},
      std::move(hints), colorize);
}
...
inline std::string format_error(
    const std::string& err_msg, const toml::basic_value<C, T, A>& v1,
    const std::string& comment1, const toml::basic_value<C, T, A>& v2,
    const std::string& comment2, const toml::basic_value<C, T, A>& v3,
    const std::string& comment3, std::vector<std::string> hints = {},
    const bool colorize = TOML11_ERROR_MESSAGE_COLORIZED) {
  return detail::format_underline(err_msg,
                                  {{v1.location(), comment1},
                                   {v2.location(), comment2},
                                   {v3.location(), comment3}},
                                  std::move(hints), colorize);
}

template <typename Visitor, typename C, template <typename...> class T,
          template <typename...> class A>
detail::return_type_of_t<Visitor, const toml::boolean&> visit(
    Visitor&& visitor, const toml::basic_value<C, T, A>& v) {
  switch (v.type()) {
    case value_t::boolean: {
  ...
  throw std::runtime_error(
      format_error("[error] toml::visit: toml::basic_value "
                   "does not have any valid basic_value.",
                   v, "here"));
}

template <typename Visitor, typename C, template <typename...> class T,
          template <typename...> class A>
detail::return_type_of_t<Visitor, toml::boolean&> visit(
    Visitor&& visitor, toml::basic_value<C, T, A>& v) {
  switch (v.type()) {
    case value_t::boolean: {
      return visitor(v.as_boolean());
    }
