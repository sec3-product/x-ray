...
1. Build [LLVM](https://github.com/llvm/llvm-project/tree/llvmorg-14.0.6)
   locally.
2. Compile `code-parser` and `code-analyzer`, our analysis tools specifically for
   the Rust language.
...
code-analyzer is a code scanning tool that scans the generated LLVM IR (Intermediate Representation) to get a vulnerability list.

To manually build code-analyzer, please see following steps:

#### 1. Set LLVM_DIR and MLIR_DIR path

Before running cmake, you need to modify `./code-analyzer/CMakeLists.txt` to specify the LLVM_DIR path. This depends on the `llvm-project` you previously built:
...
Solution: Modify `./code-analyzer/tools/Raceanalyzer/CMakeLists.txt`, add this line:
...
Note that only `coderrect.json` and the relevant binaries of coderrect are copied here. Binaries from code-parser and code-analyzer are not copied. You still need to manually create the link.
...
#### 3. create binary links for code-parser and code-analyzer

Create two soft links for `code-parser` and `code-analyzer` binaries, which should be put at the same path as `coderrect`.

```
ln -s ./code-analyzer/build/bin/racedetect [your-path]/sec3/bin/racedetect
...
1. Make sure code-parser and code-analyzer have been correctly built

2. Add `coderrect` to system $PATH, and create link of `sol-racedetect` and `racedetect` at this path. See [create binary links](./README.md#3-create-binary-links-for-code-parser-and-code-analyzer)  
