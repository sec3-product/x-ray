...
private:
        ...
        */
        ...
        if (isEntryNode) {
            ...
            auto result = ctxFunMap
                              .insert(std::make_pair(std::make_pair(curCtx, F),  // ctx + llvm::Function
        ...
        if (!interceptResult.redirectTo || interceptResult.option == InterceptResult::Option::IGNORE_FUN) {
    ...
    inline void createEdge(CallNodeTy *src, CallNodeTy *dst, const llvm::Function *originalTarget,
            ...
            for (const auto &inst : BB) {
                if (llvm::isa<llvm::CallInst>(inst) || llvm::isa<llvm::InvokeInst>(inst)) {
                    ...
                    if (cs.isIndirectCall()) {
                        ...
                        //llvm::outs() << "!!! visitCallNode directCall: " << inst << " func: " << fun->getName() << "\n";

                        auto targetNode = visitDirectCallSite(calledCtx, context, fun, &inst, BFSQueue, beforeNewNode,
    ...
    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirect>
    inline CallNodeTy *visitDirectCallSite(const ctx *curCtx, const ctx *preCtx, const llvm::Function *fun,
                                           const llvm::Instruction *I, std::queue<CallNodeTy *> &BFSQueue,
                                           BeforeNewNode beforeNewNode, OnNewDirectNode onNewDirect,
                                           OnNewInDirect onNewInDirect) {
        assert(fun != nullptr);
        auto r = this->getOrCreateCallNode(curCtx, preCtx, fun, I, beforeNewNode, onNewDirect, onNewInDirect);
...
public:
    ...
    void buildInitCallGraph(BeforeNewNode beforeNewNode, OnNewDirectNode onNewDirect, OnNewInDirectNode onNewIndirect,
        ...
        */
        ...
        auto entryNode =
            getOrCreateCallNode(initialCtx, initialCtx, entryFun, nullptr, beforeNewNode, onNewDirect, onNewIndirect)
                .first;
    ...
    template <typename BeforeNewNode, typename OnNewDirectNode, typename OnNewInDirectNode, typename OnNewEdge>
    void resolveCallTo(CallGraphNode<ctx> *resolvedNode, const llvm::Function *target, BeforeNewNode beforeNewNode,
                       OnNewDirectNode onNewDirect, OnNewInDirectNode onNewIndirect, OnNewEdge onNewEdge) {
        // TODO: add it back
        /*
        static_assert(std::is_invocable_r<bool, OnNewDirectNode, CallNodeTy *>::value, "");
        ...
        */
        ...
        for (; it != et; it++) {
            ...
            result =
                getOrCreateCallNode(calleeCtx, callerCtx, target, callInst, beforeNewNode, onNewDirect, onNewIndirect);
    ...
    [[nodiscard]] inline llvm::StringRef getEntryName() const { return this->entryName; }

    [[nodiscard]] inline const llvm::Module *getLLVMModule() { return this->llvmModule; }

    [[nodiscard]] inline const llvm::DataLayout &getDataLayout() { return this->llvmModule->getDataLayout(); }

    [[nodiscard]] inline const CallGraphTy *getCallGraph() { return callGraph.get(); }
    ...
    [[nodiscard]] inline const CallGraphNode<ctx> *getInDirectNode(const ctx *C, const llvm::Instruction *I) {
        ...
        if (it == ctxFunPtrMap.end()) return nullptr;  // JEFF
