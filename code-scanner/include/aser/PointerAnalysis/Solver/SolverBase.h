    ...
    inline bool processOffset(CGNodeTy *src, CGNodeTy *dst, CallBack callBack = Noop{}) {
        ...
        for (auto objNode : nodeVec) {
            ...
            if (!PT::has(ptrNode->getNodeID(), fieldObj->getObjectID())) {
                ...
                // auto ptrElemType = ptrNode->getPointer()->getValue()->getType()->getPointerElementType();
                // auto offsetType = fieldObj->getObject()->getOffsetType(idx->getModule()->getDataLayout());

                // auto offsetType = fieldObj->getObject()->getOffsetType()->getPointerElementType();
                // offset is calculate by the size? offset / 4
                // auto offsetType = objElemType->getContainedType(loffset);  // JEFF not sure (34860
                // is there a bug?)
...
//                        // for scalar object and the field-insensitive object, the type can be null;
//                        llvm::outs() << "offsetType: null\n";
//                    }
//
//                    llvm::outs() << "\nptrElemType: " << *ptrElemType << "\nobjElemType: " << *objElemType << "\n"
//                                 << "    poffset: " << poffset << " loffset: " << loffset
//                                 << "  offsetType: " << *printOffsetType << "\n";
//
//                    llvm::outs() << "PTA offset update size: " << nodeVec.size()
//                                 << " ptr: " << ptrNode->toString()         //<< " value: " << *ptr->getValue()
//                                 << "\nfieldObj: " << fieldObj->toString()  //<< " value: " << *fieldObj->getValue()
//                                 << "\n";                                   // JEFF
//                }
                // i8 is a special pointer?
                // if (ptrElemType->isIntegerTy(8) || ptrElemType->isPointerTy() || ptrElemType->isStructTy() ||
                //     ptrElemType->isFunctionTy())  // JEFF
                // {
//                    if (nodeVec.size() > 10000)
//                        llvm::outs() << "Large PTA offset update size: " << nodeVec.size()
//                                     << " ptr: " << ptrNode->toString()         //<< " value: " << *ptr->getValue()
//                                     << "\nfieldObj: " << fieldObj->toString()  //<< " value: " << *fieldObj->getValue()
//                                     << "\n";                                   // JEFF

                    // JEFF TODO: get the type of fieldObj->offet
                    // Peiming: We do not need to do type filtering here, because the type filtering has
                    // already been conducted before indexing the object, incompatible object will be filter out already
//                    if (nodeVec.size() == 1 || isZeroOffsetTypeInRootType(offsetType,
//                                                                          ptrElemType,
...
//        // JEFF debug
//        if (!changed) {
//            llvm::outs() << "NO-MATCHED-OFFSET-TYPE: debugging (size=" << nodeVec.size() << ")...\n";
//            // something is wrong
//            for (auto objNode : nodeVec) {
//                // this might create new object, thus modify the points-to set
//                auto *fieldObj = llvm::cast_or_null<ObjNodeTy>(LMT::indexObject(this->getLangModel(), objNode, idx));
...
//                }
//                auto ptrElemType = ptrNode->getPointer()->getValue()->getType()->getPointerElementType();
//                auto objElemType = fieldObj->getObject()->getValue()->getType()->getPointerElementType();
...
//        }
    ...
    bool processSpecial(CGNodeTy *src, CGNodeTy *dst, CallBack callBack = Noop{}) {
        // TODO: we should do a cache here as well! as handling special constraints are expensive
        assert(!src->hasSuperNode() && !dst->hasSuperNode());
...
public:
    ...
    // a mapping between lock variable name (whose pts is empty) and the speical anon obj
    ...
    [[nodiscard]] bool hasIdenticalPTS(const ctx *c1, const llvm::Value *v1, const ctx *c2,
    ...
    [[nodiscard]] inline const InDirectCallSite<ctx> *getInDirectCallSite(const ctx *C, const llvm::Instruction *I) {
