...
private:
    size_t buffer_length() const { return p_ - begin_buffer_; }
};
...
public:
    string_result(string_result&& val) : s_(nullptr) { std::swap(s_, val.s_); }

    string_result(output_type& s) : s_(std::addressof(s)) {}

    string_result& operator=(string_result&& val) { std::swap(s_, val.s_); }

    void flush() {}

    void append(const value_type* s, size_t length) { s_->insert(s_->end(), s, s + length); }

    void push_back(value_type ch) { s_->push_back(ch); }
};
...
public:
    ...
    bytes_result(output_type& s) : s_(s) {}

    // bytes_result& operator=(bytes_result&&) = default;

    void flush() {}

    void append(const uint8_t* s, size_t length) { s_.insert(s_.end(), s, s + length); }

    void push_back(uint8_t ch) { s_.push_back(ch); }
};
