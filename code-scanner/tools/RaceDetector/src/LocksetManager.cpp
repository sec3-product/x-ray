...
LocksetManager::ID LocksetManager::getRetrofitLockSetID(std::vector<const ObjTy *> pts, LocksetManager::ID id) {
    ...
    // if a the pts for a lock pointer is {A, B}, we will only consider it holds lock A
    // while in some other place, there's a lock pointer whose pts is {B}
    // we will consider them "not sharing lock"
    bool OPT = false;
    if (pts.size() > MAX_LOCKS_TO_CONSIDER) {
        OPT = true;
        if (pts.size() > 10) LOG_WARN("Too many locks ({}) in the retrofit points-to set!", pts.size());
    }
    for (auto o : pts) {
...
std::vector<const ObjTy *> LocksetManager::getLockSet(LocksetManager::ID id) { return idTolocksetMap[id]; }

// TODO: cache I->lockStr
std::string LocksetManager::findLockStr(const llvm::Instruction *I, const llvm::Value *lockPtr) {
    if (lockStrInstCache.find(I) != lockStrInstCache.end()) return lockStrInstCache.at(I);
    ...
    auto found1 = lockStr.find("lock(");  // TODO: customized APIs may not work: spin_lock_irqsave
    ...
    if (found1 + 4 == lockStr.find("("))
        found2 = lockStr.find_last_of(")");  // malloc_mutex_lock(tsd_tsdn(tsd), &info->mtx);
    else
    ...
    if (lockStr2.find(",") != std::string::npos) {
        if (DEBUG_LOCK_STR) llvm::outs() << "process comma lockStr: " << lockStr << "\n";
    ...
    if (fullname.length() > 0) {
        ...
        if (lastSlashindex == std::string::npos) lastSlashindex = -1;
        auto rawname = fullname.substr(lastSlashindex + 1);
        size_t lastDotindex = rawname.find_last_of(".");
        if (lastDotindex == std::string::npos) lastDotindex = fullname.length() - 1;
        rawname = rawname.substr(0, lastDotindex);
        lockStr2 = rawname + ":" + lockStr2;
    }
    if (DEBUG_LOCK_STR) llvm::outs() << "final lockStr: " << lockStr2 << "\n";
...
const llvm::CallBase *LocksetManager::findRAIILockInst(const llvm::Instruction *inst) {
    if (raiiLockInstCache.find(inst) != raiiLockInstCache.end()) return raiiLockInstCache.at(inst);
    raiiLockInstCache[inst] = nullptr;
    return nullptr;
}

// invariant: returned pts size must be one
void LocksetManager::getLockSet(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr,
                                std::vector<const ObjTy *> &pts) {
    // cache is used to avoid a problem: PTA may return wrong results on the same lock pointer lock/unlock
    ...
    if (pts.empty()) {
        ...
        if (!lockStr.empty()) pta.getPointsToForSpecialLockPtr(context, I, lockStr, lockPtr, pts);
        if (pts.empty()) {
            if (DEBUG_LOCK_STR) llvm::outs() << "empty lockStr empty pta - likely RAII unlock? inst: " << *I << "\n";
    ...
    if (!lockStr.empty()) {
        ...
        if (DEBUG_LOCK_STR)
            llvm::outs() << "adding pts to cache for lockStr: " << lockStr << " pts: " << pts.front()->getValue()
                         << "\n";
    }
...
void LocksetManager::acquireLock(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr) {
    std::vector<const ObjTy *> pts;
    ...
    // pta will create an anonymous object based on the lock variable name (and maintain the mapping)
    // NOTE: if the lock variable name changes (assigned to a temp variable), this will fail
    else {
        ...
        //         if (!curLockset.empty()) pts.push_back(curLockset.front());
        // } else {
        //     // llvm::outs() << "successful str unlock for: " << pts.front()->getValue() << "\n";
        //     // LOG_TRACE("successful str unlock for: {} ", lockStr);
        // }
    }

    // it makes no sense if a lock pointer points to more than 10 locks
    // FIXME: Yanze: simply ignore this lock/unlock? shouldn't we at least take one lock?
    bool OPT = false;
    if (pts.size() > MAX_LOCKS_TO_CONSIDER) {
        OPT = true;
        if (pts.size() > 10) LOG_WARN("Too many locks ({}) in the points-to set: {}!", pts.size(), *lockPtr);
    }
    for (auto o : pts) {
...
void LocksetManager::releaseLock(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr) {
    std::vector<const ObjTy *> pts;
    if (!lockPtr) {
        // pts.push_back(LangModel::getSpecialNodeForLock(lockPtr));
        if (!curLockset.empty()) pts.push_back(curLockset.front());
...
void LocksetManager::addRWLock(const ctx *context, const llvm::Value *lockPtr) {
    ...
    bool OPT = false;
    if (pts.size() > MAX_LOCKS_TO_CONSIDER) {
        OPT = true;  // it makes no sense if a lock pointer points to more than 10 locks
        if (pts.size() > 10) LOG_WARN("Too many rwlocks ({}) in the points-to set: {}!", pts.size(), *lockPtr);
    }
    if (!OPT)  // by default
        for (auto o : pts) rwLocks.insert(o);
}
bool LocksetManager::hasCommonLocks(const MemAccessEvent *e1, const std::vector<const ObjTy *> &lockset1,
                                    const MemAccessEvent *e2, const std::vector<const ObjTy *> &lockset2) const {
    ...
    for (auto *lock1 : lockset1) {
        for (auto *lock2 : lockset2) {
            // llvm::outs() << "INTERESTING event e1: " << *e1->getInst() <<" lock1: "<<lock1<<" lock1->getValue():
            // "<<lock1->getValue()<< "\n"; llvm::outs() << "INTERESTING event e2: " << *e2->getInst() <<" lock2:
...
bool LocksetManager::hasRWLockButOnlyRD(const MemAccessEvent *e) const {
    ...
    for (auto *o : lockset) {
        if (!isRWLock(o)) return false;  // has other locks such as mutex

        {  // make sure it has the rwlock at least twice
            // count the number
            // llvm::outs() << "INTERESTING event e3: " << *e->getInst() << "  lockset id:" << id << "\n";
...
std::map<std::pair<LocksetManager::ID, LocksetManager::ID>, bool> sharesLockCache;
bool LocksetManager::sharesLock(const MemAccessEvent *e1, const MemAccessEvent *e2) const {
    ID id1 = e1->getLocksetID();
    ID id2 = e2->getLocksetID();

    // llvm::outs() << "DEBUG e1: " << *e1->getInst() << "  tid1:" << e1->getTID() << "  lockset id1:" << id1 <<
    // "\n"; llvm::outs() << "DEBUG e2: " << *e2->getInst() << "  tid2:" << e2->getTID() << "  lockset id2:" << id2
    // << "\n";
    ...
    if (id1 == 0 || id2 == 0) {
        ...
        if (it != sharesLockCache.end()) {
            ...
            bool result = hasCommonLocks(e1, idTolocksetMap.at(id1), e2, idTolocksetMap.at(id2));
// update the cache
...
LocksetManager::ID LocksetManager::getCurrentLocksetId() const { return curLocksetId; }