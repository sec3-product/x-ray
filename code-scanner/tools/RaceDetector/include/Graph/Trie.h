...
extern int SAME_FUNC_BUDGET_SIZE;  // keep at most x times per func per thread 10 by default
...
static struct TrieNode *createNewNode(TrieNode *parent, const llvm::Function *key) {
    // FIXME: memory leak
...
static bool willExceedBudget(TrieNode *parent, const llvm::Function *func) {
    if (SAME_FUNC_BUDGET_SIZE != -1 && countSameFuncNodes[func] >= SAME_FUNC_BUDGET_SIZE) return true;
    if (parent) {
        if ((parent->children).find(func) != parent->children.end()) {
            TrieNode *n = (parent->children)[func];
            if (n->numberOfTimes >= CALL_STACK_BUDGET_SIZE) return true;

            // a special treatment for the main call stack
            if (USE_MAIN_CALLSTACK_HEURISTIC)
                if (parent->key->getName() == "main") {
                    LOG_DEBUG("skipped repeated func from main. func={}", demangle(func->getName().str()));
                    // llvm::outs() << "skipping repeated func: " << demangle(func->getName()) << " from main
                    // thread\n\n";
                    return true;
                }
        }
