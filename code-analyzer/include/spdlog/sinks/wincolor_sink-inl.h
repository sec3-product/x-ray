...
SPDLOG_INLINE wincolor_sink<ConsoleMutex>::wincolor_sink(HANDLE out_handle,
                                                         color_mode mode)
...
template <typename ConsoleMutex>
void SPDLOG_INLINE
wincolor_sink<ConsoleMutex>::set_color(level::level_enum level, WORD color) {
...
template <typename ConsoleMutex>
WORD SPDLOG_INLINE
wincolor_sink<ConsoleMutex>::set_foreground_color_(WORD attribs) {
...
wincolor_sink<ConsoleMutex>::write_to_file_(const memory_buf_t &formatted) {
  ...
  do {
    ...
    if (!ok || bytes_written == 0) {
      SPDLOG_THROW(
          spdlog_ex("wincolor_sink: write_to_file_ failed. GetLastError(): " +
