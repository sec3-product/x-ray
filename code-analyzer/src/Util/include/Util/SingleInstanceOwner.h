...
protected:
  ...
  // create or abort
  template <typename... Args> inline const T *create(Args &&...args) {
    auto r = innerSet.emplace(std::forward<Args>(args)...);
...
public:
  // get or abort
  template <typename... Args> inline const T *get(Args &&...args) const {
  ...
  // get or abort
  template <typename... Args> inline const T *getOrNull(Args &&...args) const {
    auto r = innerSet.find(T(std::forward<Args>(args)...));
    if (LLVM_UNLIKELY(r == innerSet.end())) {
      return nullptr;
    }
