...
#include "PointerAnalysis/Graph/ConstraintGraph/CGObjNode.h"
...
// so that map_lookup(map *m), where the map is a pointer and require pointer
// analysis to resolve the target map can not be supported here. To Support
// those feature, solver need to be updated accordingly to support index map
// objects as well. Which can be applied in future extension.

// NOTE: this is a special type of object, which does not appear in the
// constraint graph
//       it acts as a delegator to maintain and access the object
template <typename KeyT, typename LangModel> class MapObject {
private:
...
public:
  explicit MapObject(LangModel &model)
      : model(model), allElem(createAnonPtrNode()),
