...
const std::string SOL_BUILT_IN_NAME =
    "sol."; // for all the built-in sol function
...
IndirectResolveOption GraphBLASModel::onNewIndirectTargetResolvation(
    const llvm::Function *target, const llvm::Instruction *callsite) {
  if (callsite->getFunction()->getName().equals(
          "_ZZ9StartRESTRKN4util3RefEENK3$_1clEP11HTTPRequestRKNSt7__"
...
void GraphBLASModel::interceptHeapAllocSite(
  ...
  if (heapModel.isHeapInitFun(callee->getFunction())) {
    Type *type = callsite->getOperand(0)
                     ->getType()
                     ->getPointerElementType()
                     ->getPointerElementType();
    PtrNode *ptr =
        this->getPtrNode(caller->getContext(), callsite->getOperand(0));
    ...
    if (callee->getFunction()->getName().equals("f90_alloc04_chka_i8") ||
      ...
      PtrNode *ptr =
          this->getPtrNode(caller->getContext(),
                           llvm::cast<CallBase>(callsite)->getArgOperand(4));
      ObjNode *obj = this->allocHeapObj(
          caller->getContext(), callsite,
          getUnboundedArrayTy(IntegerType::get(callsite->getContext(), 8)));

      this->consGraph->addConstraints(obj->getAddrTakenNode(), ptr,
                                      Constraints::store);
      return;
    }
...
const llvm::Function *
GraphBLASModel::findSignalHandlerFunc(const llvm::Instruction *callsite) {
        ...
        if (GEPI->getNumOperands() > 2)
          if (auto CONSI = llvm::dyn_cast<llvm::Constant>(GEPI->getOperand(2)))
            if (CONSI->isZeroValue())
              ...
              if (auto BCI =
                      llvm::dyn_cast<llvm::BitCastInst>(GEPI->getNextNode())) {
                LOG_TRACE("signal handle next bitcast: {}", *BCI);

                if (const StoreInst *SI =
                        llvm::dyn_cast<llvm::StoreInst>(BCI->getNextNode())) {
                  LOG_TRACE("signal handle next store: {}", *SI);
                  if (auto sigFun = llvm::dyn_cast<llvm::Function>(
                          SI->getOperand(0)->stripPointerCasts())) {
                    sigActionHandlerMap[callsite] = sigFun;
                    return sigFun;
                  } else {
                    // indirect function pointer?
                    LOG_TRACE("find indirect signal handler: {}",
                              *SI->getOperand(0));
                    // try one more time
                    {
                      const llvm::Value *v2 =
                        ...
                        if (auto SI2 = llvm::dyn_cast<llvm::StoreInst>(U2)) {
                          if (auto sigFun2 = llvm::dyn_cast<llvm::Function>(
                            ...
                            LOG_TRACE(
                                ...
                                *SI2->getOperand(0)); // likely the func ptr is
...
const llvm::Function *
GraphBLASModel::findThreadStartFunc(PTA *pta, const ctx *context,
  ...
  // ```
    ...
    // FIXME: For indirect pthread callback, only traverse the first function
    // ptr get resolved eventually this should be fixed by having more precise
    // pta results such as adding context or inlining pthread wrapper
    auto cs = pta->getInDirectCallSite(context, forkSite);
    auto threadEntry = *cs->getResolvedNode().begin();
    if (threadEntry)
      return threadEntry->getTargetFun()->getFunction();
  }
  return nullptr;
}
// callbacks called by ConsGraphBuilder
InterceptResult
GraphBLASModel::interceptFunction(const ctx *calleeCtx, const ctx *callerCtx,
                                  const llvm::Function *F,
                                  const llvm::Instruction *callsite) {
  assert(!isHeapAllocAPI(F, callsite));

  if (F->hasName() &&
      (F->getName().equals("raxFind") || F->getName().equals("raxInsert"))) {
    return {F, InterceptResult::Option::ONLY_CALLSITE};
  }

  if (F->hasName() && (F->getName().equals("moduleRegisterApi") ||
                       F->getName().equals("RM_GetApi"))) {
    // TESTING:
  ...
  if (F->getName().startswith("_ZSt9call_once")) {
    // std::call_once
    // TODO: now only handle the simplest case when the Callable is passed as
    // function pointer
    if (F->arg_size() >= 2) {
    ...
    if (LangModel::isRustNormalCall(callsite)) {
      ...
      if (cbIndex >= 0) {
        ...
        auto callbck =
            llvm::dyn_cast<llvm::Function>(call->getArgOperand(cbIndex));
        ...
        // replace it with call back (so that at least we have something to
        // analyze).
...
static Optional<StringRef> getStringFromValue(const Value *V,
                                              const DataLayout &DL) {
  APInt offset(64, 0);
  ...
  if (auto GV = dyn_cast<GlobalVariable>(baseObj)) {
    auto strValue =
        llvm::dyn_cast_or_null<ConstantDataSequential>(GV->getInitializer());
...
bool GraphBLASModel::interceptCallSite(const CtxFunction<ctx> *caller,
  ...
  if (callee->getFunction()->hasName() &&
      (callee->getFunction()->getName().equals("raxFind") ||
       callee->getFunction()->getName().equals("raxInsert"))) {
    // TODO: choose another meaningful object for indexing for the map object
    auto mapObj =
        this->getOrAllocMapObj<StringRef>(CT::getGlobalCtx(), callsite);
    // key
    // auto strKey = getStringFromValue(CS.getArgOperand(0),
    // this->getLLVMModule()->getDataLayout());

    // PtrNode of the value inserted into map
    // auto valNode = this->getPtrNode(caller->getContext(),
    // CS.getArgOperand(1));
    ...
    //            // unknown key
    //            if
    //            (callee->getFunction()->getName().equals("moduleRegisterApi"))
    //            {
    //                // insert to map
    ...
    //                // int RM_GetApi(const char *funcname, void
    //                **targetPtrPtr) this->getConsGraph()->addConstraints(node,
    //                valNode, Constraints::store);
    //            }
    //        } else
    {
      // unknown key
  ...
  //    if (callee->getFunction()->hasName() &&
  //    callee->getName().equals("realloc")) {
  //        if (auto call = dyn_cast<CallBase>(callsite)) {
  //            auto src = this->getPtrNode(caller->getContext(),
  //            callsite->getOperand(0)); auto dst =
  //            this->getPtrNode(caller->getContext(), call);
  //
  ...
  if (callee->getFunction()->hasName() && callee->getName().equals("dlsym")) {
    if (auto call = dyn_cast<CallBase>(callsite)) {
      ...
      auto result = getStringFromValue(symbolName,
                                       this->getLLVMModule()->getDataLayout());
      if (result.hasValue()) {
        ...
        if (dlFun) {
          ...
          this->consGraph->addConstraints(funNode, symbolNode,
                                          Constraints::copy);
        }
      }
    }
  ...
  // the original target to determine whether the callsite need to be
  // intercepted
  ...
  if (fun && fun->getName().startswith("_ZSt9call_once")) {
    ...
    if (fun->arg_size() >= 2) {
      ...
      if (callableTy->isPointerTy() &&
        ...
        for (int i = 0; i < argNum; i++) {
          PtrNode *formal = this->getPtrNodeOrNull(
              callee->getContext(), &*callee->getFunction()->arg_begin() + i);
          if (formal == nullptr) {
            // the argument is not a pointer
            continue;
          } else {
            PtrNode *actual =
                this->getPtrNode(caller->getContext(), CS.getArgOperand(i + 2));
    ...
    // 1st argument, an struct evhttp_request, fed by the framework, create an
    // anonmyous object for it.
    auto actual = MMT::template allocateAnonObj<PT>(
    ...
    PtrNode *formal2 = this->getPtrNode(callee->getContext(),
                                        callee->getFunction()->getArg(1));
    PtrNode *actual2 =
        this->getPtrNode(caller->getContext(), CS.getArgOperand(2));
      ...
      if (cbIndex >= 0) { // TODO: assert callback index is >= 0
        ...
        for (auto &fArg : callbck->args()) {
          if (fArg.getType()->isPointerTy()) {
            ...
            PtrNode *actual = this->getPtrNode(
                caller->getContext(), aArg); // First three arguemnts of
...
bool GraphBLASModel::isRead(const llvm::Instruction *inst) {
  return llvm::isa<llvm::LoadInst>(inst);
}

bool GraphBLASModel::isWrite(const llvm::Instruction *inst) {
  return llvm::isa<llvm::StoreInst>(inst);
}

bool GraphBLASModel::isReadORWrite(const llvm::Instruction *inst) {
  return isRead(inst) || isWrite(inst);
}

bool GraphBLASModel::isRustNormalCall(const llvm::Function *func) {
...
bool GraphBLASModel::isRustAPI(const llvm::Function *func) {
  return func->getName().startswith(SOL_BUILT_IN_NAME);
}
