...
class GraphBLASModel
    : public LangModelBase<ctx, MemModel, PtsTy, GraphBLASModel> {
public:
...
private:
  ...
  std::map<std::pair<StringRef, const llvm::Value *>, ObjNode *>
      mapParentObjects;

public:
  GraphBLASModel(llvm::Module *M, llvm::StringRef entry) : Super(M, entry) {
    ctx::setOriginRules(
        [&](const ctx *context, const llvm::Instruction *I) -> bool {
          return false;
        });
  }
  ...
  void interceptHeapAllocSite(const CtxFunction<ctx> *caller,
                              const CtxFunction<ctx> *callee,
                              const llvm::Instruction *callsite);

  // return true if the function need to be expanded in callgraph.
  InterceptResult interceptFunction(const ctx *callerCtx, const ctx *calleeCtx,
                                    const llvm::Function *F,
                                    const llvm::Instruction *callsite);

  IndirectResolveOption
  onNewIndirectTargetResolvation(const llvm::Function *F,
                                 const llvm::Instruction *callsite);

  bool interceptCallSite(const CtxFunction<ctx> *caller,
                         const CtxFunction<ctx> *callee,
                         const llvm::Function *originalTarget,
                         const llvm::Instruction *callsite);

  static const llvm::Function *
  findSignalHandlerFunc(const llvm::Instruction *inst);
  static const llvm::Function *
  findThreadStartFunc(PartialUpdateSolver<GraphBLASModel> *pta,
                      const ctx *context, const llvm::Instruction *inst);
...
template <>
struct LangModelTrait<GraphBLASModel>
    : public LangModelTrait<
          LangModelBase<ctx, MemModel, PtsTy, GraphBLASModel>> {};
