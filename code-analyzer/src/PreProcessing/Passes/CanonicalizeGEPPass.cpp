    ...
    for (auto &I : BB) {
      if (auto GEP = dyn_cast<GetElementPtrInst>(&I)) {
        ...
        if (!isa<Constant>(GTI.getOperand())) {
          ...
          lastBasePtr =
              builder.CreateGEP(nullptr, lastBasePtr, GTI.getOperand());
        ...
        for (gep_type_iterator GTE = gep_type_end(GEP); GTI != GTE; GTI++) {
          ...
          if (isa<Constant>(op)) {
            ...
            if (consIndices.size() != 1) {
              ...
              // indices
              lastBasePtr =
                  builder.CreateGEP(nullptr, lastBasePtr, consIndices);
              LOG_TRACE("{}", *lastBasePtr);
            }
...
static RegisterPass<CanonicalizeGEPPass>
    CIP("", "Canonicalize GetElementPtr instruction", true, /*CFG only*/
        false /*is analysis*/);
