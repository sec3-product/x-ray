...
{
    printf("Usage:\n"
        "    libear.exe [options] [command line]\n"
        "Options:\n"
...
static void startAndWaitFirstInterception(CHAR *szCommand) {
    STARTUPINFOA dispatcherSi;
    PROCESS_INFORMATION dispatcherPi;
    CHAR dispatcherCommand[2048];

    CHAR path[MAX_PATH];
    GetModuleFileNameA(NULL, path, MAX_PATH);

    ZeroMemory(&dispatcherSi, sizeof(dispatcherSi));
    ZeroMemory(&dispatcherSi, sizeof(dispatcherSi));
    dispatcherSi.cb = sizeof(dispatcherSi);

    DWORD dwFlags = CREATE_DEFAULT_ERROR_MODE | CREATE_SUSPENDED;

    LPCSTR dispatcherExe = "\\coderrect-dispatcher.exe";
    LPSTR cml = new CHAR[lstrlenA(dispatcherExe) + 2 + lstrlenA(szCommand) + 1];
    lstrcpyA(cml, "coderrect-dispatcher.exe ");
    lstrcatA(cml, szCommand);

    CHAR dispatcherFullPath[MAX_PATH] = "\0";

    memcpy(dispatcherFullPath, path, ARRAYSIZE(path));

    int lastFileSeparator = 0;
    for (int i = 0; i < lstrlenA(path); i++) {
        if (dispatcherFullPath[i] == '\\') {
            lastFileSeparator = i;
        }
    }

    dispatcherFullPath[lastFileSeparator] = '\0';
    lstrcatA(dispatcherFullPath, dispatcherExe);

    printf("HELLO!!!!! %s %s\n", dispatcherFullPath, cml);
    //CreateProcessA(dispatcherFullPath, cml, NULL, NULL, TRUE, dwFlags, NULL, NULL, &dispatcherSi, &dispatcherPi);

    if (!CreateProcessA(dispatcherFullPath, cml, NULL, NULL, TRUE, dwFlags, NULL, NULL, &dispatcherSi, &dispatcherPi)) {
        DWORD dwError = GetLastError();
        //ErrorExit((wchar_t*)TEXT("CreateProcess"));
        ExitProcess(9009);
    }

    ResumeThread(dispatcherPi.hThread);
    WaitForSingleObject(dispatcherPi.hProcess, INFINITE);
}
...
int CDECL main(int argc, char** argv)
{
    ...
    for (; arg < argc && (argv[arg][0] == '-' || argv[arg][0] == '/'); arg++) {
        ...
        case 'D':
            if (nDlls < ARRAYSIZE(rpszDllsRaw)) {
                ...
                printf("libear.exe: Too many DLLs.\n");
                fNeedHelp = TRUE;
                break;
            }
            break;
        ...
        default:
            fNeedHelp = TRUE;
            printf("libear.exe: Bad argument: %s\n", argv[arg]);
            break;
        }
    }
    ...
    for (DWORD n = 0; n < nDlls; n++) {
        ...
        if (!GetFullPathNameA(rpszDllsRaw[n], ARRAYSIZE(szDllPath), szDllPath, &pszFilePart)) {
            printf("libear.exe: Error: %s is not a valid path name..\n",
                rpszDllsRaw[n]);
            return 9002;
        }
        ...
        if (hDll == NULL) {
            printf("libear.exe: Error: %s failed to load (error %ld).\n",
                rpszDllsOut[n],
                GetLastError());
            return 9003;
        }
        ...
        if (!ec.fHasOrdinal1) {
            printf("libear.exe: Error: %s does not export ordinal #1.\n",
                rpszDllsOut[n]);
    ...
    //}
    //fflush(stdout);
    //startAndWaitFirstInterception(szCommand);

    DWORD dwFlags = CREATE_DEFAULT_ERROR_MODE | CREATE_SUSPENDED;

    SetLastError(0);
    SearchPathA(NULL, szExe, ".exe", ARRAYSIZE(szFullExe), szFullExe, &pszFileExe);
    Debug("Start new session: %s", szFullExe);
    // TODO: here we need to dispatch it as well?
    if (!DetourCreateProcessWithDllsA(szFullExe[0] ? szFullExe : NULL, szCommand,
        NULL, NULL, TRUE, dwFlags, NULL, NULL,
        &si, &pi, nDlls, rpszDllsOut, NULL)) {
        DWORD dwError = GetLastError(); 
        printf("libear.exe: DetourCreateProcessWithDllEx failed: %ld\n", dwError);
        if (dwError == ERROR_INVALID_HANDLE) {
#if DETOURS_64BIT
            printf("libear.exe: Can't detour a 32-bit target process from a 64-bit parent process.\n");
#else
            printf("libear.exe: Can't detour a 64-bit target process from a 32-bit parent process.\n");
#endif
        }
        ErrorExit((wchar_t*)TEXT("Coderrect-init build process"));
        ExitProcess(9009);
    }
    ...
    if (!GetExitCodeProcess(pi.hProcess, &dwResult)) {
        printf("libear.exe: GetExitCodeProcess failed: %ld\n", GetLastError());
        return 9010;
    }
