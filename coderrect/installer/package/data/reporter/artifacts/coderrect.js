...
function setupOneUntrustfulAccount(ord, race) {
    let prefix = "#untrustfulAccount" + ord
    ...
    $(selector).html(race.description+":");
    ...
    if (race.inst.stacktrace.length > 0) {
        ...
        $("#untrustfulAccount" + ord + " .stacktrace").hide();
    }
    ...
    selector = prefix + " .url";
    var el = $(selector);
    el.attr("href", race.url)
    el.html(race.url);
}

function setupOneUnsafeOperation(ord, race) {
    let prefix = "#unsafeOperation" + ord
    let selector = prefix + " .race-no";
    $(selector).html("" + ord);
    ...
    if (race.inst.stacktrace.length > 0) {
        ...
        $("#unsafeOperation" + ord + " .stacktrace").hide();
    }
}
function setupOneCosplayAccount(ord, race) {
    let prefix = "#cosplayAccount" + ord
    let selector = prefix + " .race-no";
    $(selector).html("" + ord);
    ...
    var el = $(selector);
    el.attr("href", prefix)
    el.html(race.inst1.dir + utils.getFileSep() + race.inst1.filename);

    // set line number
    selector = prefix + " .race-var-src .line-number";
    $(selector).html(race.inst1.line);
    ...
    // set the code snippet
    selector = prefix + " .source-snippet code";
    $(selector).html(race.inst1.snippet);

    // set the stacktrace
    if (race.inst1.stacktrace.length > 0) {
        selector = prefix + " .stacktrace code";
        $(selector).html(utils.formatStacktrace(race.inst1.stacktrace));
    } else {
        $("#cosplayAccount" + ord + " .stacktrace").hide();
    }
    ...
    if (race.inst2) {
            ...
            // set href to the source file
    selector = prefix + " .race-var-src2 .filepath";
    var el = $(selector);
    el.attr("href", prefix)
    el.html(race.inst2.dir + utils.getFileSep() + race.inst2.filename);

            // set line number
    selector = prefix + " .race-var-src2 .line-number";
    $(selector).html(race.inst2.line);

        // selector = prefix + " .error-msg2";
        // $(selector).show();
        // $(selector).html(utils.preprocessPath(race.errorMsg));
...
function setupBody() {
    ...
    if (true) {
        html = $("#untrustfulAccounts").html();
        for (i = 0; i < raceData.untrustfulAccounts.length; i++) {
            var html2 = html.replace('untrustfulAccount0', 'untrustfulAccount' + raceIdx);
            $("#untrustfulAccounts").append(html2);
            setupOneUntrustfulAccount(raceIdx, raceData.untrustfulAccounts[i]);
            raceIdx++;
        }
    }
    $("#untrustfulAccount0").hide();

    if (true) {
        html = $("#unsafeOperations").html();
        for (i = 0; i < raceData.unsafeOperations.length; i++) {
            var html2 = html.replace('unsafeOperation0', 'unsafeOperation' + raceIdx);
            $("#unsafeOperations").append(html2);
            setupOneUnsafeOperation(raceIdx, raceData.unsafeOperations[i]);
            raceIdx++;
        }
    }
    $("#unsafeOperation0").hide();

    if (true) {
        html = $("#cosplayAccounts").html();
        for (i = 0; i < raceData.cosplayAccounts.length; i++) {
            var html2 = html.replace('cosplayAccount0', 'cosplayAccount' + raceIdx);
            $("#cosplayAccounts").append(html2);
            setupOneCosplayAccount(raceIdx, raceData.cosplayAccounts[i]);
            raceIdx++;
        }
    }
    $("#cosplayAccount0").hide();
...
function setupNavigation() {
    ...
    $("#newView").on("click", function (e) {
        ...
        $("#diffSummary").find("#diffRaceNum").text(newUntrustfulAccounts.length);
        $("#diffSummary").find("#diffMismatchNum").text(newUntrustfulAccounts.length);
        ...
        for (let i = 1; i <= raceData.untrustfulAccounts.length; i++) {
            if (untrustfulAccounts.includes(i - 1)) {
                $("#untrustfulAccount" + idx).show();
            }
            else {
                $("#untrustfulAccount" + idx).hide();
            }
            idx++;
        }
...
$(document).ready(function () {
    ...
    $("#numOfRaces").html(raceData.untrustfulAccounts.length
        + raceData.unsafeOperations.length
        + raceData.cosplayAccounts.length);
    ...
    $("#lineSave").on("click", function () {
        ...
        filter.config.report.filters.push(node);
        toastr.success('You just filtered some vulnerabilities from the report. Please click "Create a filter list" to persist the filter list into the configuration file.');
        filter.hideFilteredRaces(node);
    })
    ...
    $(".filter-function-toggle").on("click", function (e) {
        ...
        let confirmed = window.confirm('Are you sure to filter vulnerability calling "' + node.name + '" from the report?')
        if (!confirmed) return;
