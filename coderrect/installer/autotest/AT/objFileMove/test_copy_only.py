...
logFile = buildDir + '/test_copy_only.log'
tmpFile = buildDir + '/tmp.log'

class TestMove:
    def setup_class(self):
        os.system("cd " + home + " && \
                  rm -rf test_copy_only && \
                  mkdir test_copy_only && \
                  cp -r c-move test_copy_only/")
    ...
    def prepare_env(self, subdirs, targets):
        r = os.system("cd " + home + " && \
                  cd test_copy_only/c-move && \
                  cp Makefile.template.copy Makefile && \
