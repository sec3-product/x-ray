...
logFile = buildDir + '/test_move_copy.log'
tmpFile = buildDir + '/tmp.log'

class TestMove:
    def setup_class(self):
        os.system("cd " + home + " && \
                  rm -rf test_move_copy && \
                  mkdir test_move_copy && \
                  cp -r c-move test_move_copy/")
    ...
    def prepare_env(self, subdirs, targets):
        r = os.system("cd " + home + " && \
                  cd test_move_copy/c-move && \
                  cp Makefile.template.move_copy Makefile && \
