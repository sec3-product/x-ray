...
class TestCoderrect():
    ...
    def test_cblosc(self):
        ...
        r = os.system("cd " + path + " && \
                      ...
                      cd build && \
                      cmake .. > " + logFile + " 2>&1 && \
                      coderrect -e test_nthreads make -j $(nproc) >> " + logFile + " 2>&1")
        os.system("cp " + logFile + " " + TestCoderrect.report)
        os.system("cd " + path + " && \
    ...
    #@pytest.mark.timeout(96)
    def test_graphblas(self):
        print("\ntest case: test_graphblas", end=" ")
        proj_grr = "graphblas_recreate_race"
        path_redisGraph = TestCoderrect.home + "/RedisGraph/"
        path_grr = path_redisGraph + proj_grr
        logFile_grr = path_grr + "/at_" + proj_grr + ".log"
        r = os.system("cd " + path_redisGraph + " && \
                      rm -rf graphblas_recreate_race && \
                      tar -xf graphblas_recreate_race.tar && \
                      cd " + path_grr + " && \
                      coderrect -p /home/ubuntu/repos/RedisGraph make -j $(nproc) > " + logFile_grr + " 2>&1")
