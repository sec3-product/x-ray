...
func createRaceJson(src, dst string) error {
	...
	version := Version{
		...
		EnableMismatchedAPIs: conflib.GetBool("enableMismatchedAPI", true),
	}
...
func showSummary(raceJsonPath string) error {
	numOfOMP := 0
	numOfNormalDataRaces := 0
	numOfDeadlocks := 0
	...
	if total == 0 {
		fmt.Println("No race is detected.")
		return nil
	}
...
func diffRaceJSON(baseRawJSONPath string, currentRawJSONPath string, appendFilePath string) (bool, error) {
	...
	var newRaceCondition []int = make([]int, 0)
	for idx, v := range currentParser.RaceConditionSignatureList {
		_, ok := baseParser.RaceConditionSignatureMap[v]
		if !ok {
			newRaceCondition = append(newRaceCondition, idx)
		}
	}

	var newToctous []int = make([]int, 0)
	for idx, v := range currentParser.TocTousSignatureList {
		_, ok := baseParser.TocTousSignatureMap[v]
		if !ok {
			newToctous = append(newToctous, idx)
		}
	}

	var newMismatchedAPI []int = make([]int, 0)
	for idx, v := range currentParser.MismatchedAPISignatureList {
		_, ok := baseParser.MismatchedAPISignatureMap[v]
		if !ok {
			newMismatchedAPI = append(newMismatchedAPI, idx)
		}
	}

	foundNewBugs := false
	if len(newDataRace) != 0 || len(newToctous) != 0 || len(newMismatchedAPI) != 0 || len(newRaceCondition) != 0 {
		foundNewBugs = true
	}
	...
	jsonBytes, err = json.Marshal(newRaceCondition)
	str = "newRaceCondition = " + string(jsonBytes) + ";\n"
	if _, err = fd.Write([]byte(str)); err != nil {
		return foundNewBugs, err
	}

	jsonBytes, err = json.Marshal(newToctous)
	str = "newToctous = " + string(jsonBytes) + ";\n"
	if _, err = fd.Write([]byte(str)); err != nil {
		return foundNewBugs, err
	}

	jsonBytes, err = json.Marshal(newMismatchedAPI)
	str = "newMismatchedAPIs = " + string(jsonBytes) + ";\n"
	if _, err = fd.Write([]byte(str)); err != nil {
		return foundNewBugs, err
	}
...
func showHelpInfo(reportDir string) {
	if isCreateHTMLReport(reportDir) {
		...
		if commandExists("browse") {
			fmt.Printf("To check the race report in your browser, run \"browse %s/index.html\"\n",
				reportDir)
		} else if commandExists("xdg-open") {
			fmt.Printf("To check the race report in your browser, run \"xdg-open %s/index.html\"\n",
				reportDir)
		} else {
			if runtime.GOOS == "windows" {
				fmt.Printf("To check the race report, please open '%s\\index.html' in your browser\n",
					reportDir)
			} else {
				fmt.Printf("To check the race report, please open '%s/index.html' in your browser\n",
					reportDir)
	...
	if !conflib.GetBool("report.enableTerminal", false) {
		fmt.Println("")
		fmt.Println("If you want coderrect to show race details to terminal, please add '-t' to the command line like:")
		fmt.Println("")
...
func main() {
	...
	for i := 0; i < len(indexInfo.Executables); i++ {
		...
		if isCreateHTMLReport {
			...
			foundNewBugs, err := diffRaceJSON(baseRawJSONPath, rawRaceJSONPath, raceJSONPath)
			if err != nil {
		...
		fmt.Printf("\n----------------------------The summary of races in %s------------------------\n", executable.Name)
		// show the summary of analysis
		showSummary(rawRaceJSONPath)
		fmt.Print("\n\n")
	} // for each executable
