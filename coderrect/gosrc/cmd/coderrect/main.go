...
var usage = `
usage: coderrect [option]... <your build command line>
...
Options:
  ...
  -analyzeAll
        Let coderrect analyze all qualified binaries generated by the project.
  ...
  -mode=fast|normal|exhaust
        Specify the race detecting mode. "fast" prefers analyzing speed; 
  ...
  -continueIfBuildErrorPresents
        By default, coderrect terminates immediately when build encounters
        errors. This flag lets coderrect continue the race detection.

  -bcPaths=path1,path2
        Specify a list of comma separated directories where coderrect searchs 
        for BC files.

Examples:

1. Detect races against helloworld.c
$ coderrect gcc -fopenmp -g path/to/helloworld.c

2. Detect races against a Makefile-based project
$ coderrect path/to/makefile/directory

3. Detect races against a Makefile-based project and create 
   the report under the folder "myreport"
$ coderrect -o myreport -e LULESH make -DUSE_MPI=0  

4. Detect races against an executable "pi". It's useful when
   the project generates multiple executables.
$ coderrect -e pi make path/to/math/suite
`
...
func doCustomBuild(args []string, isCargoBuild bool) (*exec.Cmd, string) {
	...
	os.Setenv("RUSTFLAGS", "-Zsymbol-mangling-version=v0 --emit=llvm-ir -C save-temps -C opt-level=1")
...
func getAllExecFromCargoBuild(buildDir string) []string {
	...
	//
	binDir := filepath.Join(buildDir, "debug", "deps")
	...
	exampleDir := filepath.Join(buildDir, "debug", "examples")
...
func getCargoTargetBcFile(buildDir string, exeName string) string {
	...
	matched, llPath := searchFile(filepath.Join(buildDir, "debug", "deps"), g)
	if matched {
		return llPath
	}
	matched, llPath = searchFile(filepath.Join(buildDir, "debug", "examples"), g)
	if matched {
		return llPath
	}
...
func main() {
	...
	if conflib.GetBool("publish.jenkins", false) || host != "" {
		...
		fmt.Println("No race detected, no report to publish")
		os.Exit(0)
	}

	isCargoBuild := false // This is c direct Cargo Build
	// isCargoCustomBuild := false // This is a custom build that includes cargo
	if len(remainingArgs) > 0 && remainingArgs[0] == "cargo" {
		isCargoBuild = true
	}
	...
	for i := 0; i < len(executablePathList); i++ {
		...
		raceTypes := []string{"dataRaces", "raceConditions", "mismatchedAPIs", "toctou"}
