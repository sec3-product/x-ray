...
func apply(report FullReport, filters Filters) FullReport {
	...
	newTocTous := make([]DataRace, 0)
	for _, tt := range report.TocTous {
		if filterSharedObj(&tt.SharedObj, filters.Vars) ||
			filterThreadAccess(&tt.Access1, filters.Lines, filters.Functions) ||
			filterThreadAccess(&tt.Access2, filters.Lines, filters.Functions) {
			continue
		} else {
			newTocTous = append(newTocTous, tt)
		}
	}
	report.DataRaces = newDataRaces
	report.TocTous = newTocTous
	return report
}
