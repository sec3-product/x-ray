...
std::set<StringRef> UnrollThreadCreateLoopPass::THREAD_CREATE_API = {"pthread_create"};

static bool containsAny(const StringRef src, const std::set<llvm::StringRef> &list) {
    for (auto e : list) {
        if (src.contains(e)) {
            return true;
        }
...
bool UnrollThreadCreateLoopPass::runOnLoop(Loop *L, LPPassManager &LPM) {
    ...
    if (isConfiguredIndirectAPI(F)) return false;
    ...
    for (auto BB : L->getBlocks()) {
        for (BasicBlock::const_iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI) {
            ...
            if (isa<CallBase>(inst)) {
                ...
                if (CS.getCalledFunction() == nullptr) continue;

                std::string fname = demangle(CS.getCalledFunction()->getName().str());
                if (containsAny(fname, THREAD_CREATE_API) ||
                    CS.getCalledFunction()->getName().startswith(".coderrect.thread.create.")) {
                    // llvm::outs() << "======================================="
                    //              << "\n";
                    // llvm::outs() << "function: " << inst->getFunction()->getName() << "\n";
                    // llvm::outs() << "can peel? " << canPeel(L) << "\n";
                    // llvm::outs() << "is loop in simplified form? " << L->isLoopSimplifyForm() << "\n";
                    // llvm::outs() << "does loop have single exit? "
...
static RegisterPass<UnrollThreadCreateLoopPass> UPCL("", "Unroll Thread Creation Loop in IR", false, /*CFG only*/
                                                     false /*is analysis*/);
