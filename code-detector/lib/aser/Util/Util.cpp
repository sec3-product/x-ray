...
static bool isTypeEqual(const llvm::Type *T1, const llvm::Type *T2, const DataLayout &DL) {
    ...
    if (T1->isStructTy() && T2->isStructTy()) {
        ...
        // if every element types of the two types is the same
...
bool aser::isZeroOffsetTypeInRootType(const Type *rootType, const Type *elemType, const DataLayout &DL) {
...
llvm::StructType *aser::getConvertedFlexibleArrayType(const llvm::StructType *ST, llvm::Type *flexibleArrayElem) {
