...
bool RewriteModeledAPIPass::runOnFunction(llvm::Function &F) {
    ...
    // TODO: in the future, different API can be transformed in unified form so that they can get analyzed simpler
    // e.g., std::set::insert() and std::vector::push_back() are the same to pointer analysis
    VectorAPI API(&F);
    ...
    if (CONFIG_VTABLE_MODE) {
        ...
        if (!demangler.partialDemangle(F.getName())) {
            ...
            if (demangler.isCtor()) {
                ...
                for (auto &BB : F) {
                    for (auto &I : BB) {
                        // there should be a store instruction that stores the vtable into the object
                        if (auto SI = dyn_cast<StoreInst>(&I)) {
                            ...
                            if (SI->getPointerOperand()->stripInBoundsConstantOffsets() == F.arg_begin() &&
                                ...
                                // covert the store of the vtable into a special function call so that it
                                // can get intercepted by the memory model and we can then perform special handling on it
                                auto vtInit = F.getParent()->getOrInsertFunction(
                                    ".coderrect.vtable.init", llvm::Type::getVoidTy(F.getContext()),
