...
#define MAX_NUM_BB_FOR_GETTER_SETTER                                           \
  3 // minumal number of basic block for a simple loop
...
static bool inlineHeapWrapperAPIs(Module &M, set<StringRef> &HeapAPIs) {
  ...
  while (!workList.empty()) {
    ...
    for (User *user : allocFun->users()) {
      ...
      // use some heuristic here to infer whether is a customized heap
      // allocation wrapper
      ...
      if (isItaniumEncoding(caller->getName())) {
        ...
        if (demangler.partialDemangle(caller->getName().begin()))
          continue;
      ...
      if (!(baseNameRef.contains("alloc") || baseNameRef.contains("Alloc") ||
            baseNameRef.contains("new") || baseNameRef.contains("New"))) {
      ...
      // 3rd, there should be a use-def chain from allocation site to the
      // returned value
      SmallVector<Value *, 16> useChain;
...
static bool inlineHeapFreeWrapperAPIs(Module &M,
                                      const set<StringRef> &freeAPIs) {
  ItaniumPartialDemangler demangler;
  ...
  for (auto it = freeAPIs.begin(), ie = freeAPIs.end(); it != ie; it++) {
    ...
    for (auto user : freeFun->users()) {
      if (auto call = dyn_cast<CallBase>(user)) {
        if (call->getFunction()->getInstructionCount() >
            MAX_WRAP_INSTRUCTION_COUNT) {
          continue;
        }
        ...
        if (isa<Argument>(call->getArgOperand(0)->stripInBoundsOffsets())) {
          // this is a wrapper
          LOG_TRACE("Wrapper Free API. name={}",
  ...
  while (!externAPIs.empty()) {
    ...
    for (; it != ie; it++) {
      if (auto call = dyn_cast<CallBase>(*it)) {
        ...
        if (wrapper->getNumUses() == 1 ||
            wrapper->hasFnAttribute(Attribute::AlwaysInline)) {
        ...
        // avoid inlining too complicated function, this will harm the
        // performance the number can be adjusted...
...
bool WrapperFunIdentifyPass::runOnModule(llvm::Module &M) {
  ...
  set<StringRef> HeapAPIs{"malloc",         "calloc", "memalign",
                          "posix_memalign", "_Znwm",  "_Znam"};
...
static RegisterPass<WrapperFunIdentifyPass>
    HWIP("", "Passes that identifies the heap wrapper APIs", true, /*CFG only*/
         false /*is analysis*/);
