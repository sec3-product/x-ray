...
static Function *getCallocFunction(Module &M, Type *retType, Type *para1,
                                   Type *para2) {
  ...
  if (calloc == nullptr) {
    ...
    if (auto GA = dyn_cast<GlobalAlias>(tmp.getCallee())) {
      FunctionCallee callocTmp =
          M.getOrInsertFunction(".calloc.tmp", retType, para2, para2);
      ...
      auto call = builder.CreateCall(
          callocTmp, {aliasee->arg_begin(), aliasee->arg_begin() + 1});
...
static bool identifyOverridenHeapAPIs(Module &M, set<StringRef> &HeapAPIs) {
  ...
  for (Function &F : M) {
    ...
    if (HeapAPIs.find(F.getName()) != HeapAPIs.end())
      continue;
    ...
    // for C++
    if (!isItaniumEncoding(funName) ||
        demangler.partialDemangle(funName.begin()))
      continue;
    ...
    if (baseNameRef.startswith("operator new")) {
      ...
      FunctionCallee Znwm =
          M.getOrInsertFunction("_Znwm", i8ptr, F.arg_begin()->getType());
      if (!F.isDeclaration()) {
    ...
    if (ctxNameRef.startswith("std::allocator_traits<")) {
      if (baseNameRef.startswith("allocate")) {
        ...
        Function *calloc =
            getCallocFunction(M, i8ptr, num->getType(), num->getType());
        ...
        uint64_t allocSize = M.getDataLayout().getTypeAllocSize(
            F.getReturnType()->getPointerElementType());
...
static RegisterPass<StandardHeapAPIRewritePass>
    SHARP("", "Rewrite overrides of malloc-like APIs", false,
          /*CFG only*/ false /*is analysis*/);
