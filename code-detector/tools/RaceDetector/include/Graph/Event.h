...
// Each logical Type of instruction has its own abstract class that defines its interface
// See below the Event class for details
class Event {
private:
    EventType type;
    TID tid;
...
public:
    Event() = delete;
    Event(const llvm::Instruction *const inst, const ctx *const context, EventType type, TID tid)
    ...
    inline const std::vector<std::string> getInlinedCallSiteStrings(bool isCpp) const {
        ...
        if (DILocation *Loc = inst->getDebugLoc()) {
            while (auto Loc2 = Loc->getInlinedAt()) {
                ...
                if (csStr.empty()) csStr = "unknown (inlined)";
                csStr = csStr + " [" + Loc2->getFilename().str() + ":" + std::to_string(Loc2->getLine()) + "]";
                ...
                // llvm::outs() << "InlinedCallSiteString call: " << funcName << " at Dir: " << Dir
...
// Each logical Type of instruction has its own abstract class that defines its interface
// Different implementations of that event type may implement that interface
// For example,
//   The ReadEvent class has a virtual method getPointerOperand that returns the llvm::Value the event is reading.
//   The LoadEvent implements the ReadEvent interface and defines this function for a LoadInst.
//   The VecReadEvent is also a read but has a different implementation to support reading from std::vector
//   Anyone can add a new ReadEvent type by inheriting from ReadEvent and implementing its interface

class MemAccessEvent : public Event {
    LocksetManager::ID locksetID;

public:
    explicit MemAccessEvent(const ctx *const context, const llvm::Instruction *const inst, EventType type, TID tid,
                            LocksetManager::ID locksetID)
...
public:
    explicit ReadEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid,
                       LocksetManager::ID locksetID)
...
public:
    explicit ApiReadEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid,
                          LocksetManager::ID locksetID, bool isCtor = false)
        : MemAccessEvent(context, inst, EventType::APIRead, tid, locksetID), isCtor(isCtor) {}
    const llvm::Value *getPointerOperand() const override {
        // if call or indirect call, return the first parameter
        // if (auto call = llvm::cast<llvm::CallBase>(this->getInst())) return call->getArgOperand(0);
        if (auto call = llvm::dyn_cast<llvm::CallBase>(getInst())) {
            if (auto callee = llvm::dyn_cast<Function>(call->getCalledOperand()->stripPointerCasts())) {
                if (callee->getFunction().getName().equals("f90_str_copy_klen")) {
                    // %46 = call i32 (i32, i8*, i64, i8*, i64, ...) %45(i32 1, i8* src, i64 100, i8* dst, i64 50), !dbg
                    // !44
                    return call->getArgOperand(3);
                }
            }
            if (call->getCalledFunction()->getName().equals("st.model.instVar")) {
                //%5 = call i8* @st.model.instVar(i8* %0, i8* getelementptr inbounds ([11 x i8], [11 x i8]*
...
public:
    explicit WriteEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid,
                        LocksetManager::ID locksetID)
...
public:
    explicit ApiWriteEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid,
                           LocksetManager::ID locksetID)
...
public:
    ...
    // is that for intercepted callsite, the instruction may not directly reflect the function that gets called
    // For example, pthread_create(t, NULL, func), the actual function gets called is `func`
    // However, the `inst` should be used as callsite
    // because for any callgraph node, there will be multiple callsites.
    // NOTE: the context for CallEvent seems not matter for now
    explicit CallEvent(const ctx *const context, const llvm::Instruction *const inst, const CallGraphNodeTy *callNode,
                       TID tid)
        : Event(inst, context, EventType::Call, tid), callNode(callNode), endId(0) {}
    ...
    explicit CallEvent(const ctx *const context, const llvm::Instruction *const inst, llvm::StringRef funName, TID tid)
        : Event(inst, context, EventType::Call, tid), callNode(nullptr), funcName(funName), endId(0) {}
    ...
    std::string getCallSiteString(bool isCpp) {
        ...
        if (loc) csStr = csStr + " [" + loc->getFilename().str() + ":" + std::to_string(loc->getLine()) + "]";
        return csStr;
    }
...
public:
    explicit ForkEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid)
...
public:
    explicit LockEvent(const ctx *const context, const llvm::Instruction *const inst, TID tid,
                       LocksetManager::ID locksetID, const llvm::Value *lockPtr, EventType type)
        : Event(inst, context, type, tid), lockPtr(lockPtr), locksetID(locksetID) {}
