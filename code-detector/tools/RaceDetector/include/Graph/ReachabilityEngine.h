...
public:
    ...
    void addEdge(std::string sid1, std::string sid2) {
        ...
        if (DEBUG_HB) llvm::outs() << "ReachEngine add edge: " << sid1 << " -> " << sid2 << "\n";
        ID id1 = getId(sid1);
        ID id2 = getId(sid2);
    ...
    bool canReach(std::string sid1, std::string sid2) {
        ...
        if (DEBUG_HB) llvm::outs() << "ReachEngine query edge: " << sid1 << " -> " << sid2 << "\n";
        ...
        if (unreachable.find(sig) != unreachable.end()) {
            ...
            if (DEBUG_HB) llvm::outs() << "ReachEngine query edge: " << sid1 << " -> " << sid2 << " false\n";
            return false;

        } else if (hasEdge(id1, id2)) {
            // LOG_INFO("Result: REACHABLE");
            if (DEBUG_HB) llvm::outs() << "ReachEngine query edge: " << sid1 << " -> " << sid2 << " true\n";
            return true;
...
#pragma omp critical(edgeSetMap)
                        ...
                        for (ID nextID : s) {
                            if (visited.test_and_set(nextID) &&
                                unreachable.find(hashIDPair(nextID, id2)) == unreachable.end())
                                stack.push(nextID);
                        }
                    }
                }
