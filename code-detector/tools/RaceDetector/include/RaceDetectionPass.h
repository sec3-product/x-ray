...
private:
    ...
    bool addThreadStartFunction(const llvm::Function *func) {
        if (threadStartFunctions.find(func) != threadStartFunctions.end()) {
            if (DEBUG_RUST_API) llvm::outs() << "addThreadStartFunction false: " << func->getName() << "\n";
            return false;
        } else {
            if (DEBUG_RUST_API) llvm::outs() << "addThreadStartFunction: " << func->getName() << "\n";
    ...
    bool hasThreadStartInitFunction(std::string symbol) {
        for (auto func : threadStartFunctions) {
            if (func->getName().contains(symbol)) {
                if (DEBUG_RUST_API)
                    llvm::outs() << "hasThreadStartInitFunction: " << func->getName() << " " << symbol << "\n";
                return true;
            }
        }
        if (DEBUG_RUST_API) llvm::outs() << "hasThreadStartInitFunction false: " << symbol << "\n";
        return false;
    }
    ...
    bool isGlobalStateAccount(llvm::StringRef accountName) {
        if (globalStateAccounts.find(accountName) != globalStateAccounts.end()) {
            if (DEBUG_RUST_API) llvm::outs() << "isGlobalStateAccount: " << accountName << "\n";

            return true;
        } else {
            if (DEBUG_RUST_API) llvm::outs() << "isGlobalStateAccount false: " << accountName << "\n";
        }
        return false;
    }
    std::map<const llvm::Function *, std::vector<std::pair<llvm::StringRef, llvm::StringRef>>> funcArgTypesMap;
    bool hasValueLessMoreThan(const llvm::Value *value, const llvm::Instruction *inst, bool isLess);
    ...
    std::map<const llvm::Function *, std::vector<std::pair<llvm::StringRef, llvm::StringRef>>>
    ...
    bool accountTypeContainsMoreThanOneMint(llvm::StringRef struct_name) {
        ...
        if (func_struct) {
            ...
            for (auto pair : fieldTypes) {
                if (pair.first.contains("mint") && pair.second.equals("Pubkey")) num_mints++;
            }
        }
        if (num_mints > 1) return true;

        return false;
    }
    bool isAnchorStructFunction(const llvm::Function *func) {
        return anchorStructFunctionFieldsMap.find(func) != anchorStructFunctionFieldsMap.end();
    }
    bool isAnchorTokenProgram(llvm::StringRef accountName) {
        for (auto [func, fieldTypes] : anchorStructFunctionFieldsMap) {
            for (auto pair : fieldTypes) {
                if (pair.first.equals(accountName) && pair.second.equals("Program<'info, Token>")) return true;
            }
        }
        return false;
    }
    bool isAnchorTokenAccount(llvm::StringRef accountName) {
        for (auto [func, fieldTypes] : anchorStructFunctionFieldsMap) {
            for (auto pair : fieldTypes) {
                if (pair.first.equals(accountName) && pair.second.contains("TokenAccount>")) return true;
            }
        }
        return false;
    }
    ...
    bool isAnchorDataAccount(llvm::StringRef accountName) {
        for (auto [func, fieldTypes] : anchorStructFunctionFieldsMap) {
            for (auto pair : fieldTypes) {
                if (pair.first.equals(accountName) &&
                    (pair.second.startswith("Box<Account<'info") || pair.second.startswith("Account<'info")))
                    return true;
            }
        }
        return false;
    ...
    bool isAccessControlInstruction(llvm::StringRef sig) {
        for (auto [name, e] : accessControlMap) {
            if (name.contains(sig)) {
                if (DEBUG_RUST_API) llvm::outs() << "isAccessControlInstruction: " << sig << "\n";
                return true;
            }
        }
        if (DEBUG_RUST_API) llvm::outs() << "isAccessControlInstruction false: " << sig << "\n";

        return false;
    }

    std::map<const llvm::Function *, std::set<llvm::StringRef>> potentialInitOrderRelatedAccountsMap;
    std::map<const llvm::Function *, std::set<llvm::StringRef>> potentialCancelOrderRelatedAccountsMap;
    std::map<const llvm::Function *, std::set<llvm::StringRef>> potentialExchangeOrderRelatedAccountsMap;
    ...
    bool isAccountPDA(llvm::StringRef accountName) {
        for (auto [accountPda, inst] : accountsPDAMap) {
            if (accountPda.contains(accountName)) {
                if (DEBUG_RUST_API) llvm::outs() << "isAccountPDA: " << accountName << "\n";
                return true;
            }
        }
        if (DEBUG_RUST_API) llvm::outs() << "isAccountPDA false: " << accountName << "\n";
    ...
    bool isAccountUsedInSeedsProgramAddress(llvm::StringRef accountName) {
        if (accountsSeedProgramAddressMap.find(accountName) != accountsSeedProgramAddressMap.end()) {
            if (DEBUG_RUST_API) llvm::outs() << "isAccountUsedInSeedsProgramAddress: " << accountName << "\n";
            return true;
        }
        if (DEBUG_RUST_API) llvm::outs() << "isAccountUsedInSeedsProgramAddress false: " << accountName << "\n";

        return false;
    }
    std::map<std::pair<llvm::StringRef, llvm::StringRef>, const Event *> assertKeyNotEqualMap;
    bool isAccountKeyNotEqual(llvm::StringRef accountName1, llvm::StringRef accountName2) {
        for (auto [pair, inst] : assertKeyNotEqualMap) {
            if ((pair.first.contains(accountName1.str() + ".key") &&
                ...
                if (DEBUG_RUST_API)
                    llvm::outs() << "isAccountKeyNotEqual: " << accountName1 << " " << accountName2 << "\n";
                return true;
            }
        }

        if (DEBUG_RUST_API)
            llvm::outs() << "isAccountKeyNotEqual false: " << accountName1 << " " << accountName2 << "\n";

        return false;
    }

    void updateKeyEqualMap(StaticThread *thread, const Event *e, bool isEqual, bool isNotEqual,
    ...
    void handleConditionalCheck0(const aser::ctx *ctx, TID tid, const llvm::Function *func,
                                 const llvm::Instruction *inst, StaticThread *thread, const llvm::Value *value);
    const llvm::Function *findCallStackNonAnonFunc(const Event *e);
    llvm::StringRef findCallStackAccountAliasName(const llvm::Function *func, const Event *e, llvm::StringRef valueName,
                                                  bool stripKey = true);

    llvm::StringRef findNewStructAccountName(TID tid, const llvm::Instruction *inst, const llvm::StringRef name);

    void traverseFunctionWrapper(const aser::ctx *ctx, StaticThread *thread,
                                 std::vector<const llvm::Function *> &callStack, const llvm::Instruction *inst,
                                 const llvm::Function *f,
                                 std::map<uint8_t, const llvm::Constant *> *constArgs = nullptr);

    std::map<const llvm::Function *, std::set<const llvm::Function *>> exclusiveFunctionsMap;
...
public:
    ...
    std::map<unsigned int, std::map<TID, std::vector<MemAccessEvent *>>> memWrites;
    std::map<unsigned int, std::map<TID, std::vector<MemAccessEvent *>>> memReads;

    std::vector<unsigned int> sharedObjIdxs;
    std::map<unsigned int, std::map<TID, std::vector<MemAccessEvent *>>> memWritesMask;
    std::map<unsigned int, std::map<TID, std::vector<MemAccessEvent *>>> memReadsMask;
