...
bool StaticThread::isAccountCompatibleOrder(llvm::StringRef &accountName1,
  ...
  if (IDL_INSTRUCTION_ACCOUNTS.find(anchor_idl_instruction_name) !=
    ...
    // if this size less than idl size, it means some accounts have been
    // removed...
    auto size = size1;
    if (size1 > size2) {
      size = size2;
    ...
    for (uint i = 0; i < size; i++) {
      ...
      // llvm::outs() << "idl account_name: " << item.name << " isMut: " <<
      // item.isMut
      //              << " isSigner: " << item.isSigner << " isNested: " <<
      //              item.isNested << "\n";
      if (item.isNested) {
      } else {
        auto pair = anchorStructFunctionFields[i];
        // llvm::outs() << "current account_name: " << pair.first << " type: "
        // << pair.second << "\n";
        ...
        if (item.name != accountName_anchor) {
          ...
          for (uint j = i + 1; j < size; j++) {
            ...
            // llvm::outs() << "next account_name: " << pair2.first << " type: "
            // << pair2.second << "\n";
            auto accountName2_anchor = convertToAnchorString(pair2.first);
            if (item.name == accountName2_anchor) {
              if (DEBUG_RUST_API)
                llvm::outs() << "reordered account1: " << pair.first
                             << " account2: " << pair2.first << "\n";
...
int StaticThread::isAccountCompatibleAddOrMut(llvm::StringRef accountName) {
  if (DEBUG_RUST_API)
    llvm::outs() << "isAccountCompatibleAddOrMut anchor_idl_instruction_name: "
  ...
  if (IDL_INSTRUCTION_ACCOUNTS.find(anchor_idl_instruction_name) !=
    ...
    if (DEBUG_RUST_API)
      llvm::outs() << "accountName_anchor: " << accountName_anchor
                   << " isMut: " << isMut << "\n";
    {
      for (const auto &item : accounts) {
        // llvm::outs() << "account_name: " << item.name << " isMut: " <<
        // item.isMut << " isSigner: " << item.isSigner << "\n";
        if (item.name == accountName_anchor) {
          // llvm::outs() << "IDL has_account: " << accountName << "\n";
          if (isMut && !item.isMut) {
            if (DEBUG_RUST_API)
              llvm::outs() << "IDL isMut changed to: " << isMut << "\n";
            return 1;
          }
          return 0;
        }
