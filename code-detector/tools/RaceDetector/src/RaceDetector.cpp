...
cl::opt<bool> DebugIR("debug-ir", cl::desc("Load from modified.ll directly, no preprocessing and openlib rewriting"),
                      cl::init(false));
cl::opt<bool> SkipPreProcess("skip-preprocess", cl::desc("Do not run preprocessing pass"), cl::init(true));
cl::opt<bool> NoLinkStub("no-stub", cl::desc("Do not link stub files"), cl::init(true));
cl::opt<bool> SkipOverflowChecks("skip-overflow", cl::desc("skip overflow checks"));

cl::opt<std::string> TargetModulePath(cl::Positional, cl::desc("path to input bitcode file"));
cl::opt<bool> ConfigDumpIR("dump-ir", cl::desc("Dump the modified ir file"));
cl::opt<bool> ConfigNoFilter("no-filter", cl::desc("Turn off the filtering for race report"));
cl::opt<bool> ConfigIncludeAtomic("include-atomics", cl::desc("Include races on atomic operations"));
cl::opt<std::string> ConfigOutputPath("o", cl::desc("JSON output path"), cl::value_desc("path"));
cl::opt<bool> ConfigNoOMP("nomp", cl::desc("Turn off the OpenMP race detection"));
cl::opt<bool> ConfigNoAV("no-av", cl::desc("Turn off the atomicity violation detection"));
cl::opt<bool> ConfigNoOV("no-ov", cl::desc("Turn off the order violation detection"));
cl::opt<bool> ConfigCheckIdenticalWrites("check-identical-writes",
                                         cl::desc("Turn on detecting races bewteen identical writes"), cl::init(true));
cl::opt<bool> ConfigNoMissedOMP("no-missed-omp", cl::desc("Do not scan for missed omp regions"));
cl::opt<bool> ConfigNoMissAPI("no-miss-match-api", cl::desc("Turn off the miss-match api detection"));
cl::opt<bool> ConfigDebugLog("v", cl::desc("Turn off log to file"));
cl::opt<int> ConfigReportLimit("limit", cl::desc("Max number of races can be reported"), cl::value_desc("number"),
                               cl::init(-1));

cl::opt<bool> ConfigIgnoreReadWriteRaces("ignore-rw", cl::desc("Ignore read-write races"));
cl::opt<bool> ConfigIgnoreWriteWriteRaces("ignore-ww", cl::desc("Ignore write-write races"));

cl::opt<bool> ConfigNoReportLimit("no-limit", cl::desc("No limit for the number of races reported"));
cl::opt<bool> ConfigAnalyzeAPI("analyze-api", cl::desc("Analyze all public apis in the library"));
cl::opt<bool> ConfigPrintAPI("print-api", cl::desc("Print all public apis in the library"));
cl::opt<bool> ConfigOpenLibOnce("openlib-once", cl::desc("Analyze only once for every public api in the library"));
cl::opt<bool> ConfigOptimalAPI("openlib-optimal", cl::desc("Analyze all public synchronization apis in the library"));
cl::opt<bool> ConfigForkAPI("openlib-fork", cl::desc("Analyze all smalltalk fork apis in the library"));

cl::opt<size_t> MaxIndirectTarget("max-indirect-target", cl::init(2),
                                  cl::desc("max number of indirect call target that can be resolved by indirect call"));
cl::opt<bool> ConfigLoopUnroll("Xloop-unroll", cl::desc("Turn on unrolling the thread creation loops"));
cl::opt<bool> ConfigNoFalseAlias("Xno-false-alias", cl::desc("Turn off checking false alias"));
cl::opt<bool> ConfigNoProducerConsumer("Xno-producer-consumer", cl::desc("Turn off checking producer consumer"));
cl::opt<bool> ConfigEntryPointOnce("entry-point-once", cl::desc("Create only one thread for each entry point"));
cl::opt<bool> ConfigPrintImmediately("Xprint-fast", cl::desc("Print races immediately on terminal"));
cl::opt<bool> ConfigTerminateImmediately("one-race", cl::desc("Print a race and terminate immediately"));
cl::opt<bool> ConfigPrintCoverage("Xprint-cov", cl::desc("Print analysis code coverage"));

cl::opt<bool> ConfigIntegrateDynamic("Xdynamic", cl::desc("Store race results for dynamic analysis"));

cl::opt<bool> ConfigShowSummary("Xshow-race-summary", cl::desc("show race summary"));
cl::opt<bool> ConfigShowDetail("Xshow-race-detail", cl::desc("show race details"));
cl::opt<bool> ConfigShowAllTerminal("t", cl::desc("show race detail and summary on the terminal"));

cl::opt<bool> ConfigKeepLockPath("Xkeep-lock-path", cl::desc("Turn on exploring all lock paths"));

cl::opt<size_t> PTAAnonLimit("Xpta-anon-limit", cl::init(10000),
                             cl::desc("max number of anonymous abjects to allocate in pointer analysis. (Use this if "
...
cl::opt<bool> ConfigIgnoreRepeatedMainCallStack("skip-repeat-cs", cl::desc("Skip repeated call stack in main thread"));
cl::opt<bool> ConfigNoOrigin("no-origin", cl::desc("Use context-insensitive pointer analysis"));
cl::opt<bool> ConfigDebugThread("debug-threads", cl::desc("Turn on debug thread logs"));
cl::opt<bool> ConfigDebugRace("debug-race", cl::desc("Turn on debug race logs"));
cl::opt<bool> ConfigDebugOMPRace("debug-omp-race", cl::desc("Turn on debug openmp race logs"));
cl::opt<bool> ConfigDebugCS("debug-cs", cl::desc("Turn on debug call stack"));
cl::opt<bool> ConfigShowCallStack("show-call-stack", cl::desc("Print call stack in the graph"));
cl::opt<bool> ConfigShowLockStack("show-lock-stack", cl::desc("Print lock stack in the graph"));
cl::opt<bool> ConfigDumpLockSet("debug-dump-lockset", cl::desc("Print lock set in the graph"));

cl::opt<bool> ConfigDebugIndirectCall("debug-indirect", cl::desc("Turn on debug indirect call"));
cl::opt<bool> ConfigDebugIndirectCallAll("debug-indirect-all", cl::desc("Turn on debug indirect call"));
cl::opt<bool> ConfigDebugPTA("debug-pta", cl::desc("Turn on debug pointer analysis"));
cl::opt<bool> ConfigDebugPTAVerbose("debug-pta-verbose", cl::desc("Turn on debug pointer analysis verbose mode"));

cl::list<std::string> ConfigDebugDebugDebug("debug-focus", cl::CommaSeparated, cl::desc("debug focus mode"));

cl::opt<bool> ConfigDebugLock("debug-lock", cl::desc("Turn on debug lock set"));
cl::opt<bool> ConfigDebugLockStr("debug-lock-str", cl::desc("Turn on debug lock string"));
cl::opt<bool> ConfigDebugRustAPI("debug-sol", cl::desc("Turn on debug sol api"));
cl::opt<bool> ConfigDebugAPI("debug-api", cl::desc("Turn on debug open API"));

cl::opt<bool> ConfigDebugHB("debug-hb", cl::desc("Turn on debug happens-before"));
cl::opt<bool> ConfigDebugEvent("debug-event", cl::desc("Turn on debug read-write"));
cl::opt<bool> ConfigDebugHappenInParallel("debug-hip", cl::desc("Turn on debug happen-in-parallel"));
cl::opt<bool> ConfigIngoreLock("no-lock", cl::desc("Turn off the lockset check"));
cl::opt<bool> ConfigNoPathSensitive("no-ps", cl::desc("Turn off path-sensitive analysis"));

cl::opt<bool> ConfigNoOldOMPAliasAnalysis("Xno-old-omp-alias", cl::desc("Turn off the old OMPIndexAlias analysis"));
cl::opt<bool> CONFIG_NO_KEYWORD_FILTER("Xno-svfFilter", cl::desc("Turn off keyword filter in SVF pass"));
cl::opt<bool> ConfigFlowFilter("flowFilter", cl::desc("Enable flow filter"));
cl::opt<bool> ConfigDisableProgress("no-progress", cl::desc("Does not print spinner progress"));
...
int SAME_FUNC_BUDGET_SIZE;  // keep at most x times per func per thread 10 by default
int FUNC_COUNT_BUDGET;      // 100,000 by default

bool USE_FAKE_MAIN = true;
// static OriginsSetter<3> OS{"pthread_create", "GrB_Matrix_new", "__kmpc_fork_call", "__kmpc_fork_teams"};
// using Model = GraphBLASModel<KOrigin<1>>;
...
const llvm::Function *getFunctionMatchStartEndName(llvm::StringRef startName, llvm::StringRef endName) {
    for (auto [name, func] : FUNC_NAME_MAP) {
        if (name.startswith(startName) && name.endswith(endName) && !name.contains(".anon.")) return func;
    }
    return nullptr;
}
// for smalltalk
std::set<std::string> SmallTalkReadAPINames{"st.next", "st.get:", "st.size", "st.notEmpty", "st.includes:"};
std::set<std::string> SmallTalkWriteAPINames{"st.nextPut:", "st.removeKey:", "st.at:put:", "st.removeFirst", "st.add:"};

std::set<std::string> SmallTalkCTRemoteClasses;

vector<string> DEBUG_FOCUS_VEC;

ModulePass *createOMPConstantPropagationPass();
namespace aser {
...
openlib::OpenLibConfig initOpenLibConf(Module *module) {
    ...
    for (jsoncons::json &j : entrys) {
        switch (j.kind()) {
            ...
            default: {
                auto entryMap = j.as<std::map<std::string, std::map<std::string, bool>>>();
    ...
    config.explorePublicAPIs = config.mode | ConfigAnalyzeAPI | analyzeAPI;  // optimal always explore public apis
...
void initRaceDetect() {
    NONE_PARALLEL_FUNCTIONS = conflib::Get<map<string, string>>("notParallelFunctionPairs", {});

    CONFIG_IGNORE_READ_WRITE_RACES = ConfigIgnoreReadWriteRaces | conflib::Get<bool>("ignoreReadWriteRaces", false);
    CONFIG_IGNORE_WRITE_WRITE_RACES = ConfigIgnoreWriteWriteRaces | conflib::Get<bool>("ignoreWriteWriteRaces", false);

    IGNORED_FUN_ALL = conflib::Get<std::vector<std::string>>("ignoreRacesInFunctions", {});
    IGNORED_VAR_ALL = conflib::Get<std::vector<std::string>>("ignoreRaceVariables", {});
    IGNORED_LOCATION_ALL = conflib::Get<std::vector<std::string>>("ignoreRacesAtLocations", {});
    LOW_PRIORITY_FILE_NAMES = conflib::Get<std::vector<std::string>>("lowPriorityFiles", {});
    HIGH_PRIORITY_FILE_NAMES = conflib::Get<std::vector<std::string>>("highPriorityFiles", {});
    LOW_PRIORITY_VAR_NAMES = conflib::Get<std::vector<std::string>>("lowPriorityRaces", {});
    HIGH_PRIORITY_VAR_NAMES = conflib::Get<std::vector<std::string>>("highPriorityRaces", {});
...
static std::unique_ptr<Module> loadFile(const std::string &FN, LLVMContext &Context, bool abortOnFail) {
    SMDiagnostic Err;
    std::unique_ptr<Module> Result;
    if (DebugIR) {
        // false to ignore potentially broken IR, our preprocessing passes will make the IR unreadable
        // Result = parseIRFile(FN, Err, Context, false);
    ...
    // count line number
    {
        ...
        unsigned line_count = std::count(std::istream_iterator<char>(myfile), std::istream_iterator<char>(), '\n');
...
static bool linkFiles(LLVMContext &Context, aser::Linker &L, const std::vector<std::string> &Files, unsigned Flags) {
...
const std::set<llvm::StringRef> SKIPPED_APIS{
    "llvm.", "pthread_", "__kmpc_",         "_mp_",         ".omp.",  "omp_", "__clang_", "_ZNSt",
    "_ZSt",  "_ZNKSt",   "_ZN14coderrect_", "__coderrect_", "printf", "je_"};
...
int versionCompare(string v1, string v2) {         // v1: the real version: ^0.20.1
    ...
    v1.erase(std::remove_if(v1.begin(), v1.end(), [](char c) { return !(c >= '0' && c <= '9') && c != '.'; }),
             v1.end());
    // llvm::outs() << "v1: " << v1 << "\n";
    // llvm::outs() << "v2: " << v2 << "\n";
            ...
            for (auto &I : BB) {
                if (isa<CallBase>(&I)) {
                    ...
                    if (valueName1 == anchor_lang_version) {
                        if (versionCompare(valueName2.str(), "0.24.2") == -1) anchorVersionTooOld = true;
                    ...
                    if (valueName1 == spl_token_version) {
                        if (versionCompare(valueName2.str(), "3.1.1") == -1) splVersionTooOld = true;
                        llvm::outs() << "spl_version: " << valueName2 << " splVersionTooOld: " << splVersionTooOld
                                     << "\n";
                    }
                    ...
                    if (valueName1 == solana_program_version) {
                        if (versionCompare(valueName2.str(), "1.10.29") == -1) solanaVersionTooOld = true;
    ...
    if (j.is_array()) {
        ...
        for (const auto &item : j.array_range()) {
            ...
            if (item.contains("accounts")) {
                ...
                IDL_INSTRUCTION_ACCOUNTS[api_name].emplace_back(account_name, false, false, true);
                ...
                // llvm::outs() << "account_name: " << account_name << " isMut: " << isMut << " isSigner: " <<
                // isSigner<< "\n";
                IDL_INSTRUCTION_ACCOUNTS[api_name].emplace_back(account_name, isMut, isSigner);
            }
        }
    }
}
            ...
            for (auto &I : BB) {
                if (isa<CallBase>(&I)) {
                    ...
                    if (IDL_INSTRUCTION_ACCOUNTS.empty()) {
                        // mainnet only
                        // anchor --provider.cluster mainnet idl fetch nosRB8DUV67oLNrL45bo2pFLrmsWPiewe2Lk2DRNYCp -o
                        ...
                        auto cmd =
                            "/usr/bin/anchor --provider.cluster mainnet idl fetch " + address_str + +" -o " + path;
...
void getAllUnexploredFunctionsbyPartialName(std::set<const llvm::Function *> &result, StringRef sig) {
...
void reportUnexploredFunctions() {
    ...
    if (size > 0) {
        ...
        for (auto func : CR_UNExploredFunctions) {
            if (openlib::isInferredPublicAPI(func)) llvm::outs() << openlib::getCleanFunctionName(func) << "\n";
        }
    }
...
int main(int argc, char **argv) {
    // llvm::outs() << sizeof(std::pair<NodeID, NodeID>) << ", " << sizeof(std::pair<void *, void *>);
    // return 1;

    // InitLLVM will setup signal handler to print stack trace when the program crashes.
    ...
    // TODO: user specified heap api should be used to detect heap wrapper function as well!
    auto heapAPIs = conflib::Get<std::vector<std::string>>("heapAllocFunctions", {});

    CONFIG_MUST_EXPLORE_APIS = conflib::Get<std::vector<std::string>>("mustExploreFunctions", {});
    CONFIG_INDIRECT_APIS = conflib::Get<std::vector<std::string>>("indirectFunctions", {});
    CRITICAL_INDIRECT_TARGETS = conflib::Get<map<string, string>>("criticalIndirectTargets", {});
    if (true) {  // for solana sve
        ...
        SOLANA_SVE_DB = conflib::Get<std::map<std::string, std::map<std::string, std::string>>>("solana.sve", {});
        for (auto &[key, valueMap] : SOLANA_SVE_DB) {
            ...
            if (on == "false" || (isFreePlan && (free != "true" && freeSVEs.find(key) == freeSVEs.end())))
    ...
    DEBUG_FOCUS_VEC = ConfigDebugDebugDebug;  // conflib::Get<std::vector<std::string>>("debugdebugdebug", {});
    ...
    auto enableIdenticalWrites = conflib::Get<bool>("enableIdenticalWrites", false);
    ...
    auto enableImmediatePrint = conflib::Get<bool>("report.enableTerminal", false);
    auto enableShowRaceSummary = conflib::Get<bool>("enablePrintRaceSummary", false);
    auto enableShowRaceDetail = conflib::Get<bool>("enablePrintRaceDetail", false);
    ...
    CONFIG_CHECK_UncheckedAccount = conflib::Get<bool>("solana.account.UncheckedAccount", true);
    ...
    CONFIG_CHECK_IDENTICAL_WRITE = ConfigCheckIdenticalWrites | enableIdenticalWrites;

    CONFIG_INCLUDE_ATOMIC = ConfigIncludeAtomic | conflib::Get<bool>("includeAtomics", false);
    CONFIG_NO_FILTER = ConfigNoFilter | !enableFilter;
    CONFIG_NO_REPORT_LIMIT = ConfigNoReportLimit | (conflib::Get<int>("raceLimit", -1) < 0);
    ...
    PRINT_IMMEDIATELY = ConfigPrintImmediately | enableImmediatePrint | ConfigShowAllTerminal;
    TERMINATE_IMMEDIATELY = ConfigTerminateImmediately;
    CONFIG_SHOW_SUMMARY = ConfigShowSummary | enableShowRaceSummary | ConfigShowAllTerminal;  // show race summary
    CONFIG_SHOW_DETAIL = ConfigShowDetail | enableShowRaceDetail | ConfigShowAllTerminal;     // show race details
    ...
    for (auto f : skipFun) {
        llvm::Regex pat(f);
        if (pat.match("main")) {
            error("[ABORT] The main function will be excluded by the \"ignoreFunctions\" in the config file.");
    ...
    LLVMContext Context;
    // load it now as we need the triple information to determine which stub to link
    auto module = loadFile(TargetModulePath, Context, true);
    module->setModuleIdentifier("coderrect.stub.pid" + std::to_string(getpid()));

    // add -Xuse-fi-model-- this needs to be added to PTA configure before anything else
    if (!module->getFunction("main") && module->getFunction("MAIN_")) {
        CONFIG_USE_FI_MODE = true;  // for now fortran, use field-insensitive model by default
    ...
    if (!SkipPreProcess && !DebugIR) {
        ...
        preProcessor.runOnModule(*module, [&](llvm::legacy::PassManagerBase &MPM) -> void {
...
static llvm::RegisterPass<PointerAnalysisPass<PTA>> PAP("Pointer Analysis Wrapper Pass",
