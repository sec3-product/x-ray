...
extern void getAllUnexploredFunctionsbyPartialName(std::set<const llvm::Function *> &result, StringRef sig);

EventID Event::ID_counter = 0;
llvm::StringRef stripSelfAccountName(llvm::StringRef account_name) {
    if (account_name.find("self.") != string::npos) account_name = account_name.substr(5);
    return account_name;
}
...
llvm::StringRef getStructName(llvm::StringRef valueName) {
    ...
    auto struct_name = valueName.substr(found_left + 1, found_right - found_left - 1);
    // parse further
...
llvm::StringRef stripDotKey(llvm::StringRef valueName) {
    ...
    if (foundDotKey != string::npos) valueName = valueName.substr(0, foundDotKey);
    return valueName;
}

llvm::StringRef stripAtErrorAccountName(llvm::StringRef valueName) {
    auto foundAt = valueName.find("@");
    if (foundAt != string::npos) valueName = valueName.substr(0, foundAt);
    return valueName;
}
llvm::StringRef stripAccountName(llvm::StringRef valueName) {
    if (valueName.empty()) return valueName;
    while (valueName.front() == '*' || valueName.front() == '&' || valueName.front() == '(')
...
llvm::StringRef findMacroAccount(llvm::StringRef account) {
    ...
    account = stripAccountName(account);
    {
        ...
        if (found != string::npos) account = account.substr(found + 1);
    }
    {
        auto found = account.find_last_of(".");
        if (found != string::npos) account = account.substr(found + 1);
...
llvm::StringRef RaceDetectionPass::findNewStructAccountName(TID tid, const llvm::Instruction *inst,
    ...
    while (!CS2.getTargetFunction()->getName().startswith("sol.model.opaqueAssign") ||
    ...
    if (DEBUG_RUST_API)
        llvm::outs() << "tid: " << tid << " " << name << ": " << account_name << " call: " << *inst << "\n";
    return stripCtxAccountsName(account_name);
}

std::pair<llvm::StringRef, const llvm::Instruction *> getProgramIdAccountName(const llvm::Instruction *inst) {
    if (auto previousInst = dyn_cast<CallBase>(inst->getPrevNonDebugInstruction())) {
            ...
            if (valueName == "program_id") {
                ...
                while (prevInst) {
                    if (auto callValue2 = dyn_cast<CallBase>(prevInst)) {
                        ...
                        if (CS2.getTargetFunction()->getName().equals("sol.model.opaqueAssign")) {
                            ...
                            if (valueName1 == "program_id") {
                                ...
                                auto found = account.find_last_of(".");
                                if (found != string::npos) account = account.substr(0, found);
...
bool RaceDetectionPass::hasValueLessMoreThan(const llvm::Value *value, const llvm::Instruction *inst, bool isLess) {
    auto symbol = "<";
    if (!isLess) symbol = ">";
    auto valueLessThan = LangModel::findGlobalString(value).str() + symbol;
    std::string snippet = getSourceLoc(inst).getSnippet();
    snippet.erase(std::remove(snippet.begin(), snippet.end(), ' '), snippet.end());
    if (DEBUG_RUST_API) {
        llvm::outs() << "valueLessThan: " << valueLessThan << "\n";
        llvm::outs() << "snippet: \n" << snippet << "\n";
    }
    if (snippet.find(valueLessThan) != string::npos) return true;
    return false;
}
bool isUpper(const std::string &s) {
    return std::all_of(s.begin(), s.end(), [](unsigned char c) { return std::isupper(c) || c == '_'; });
}
bool is_all_capital_or_number(const std::string &s) {
    std::string::const_iterator it = s.begin();
    while (it != s.end() && (std::isupper(*it) || std::isdigit(*it) || *it == '_')) ++it;
    return !s.empty() && it == s.end();
}
bool is_all_capital(const std::string &s) {
    std::string::const_iterator it = s.begin();
    while (it != s.end() && (std::isupper(*it) || *it == '_')) ++it;
    return !s.empty() && it == s.end();
}
...
bool is_constant(const std::string &s) { return is_number(s) || is_all_capital(s); }
bool RaceDetectionPass::isSafeType(const llvm::Function *func, const llvm::Value *value) {
    if (auto arg = llvm::dyn_cast<llvm::Argument>(value)) {
        auto type = funcArgTypesMap[func][arg->getArgNo()].second;
        if (isUpper(type.str()) || type.size() >= 5 || type.startswith("i") || type.contains("Dec")) return true;
    }
    return false;
}
bool RaceDetectionPass::isSafeVariable(const llvm::Function *func, const llvm::Value *value) {
    if (auto arg = dyn_cast<Argument>(value)) {
        ...
        auto valueName = LangModel::findGlobalString(value);
        if (valueName.find("as i") != string::npos || valueName.find(".len") != string::npos ||
            valueName.find("_len") != string::npos || valueName.find("length_") != string::npos ||
            valueName.find("size") != string::npos || valueName.find("steps") != string::npos ||
            valueName.find("gas") != string::npos || valueName.find("bits") != string::npos ||
            valueName.find("shift") != string::npos ||
            (valueName.find("pos") != string::npos && valueName.find("iter") == string::npos)) {
            return true;
        }
    }
    return false;
}
const llvm::Function *RaceDetectionPass::findCallStackNonAnonFunc(const Event *e) {
    ...
    if (func->getName().contains(".anon.")) {
        ...
        for (; i >= 0; i--) {
            ...
            if (!func->getName().contains(".anon.")) break;
        }
    }
    return func;
}
llvm::StringRef RaceDetectionPass::findCallStackAccountAliasName(const llvm::Function *func, const Event *e,
                                                                 llvm::StringRef valueName, bool stripKey) {
    if (DEBUG_RUST_API) llvm::outs() << "finding alias account for: " << valueName << "\n";
    if (valueName.empty()) return valueName;
    // strip & or *
    while (valueName.front() == '*' || valueName.front() == '&') valueName = valueName.substr(1);
    auto callTrace = getCallEventStack(e, callEventTraces);
    int i = callTrace.size() - 1;  // start index
    // skip .anon.
    if (func->getName().contains(".anon.")) {
    ...
    for (; ArgNo < size; ArgNo++) {
        ...
        if (pair.first.equals(valueName)) break;
        if (valueName.startswith("ctx.") && pair.first.equals("ctx")) break;
        // skip ctx.accounts.
        if (valueName.contains(".")) {
            auto valueName2 = valueName.substr(0, valueName.find("."));
            if (pair.first.equals(valueName2)) break;
        }
    }
    if (ArgNo < size) {
        if (DEBUG_RUST_API) llvm::outs() << "OK - passed as parameter account: " << valueName << "\n";
        auto callTrace = getCallEventStack(e, callEventTraces);
        for (; i >= 0; i--) {
            ...
            if (auto call_site = dyn_cast<CallBase>(e_caller)) {
                ...
                // skip anon block
                if (CS4.getNumArgOperands() < ArgNo + 1) continue;
                if (CS4.getCalledFunction()->getName().contains(".anon.")) continue;
                ...
                if (auto arg_j = dyn_cast<Argument>(value_j)) {
                    ...
                    if (auto callValue_j = dyn_cast<CallBase>(value_j)) {
                        if (callValue_j->getCalledFunction()->getName().startswith("sol.clone.") ||
                            callValue_j->getCalledFunction()->getName().startswith("sol.to_account_info.") ||
                            callValue_j->getCalledFunction()->getName().startswith("sol.to_owned.")) {
                            ...
                            if (valueName_j.startswith("sol.")) valueName_j = valueName_j.substr(4);
                            if (valueName_j.endswith(".0"))
                                valueName_j = valueName_j.substr(0, valueName_j.str().length() - 2);
                            if (DEBUG_RUST_API) llvm::outs() << "Passed account1: " << valueName_j << "\n";
                            return valueName_j;
                        }
                    }
                    ...
                    if (found != string::npos)
                        if (stripKey) valueName_j = valueName_j.substr(0, found);

                    valueName_j = stripAll(valueName_j);

                    if (DEBUG_RUST_API) llvm::outs() << "Passed account2: " << valueName_j << "\n";
                    return valueName_j;
                    // break;
                }
            }
...
void RaceDetectionPass::addCheckKeyEqual(const aser::ctx *ctx, TID tid, const llvm::Instruction *inst,
                                         StaticThread *thread, CallSite &CS) {
    if (DEBUG_RUST_API) llvm::outs() << "assert_eq: " << *inst << "\n";
    // only deal with two or more arguments
    if (CS.getNumArgOperands() < 2) return;
    ...
    if (valueName1.contains(".key")) valueKey1 = true;
    if (valueName2.contains(".key")) valueKey2 = true;
    ...
    if (foundComma != string::npos) valueName2 = valueName2.substr(0, foundComma);
    ...
    if (valueKey1) {
        ...
        if (foundDot1 != string::npos) valueName1 = valueName1.substr(0, foundDot1);
    ...
    if (valueKey2) {
        ...
        if (foundDot2 != string::npos) valueName2 = valueName2.substr(0, foundDot2);
...
void RaceDetectionPass::handleConditionalCheck0(const aser::ctx *ctx, TID tid, const llvm::Function *func,
    ...
    if (auto callValue = dyn_cast<CallBase>(value)) {
        ...
        if (CS2.getTargetFunction()->getName().equals("sol.!=") ||
            ...
            // check for arguments
            if (auto arg1 = dyn_cast<Argument>(value1)) valueName1 = funcArgTypesMap[func][arg1->getArgNo()].first;
            if (auto arg2 = dyn_cast<Argument>(value2)) valueName2 = funcArgTypesMap[func][arg2->getArgNo()].first;

            auto e = graph->createReadEvent(ctx, inst, tid);
            // if ctx.accounts.authority.key != &token.owner

            // while, it could be passed as f(account.key)...
            auto account1_raw = findCallStackAccountAliasName(func, e, valueName1, false);
            auto account2_raw = findCallStackAccountAliasName(func, e, valueName2, false);

            if (DEBUG_RUST_API)
                llvm::outs() << "sol.if data1: " << valueName1 << " data2: " << valueName2
                             << " account1_raw: " << account1_raw << " account2_raw: " << account2_raw << "\n";

            bool valueKey1 = false;
            bool valueKey2 = false;
            if (valueName1.contains(".key") || account1_raw.contains(".key")) valueKey1 = true;
            if (valueName2.contains(".key") || account2_raw.contains(".key")) valueKey2 = true;
            if (valueName1.empty()) {
                if (auto callValue1 = dyn_cast<CallBase>(value1)) {
                    if (callValue1->getCalledFunction()->getName().startswith("sol.key.")) {
                        valueName1 = LangModel::findGlobalString(callValue1->getArgOperand(0));
                        valueKey1 = true;
                    }
                } else if (callValue1->getCalledFunction()->getName().startswith("sol.match.")) {
                    valueName1 = LangModel::findGlobalString(callValue1->getArgOperand(0));
                }
            }
            if (valueName2.empty()) {
                if (auto callValue2 = dyn_cast<CallBase>(value2)) {
                    if (callValue2->getCalledFunction()->getName().startswith("sol.key.")) {
                        valueName2 = LangModel::findGlobalString(callValue2->getArgOperand(0));
                        valueKey2 = true;
                    } else if (callValue2->getCalledFunction()->getName().startswith("sol.match.")) {
                        valueName2 = LangModel::findGlobalString(callValue2->getArgOperand(0));
                    }
                }
            }
            ...
            if (valueKey1 || valueKey2) {
                ...
                if (valueKey1) thread->assertKeyEqualMap[pair1] = e;
                if (valueKey2) thread->assertKeyEqualMap[pair2] = e;
                ...
                if (foundDot1 != string::npos) account1 = account1.substr(0, foundDot1);
                ...
                if (foundDot2 != string::npos) account2 = account2.substr(0, foundDot2);
                ...
                if (valueKey1 && valueKey2) {
                    ...
                    if (DEBUG_RUST_API)
                        llvm::outs() << "checkDuplicateAccountKeyEqualMap account1: " << account1
                                     << " account2: " << account2 << "\n";
                }
                ...
                if (account1 != valueName1 || account2 != valueName2) {
                    ...
                    if (valueKey1) {
                        ...
                        if (account2.contains("pda")) accountsPDAMap[account1] = e;
                    }
                    if (valueKey2) {
                        thread->assertKeyEqualMap[pair_real2] = e;
                        if (account1.contains("pda") || account1.contains("canon_")) accountsPDAMap[account2] = e;
                    }
                }
            }
            if (valueName1.contains(".owner") || account1_raw.contains(".owner")) {
            ...
            } else if (valueName2.contains(".owner") || account2_raw.contains(".owner")) {
                // skip user-level .owner
                ...
                if (!thread->isAccountsUnpackedData(valueName2)) {
                    ...
                    thread->assertOwnerEqualMap[pair] = e;
                       ...
                       valueName1.contains("expected_address") || valueName2.contains("expected_address")) {
                auto pair = std::make_pair(valueName1, valueName2);
                ...
                if (!thread->isInAccountsMap(valueName1)) {
                    ...
                    if (!valueName1.empty()) accountNew = true;
                }
                if (!thread->isInAccountsMap(valueName2)) {
                    valueName2 = findCallStackAccountAliasName(func, e, valueName2);
                    // llvm::outs() << "valueName2: " << valueName2 << "\n";
                    if (!valueName2.empty()) accountNew = true;
                }
                if (accountNew) {
            ...
            if (prevInst) {
                if (auto callValue3 = dyn_cast<CallBase>(prevInst)) {
                    ...
                    if (CS3.getTargetFunction()->getName().equals("sol.model.opaqueAssign")) {
                        ...
                        if (prevPrevInst == CS3.getArgOperand(1)) {
                            if (auto callValue4 = dyn_cast<CallBase>(prevPrevInst)) {
                                if (callValue4->getCalledFunction()->getName().contains(".map_err.")) {
                                    CallSite CS4(callValue4);
                                    callValue4 = dyn_cast<CallBase>(CS4.getArgOperand(0));
                                }

                                CallSite CS4(callValue4);
                                if (CS4.getTargetFunction()->getName().contains("::create_program_address.") ||
                                    CS4.getTargetFunction()->getName().contains("::find_program_address.")) {
                                    // auto valueName_x =
                                    ...
                                    std::array<llvm::StringRef, 2> valueNames = {valueName1, valueName2};
                                    for (auto valueName_x : valueNames) {
                                        ...
                                        if (DEBUG_RUST_API) llvm::outs() << "PDA valueName_x1: " << valueName_x << "\n";
                                        accountsPDAMap[valueName_x] = e;

                                        if (!thread->isInAccountsMap(valueName_x)) {
                                            valueName_x = findCallStackAccountAliasName(func, e, valueName_x);
                                            if (DEBUG_RUST_API)
                                                llvm::outs() << "PDA valueName_x2: " << valueName_x << "\n";
                                            accountsPDAMap[valueName_x] = e;
                                        }
                                    }
                                }
            ...
            if (CS2.getNumArgOperands() > 0) value = CS2.getArgOperand(0);
        }
    }
    if (!isAssertKeyAdded) {
        auto valueName = LangModel::findGlobalString(value);
        if (valueName.contains("initialized") || valueName.contains(".discrimi")) thread->hasInitializedCheck = true;
            ...
            if (account.empty() && func->getName().contains(".anon.")) {
                ...
                while (prevInst) {
                    if (auto callValue2 = dyn_cast<CallBase>(prevInst)) {
                        ...
                        if (CS2.getTargetFunction()->getName().equals("sol.model.opaqueAssign")) {
                            ...
                            if (valueName.contains(valueName1)) account = valueName2;
                            if (valueName.contains(valueName2)) account = valueName1;
                            break;
                        }
                    }
                    prevInst = prevInst->getPrevNonDebugInstruction();
                ...
                if (!thread->isInAccountsMap(account)) {
                    ...
                    for (int k = callStack.size() - 1; k >= 0; k--) {
                        funcx = callStack[k];
                        if (!funcx->getName().contains(".anon.")) break;
                    }
                    account = findCallStackAccountAliasName(funcx, e, account);
                }
            }
            if (DEBUG_RUST_API) llvm::outs() << "sol.if valueName: " << valueName << " account: " << account << "\n";
            if (!account.empty()) {
...
void RaceDetectionPass::updateKeyEqualMap(StaticThread *thread, const Event *e, bool isEqual, bool isNotEqual,
                                          llvm::StringRef valueName1, llvm::StringRef valueName2) {
    valueName1 = stripAccountName(valueName1);
    valueName2 = stripAccountName(valueName2);
    if (DEBUG_RUST_API) llvm::outs() << "updateKeyEqualMap constraint: " << valueName1 << " - " << valueName2 << "\n";
    bool valueKey1 = false;
    bool valueKey2 = false;
    if (valueName1.contains(".key")) valueKey1 = true;
    if (valueName2.contains(".key")) valueKey2 = true;
                    ...
                    if (auto callValue = dyn_cast<CallBase>(nextInst)) {
                        ...
                        if (CS.getTargetFunction()->getName().startswith("sol.match.")) {
                            if (CS.getNumArgOperands() > 1) {
                                if (auto funStart0 = llvm::dyn_cast<llvm::CallBase>(CS.getArgOperand(0))) {
                                    if (funStart0->getCalledFunction()->getName().startswith("sol.ifTrue.anon.") ||
                                        funStart0->getCalledFunction()->getName().startswith("sol.ifFalse.anon.")) {
                                        funStart0 = llvm::dyn_cast<llvm::CallBase>(funStart0->getArgOperand(0));

                                        for (auto i = 1; i < CS.getNumArgOperands(); i++) {
                                            if (auto funStart_i = llvm::dyn_cast<llvm::CallBase>(CS.getArgOperand(i))) {
                                                if (funStart_i->getCalledFunction()->getName().startswith(
                                                        "sol.ifTrue.anon.") ||
                                                    funStart_i->getCalledFunction()->getName().startswith(
                                                        "sol.ifFalse.anon.")) {
                                                    funStart_i =
                                                        llvm::dyn_cast<llvm::CallBase>(funStart_i->getArgOperand(0));

                                                    if (i < 2) {
                                                        if (DEBUG_RUST_API)
                                                            llvm::outs() << "sol.match in: " << func->getName() << "\n";
                                                        if (DEBUG_RUST_API)
                                                            llvm::outs()
                                                                << "exclusive func: "
                                                    ...
                                                    exclusiveFunctionsMap[func].insert(funStart_i->getCalledFunction());
                                                }
                                            }
                                        }
                                    }
                            ...
                            auto exclusiveFunctions = exclusiveFunctionsMap[inst1->getFunction()];
                            if (exclusiveFunctions.find(target1->getCalledFunction()) != exclusiveFunctions.end() &&
                                exclusiveFunctions.find(target2->getCalledFunction()) != exclusiveFunctions.end()) {
                                if (DEBUG_RUST_API)
                                    llvm::outs()
                                        << "OKOKOK exclusive functions: " << target1->getCalledFunction()->getName()
                                        << " X " << target2->getCalledFunction()->getName() << "\n";
                                return true;
                            }
                            break;
                        }
...
void RaceDetectionPass::traverseFunction(const aser::ctx *ctx, const Function *func0, StaticThread *thread,
    ...
    if (find(callStack.begin(), callStack.end(), func) == callStack.end()) {
        ...
        if (count % FUNC_COUNT_PROGRESS_THESHOLD == 0) {
            ...
            if (FUNC_COUNT_BUDGET != -1 && count == FUNC_COUNT_BUDGET) {
                // PEIMING: simply return here, as func has not been pushed into callstack yet
                return;
            }
        }
        ...
        if (DEBUG_CALL_STACK) {  //+ " push "
            ...
            if (funcName.find("__cxx_global_") == string::npos && funcName.find("boost::") == string::npos &&
                funcName.find("__gnu_cxx::") == string::npos) {  // skip std:: and boost::
    ...
    // LOG_DEBUG("thread {} enter func : {}", tid, demangle(func->getName().str()));
            ...
            if (isa<ReturnInst>(inst)) {
                if (inst->getNumOperands() > 0 && isa<LoadInst>(inst->getOperand(0))) {
                    ...
                    if (threadIDValueMap.count(handle)) {
                        ...
                        LOG_DEBUG("Thread returned from function. thread={}, func={}, eventID={}", *handle,
                                  func->getName(), e->getID());
                    }
                }
                ...
                if (!CS.isIndirectCall()) {
                    if (LangModel::isRustAPI(CS.getCalledFunction())) {
                        ...
                        if (targetFuncName.startswith("sol.iter.")) continue;

                        if (CS.getTargetFunction()->getName().startswith("sol.match.")) {
                            if (DEBUG_RUST_API) llvm::outs() << "sol.match. funcx: " << func->getName() << "\n";
                            if (func->getName().contains("::process_instruction.") ||
                                ...
                                for (auto i = 0; i < CS.getNumArgOperands(); i++) {
                                    if (auto funStart = llvm::dyn_cast<llvm::CallBase>(CS.getArgOperand(i))) {
                                        if (funStart->getCalledFunction()->getName().startswith("sol.ifTrue.anon.") ||
                                            funStart->getCalledFunction()->getName().startswith("sol.ifFalse.anon."))
                                            funStart = llvm::dyn_cast<llvm::CallBase>(funStart->getArgOperand(0));

                                        if (DEBUG_RUST_API)
                                            llvm::outs() << "sol.match. funStart: " << *funStart << "\n";
                                        // skip sol.Ok.0
                                        if (funStart->getCalledFunction()->getName().startswith("sol.Ok.")) continue;
                                        if (funStart->getCalledFunction()->getName().startswith("sol.if.")) continue;
                                        if (funStart->getCalledFunction()->getName().startswith("sol.Err.")) continue;
                                        bool added = addThreadStartFunction(funStart->getCalledFunction());
                                        if (!added) continue;
                                        if (tid == 0) {
                        ...
                        if (LangModel::isRustModelAPI(CS.getCalledFunction())) {
                            if (CS.getCalledFunction()->getName().startswith("sol.model.opaqueAssign")) {
                                ...
                                if (isa<CallBase>(CS.getArgOperand(1))) {
                                    ...
                                    if (callInst->getCalledFunction()->getName().startswith("sol.clone.1")) {
                                        ...
                                        if (isa<CallBase>(CS2.getArgOperand(0))) {
                                            ...
                                            if (callInst2->getCalledFunction()->getName().contains("_account_info.")) {
                                        ...
                                        if (!account.empty() && account != valueName) {
                                            ...
                                            if (DEBUG_RUST_API)
                                                llvm::outs()
                                                    << "account: " << account << " aliasAccount: " << valueName << "\n";
                                        }
                                        ...
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        if (!valueName.empty()) thread->accountsMap[valueName] = e;

                                    } else if (callInst->getCalledFunction()->getName().contains("_account_info.")) {
                                        // llvm::outs() << "calling: " << callInst->getCalledFunction()->getName() <<
                                        // "\n";
                                        auto aliasAccount =
                                    ...
                                    } else if (callInst->getCalledFunction()->getName().contains("::unpack.")) {
                                        // unpacked data - could be untrustful
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        thread->accountsUnpackedDataMap[valueName] = e;
                                    } else if (callInst->getCalledFunction()->getName().contains("sol.key.")) {
                                        CallSite CS2(callInst);
                                        auto value2 = CS2.getArgOperand(0);
                                        auto valueName2 = LangModel::findGlobalString(value2);
                                        auto account = stripAll(valueName2);
                                    ...
                                    if (valueName.contains("seeds")) {
                                        ...
                                        } else if (valueName1.contains("pool.") && !valueName1.contains("fee_") &&
                                            ...
                                            while (nextInst) {
                                                if (auto callValue2 = dyn_cast<CallBase>(nextInst)) {
                                                    ...
                                                    if (CS2.getTargetFunction()->getName().startswith(
                                                        ...
                                                        // llvm::outs() << "sol_memset valueName: " << valueName <<
                                                        // "\n";
                                                        break;
                                                    }
                                                }
                                ...
                                if (isMacroArrayRefUsedInFunction) {
                                    if (auto prevInst = dyn_cast<CallBase>(CS.getArgOperand(0))) {
                                        ...
                                        if (valueName1.contains("accounts") || valueName1.contains("_ais") ||
                                            ...
                                            auto prevCallName = prevInst->getCalledFunction()->getName();
                                            if (prevCallName.contains("array_ref!")) isMangoAccounts = true;
                                        }
                                        // llvm::outs() << "check.mango: isMangoAccounts: " << isMangoAccounts << "\n";
                                        ...
                                        if (isMangoAccounts) {
                                            ...
                                            for (auto value_ : value_vec) {
                                                ...
                                                // if (false)
                                                if (!value_.empty()) thread->accountsMap[value_] = e;
                                            }
                                        }
                                    }
                                }

                            } else if (CS.getCalledFunction()->getName().startswith("sol.model.macro.array_ref!.")) {
                                isMacroArrayRefUsedInFunction = true;
                            } else if (CS.getCalledFunction()->getName().startswith("sol.model.struct.new.")) {
                                // sol.model.struct.new.Transfer.
                                auto calleeName = CS.getCalledFunction()->getName();
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                auto foundTransferType = (calleeName.find("Transfer") != string::npos);
                                if (foundTransferType) {
                                    ...
                                    if (!to_name.empty()) {
                                        if (DEBUG_RUST_API)
                                            llvm::outs() << "Transfer accountsInvokedMap: from_name: " << from_name
                                                         << " to_name: " << to_name << "\n";
                                ...
                                auto foundMintType = (calleeName.find("MintTo.") != string::npos);
                                ...
                                if (foundMintType || foundBurnType) {
                                    ...
                                    // if (!mint_name.empty()) thread->accountsInvokedMap[mint_name] = e;
                                    if (!to_name.empty()) thread->accountsInvokedMap[to_name] = e;
                                    // if (foundMintType) {
                                    //     llvm::outs() << "tid: " << thread->getTID()
                                    //                  << " MintTo accountsInvokedMap:  to_name: " << to_name << "\n";
                                    // }
                                    if (!from_name.empty()) {
                                        thread->accountsInvokedMap[from_name] = e;

                                        if (!thread->isInAccountsMap(from_name)) {
                                            auto account_from_name = findCallStackAccountAliasName(func, e, from_name);
                                            if (DEBUG_RUST_API)
                                    ...
                                    if (!authority_name.empty()) {
                                        ...
                                        if (foundBurnType) {
                                            if (DEBUG_RUST_API)
                                                llvm::outs() << "Burn accountsInvokeAuthorityMap: authority_name: "
                                ...
                                // CPI Serum
                                auto foundSerumCPIType = (calleeName.find(".dex:") != string::npos);
                            ...
                            } else if (CS.getCalledFunction()->getName().startswith("sol.model.funcArg")) {
                                ...
                                lastThread->anchorStructFunctionFields.push_back(std::make_pair(field, type));
                                // skip UncheckedAccount
                                if (type.contains("UncheckedAccount") && !CONFIG_CHECK_UncheckedAccount) continue;

                                auto e = graph->createForkEvent(ctx, inst, lastThread->getTID());
                                if (type.startswith("Signer")) lastThread->asssertSignerMap[field] = e;
                                ...
                                if (found != string::npos) struct_name = struct_name.substr(0, found);
                            ...
                            } else if (CS.getCalledFunction()->getName().startswith("sol.model.struct.constraint")) {
                                ...
                                auto valueName = LangModel::findGlobalString(value);
                                bool isMutable = (cons_all.find("mut,") != string::npos) || (cons_all == "mut");
                                if (isMutable) {
                                    if (DEBUG_RUST_API) llvm::outs() << "accountsMutMap: " << valueName << "\n";
                                ...
                                auto foundInitIfNeeded = cons_all.find("init_if_needed,") != string::npos;
                                if (foundInit || foundInitIfNeeded) {
                                    ...
                                    if (DEBUG_RUST_API) llvm::outs() << "init: " << valueName << "\n";
                                }
                                ...
                                if (foundSeeds != string::npos) {
                                    ...
                                    if (found != string::npos) {
                                        ...
                                        for (auto cons : cons_vec) {
                                            if (DEBUG_RUST_API) llvm::outs() << "seeds: " << cons << "\n";
                                            // cdp_vault_owner.key()
                                            // currency_mint.to_account_info().key.as_ref()
                                            if (isFreshPDA) accountsPDASeedsMap[account_pda].push_back(cons);
                                            ...
                                            if (isConstantOrSigner && !cons.startswith("b\"") && !isUpper(cons.str()))
                                ...
                                for (auto cons : cons_vec) {
                                    if (DEBUG_RUST_API) llvm::outs() << "cons: " << cons << "\n";
                                    ...
                                    if (foundCons != string::npos) {
                                        ...
                                        for (auto cons_ : cons_vec_full) {
                                            ...
                                            if (foundErrorAt != string::npos) cons_ = cons_.substr(0, foundErrorAt);
                                            auto equalStr = "==";
                                            auto notEqualStr = "!=";
                                            auto lessOrEqualStr = "<=";
                                            ...
                                            if (found != string::npos) {
                                                size_t op_len = 2;
                                                if (isLessThan || isLargerThan) op_len = 1;
                                    ...
                                    if (foundHasOne != string::npos) {
                                        ...
                                        // has_one = staked_mint @StakingCampaignError::InvalidStakedMint
                                        ...
                                        // // these are stack variables and will be deallocated when the function
                                        // returns.. auto account_key = account.str() + ".key"; auto
                                        // cons_account_key = value_str + "." + account_key;
                                        // // UNDERSTAND WHY?
                                        // auto pair = std::make_pair(cons_account_key, account_key);
                                        // lastThread->assertKeyEqualMap[pair] = e;
                                        // auto pair2 = std::make_pair(account, account_key);
                                        // lastThread->assertKeyEqualMap[pair2] = e;
                                        // llvm::outs()
                                        //     << "T" << lastThread->getTID() << " assertKeyEqualMap: " <<
                                        //     cons_account_key
                                        //     << " == " << account_key << "\n";
                                    ...
                                    if (foundClose || foundZero) {
                                        ...
                                        if (DEBUG_RUST_API) {
                                            if (foundZero) llvm::outs() << "zero: " << account << "\n";
                                            if (foundClose) llvm::outs() << "close: " << account << "\n";
                                        }
                                        // get the lastest forked thread
                                        auto lastThread = threadList.back();
                                        if (foundZero) lastThread->asssertDiscriminatorMap[account] = e;
                                        if (foundClose) lastThread->accountsAnchorCloseMap[account] = e;
                                    }
                                    ...
                                    if (foundSigner != string::npos && foundSigner_ == string::npos &&
                                        foundSignerDot == string::npos && foundSignerPayer == string::npos) {
                                        // find the next instruction, which has e.g.,
                                        //>12|    #[account(signer)]
                                        // 13|    pub owner: AccountInfo<'info>
                                        llvm::outs() << "signer cons: " << cons << " account: " << account << "\n";
                                        if (DEBUG_RUST_API) llvm::outs() << "signer: " << account << "\n";
                                    ...
                                    if (foundBump != string::npos) {
                                        if (!foundInit) {
                                            ...
                                            if (!bumpSeed.contains(".")) {
                                                // llvm::outs() << "LIKELY BUMP SEED VULNERABILITY: " << cons <<
                                                // "\n";
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "sol.model.loop: " << *inst << "\n";
                                if (CS.getNumArgOperands() > 0) {
                                    auto value = CS.getArgOperand(0);
                                    auto valueName = LangModel::findGlobalString(value);
                                    auto funcName = valueName;
                                    // auto funcName = valueName.substr(0, valueName.find_last_of("."));
                                    // llvm::outs() << "func_loop: " << funcName << "\n";
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "sol.model.break: " << *inst << "\n";
                                if (isInLoop()) {
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "sol.model.macro: " << *inst << "\n";
                                // sol.model.macro.placeholder.
                                if (CS.getCalledFunction()->getName().startswith("sol.model.macro.placeholder.")) {
                                    ...
                                    if (func_handler) {
                                        ...
                                        if (DEBUG_RUST_API) llvm::outs() << "func_handler: NULL\n";
                                    }
                                } else {
                                    auto value = CS.getArgOperand(0);
                                    ...
                                    if (CS.getCalledFunction()->getName().startswith(
                                    ...
                                    } else {
                                        {
                                            ...
                                            while (found != string::npos) {
                                                ...
                                                if (!thread->isInAccountsMap(account)) {
                                                    ...
                                                    if (!account.empty()) thread->asssertSignerMap[account] = e;
                                                }
                                        ...
                                        }
                                        {
                                            ...
                                            while (found != string::npos) {
                                                ...
                                                if (DEBUG_RUST_API) llvm::outs() << "macro owner: " << account << "\n";
                                                if (!account.contains("=")) {
                                        ...
                                        }
                                        {
                                            ...
                                            while (found != string::npos) {
                                                ...
                                                if (DEBUG_RUST_API) llvm::outs() << "macro key: " << account << "\n";
                                                auto pair = std::make_pair(account, account2);
                                                thread->assertKeyEqualMap[pair] = e;
                            ...
                            } else if (CS.getCalledFunction()->getName().equals("sol.model.access_control")) {
                                // find
                                auto value = CS.getArgOperand(0);
                                auto calleeName = LangModel::findGlobalString(value);
                                auto found = calleeName.find("(");
                                if (found != string::npos) calleeName = calleeName.substr(0, found);
                                if (calleeName.startswith("ctx.accounts.")) {
                                    auto foundDot = calleeName.find_last_of(".");
                                    if (foundDot != string::npos) calleeName = calleeName.substr(foundDot + 1);
                                }
                                ...
                                if (func_struct) {
                                    ...
                                    if (DEBUG_RUST_API)
                                        llvm::outs()
                                            << "access_control threadStartFuncName: " << threadStartFuncName << "\n";
                                    accessControlMap[threadStartFuncName] = e;
                                    traverseFunctionWrapper(ctx, thread, callStack, inst, func_struct);
                                } else {
                                    if (DEBUG_RUST_API) llvm::outs() << "func_struct: NULL\n";
                                }
                            } else {
                                traverseFunctionWrapper(ctx, thread, callStack, inst, CS.getCalledFunction());
                            }
                            ...
                            if (func->getName().startswith("sol.model.anchor.program.")) {
                                ...
                                if (newtid > 0) {
                                    ...
                                    for (int i = 0; i < CS.getNumArgOperands(); i++) {
                                        ...
                                        if (found != string::npos) {
                                            ...
                                            if (DEBUG_RUST_API)
                                                llvm::outs()
                                                    << "New funcArg name: " << name << " type: " << type << "\n";
                                            auto pair = std::make_pair(name, type);
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "anchor struct_name: " << struct_name << "\n";
                                std::string func_struct_name = "sol.model.struct.anchor." + struct_name.str();
                                // checking issues in @sol.model.struct.anchor.LogMessage
                                auto func_struct = thisModule->getFunction(func_struct_name);
                                if (func_struct) {
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.sol_memset.3")) {
                                auto value = CS.getArgOperand(0);
                                ...
                                if (auto callValue = dyn_cast<CallBase>(value)) {
                                    ...
                                    if (valueName.find(".data") != string::npos) {
                                        ...
                                        if (found != string::npos) account = valueName.substr(0, found);
                                        if (!thread->isInAccountsMap(account))
                                            account = findCallStackAccountAliasName(func, e, account);
                            ...
                            } else if (CS.getTargetFunction()->getName().equals("sol.position.2")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.position: " << *inst << "\n";
                                ...
                                if (!valueName.empty()) thread->updateMostRecentFuncReturn(funcx, valueName);
                            } else if (CS.getTargetFunction()->getName().contains("::keccak::hashv.")) {
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                if (valueName.contains(",")) {
                                    auto e = graph->createReadEvent(ctx, inst, tid);
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "sol.if: " << *inst << "\n";
                                ...
                                if (auto callValue = dyn_cast<CallBase>(value)) {
                                    ...
                                    if (CS2.getTargetFunction()->getName().equals("sol.&&") ||
                                        CS2.getTargetFunction()->getName().equals("sol.||")) {
                                        handleConditionalCheck0(ctx, tid, func, inst, thread, CS2.getArgOperand(0));
                                        handleConditionalCheck0(ctx, tid, func, inst, thread, CS2.getArgOperand(1));
                                        handled = true;
                                    }
                                }
                                if (!handled) handleConditionalCheck0(ctx, tid, func, inst, thread, value);
                            } else if (CS.getTargetFunction()->getName().startswith("sol.&&")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.&&: " << *inst << "\n";
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                for (int i = 0; i <= 1; i++) {
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.get_account_data.2")) {
                                auto e = graph->createReadEvent(ctx, inst, tid);
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.next_account_info.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "next_account_info: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                ...
                                if (auto callValue = dyn_cast<CallBase>(nextInst)) {
                                    CallSite CS2(callValue);
                                    if (CS2.getTargetFunction()->getName().startswith("sol.model.opaqueAssign")) {
                                        ...
                                        if (found != string::npos) {
                                            ...
                                            if (DEBUG_RUST_API) llvm::outs() << "pda_address: " << pda_address << "\n";
                                            if (pda_address != "_") thread->asssertProgramAddressMap[pda_address] = e;
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.realloc.3")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.realloc: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "spl_token::instruction:: " << *inst << "\n";
                                ...
                                if (!valueName.contains("spl_token::")) {
                                    ...
                                    if (found != string::npos) accountName = valueName.substr(0, found);
                                    ...
                                    if (!thread->isAccountKeyValidated(accountName)) {
                                        ...
                                        if (!valueName.empty() && !valueName.contains("spl_token::") &&
                                            ...
                                            // llvm::errs()<< "==============VULNERABLE: arbitrary spl_token
                                            // account!============\n";
                                            // turn this off for newer versions spl 3.1.1
                                            if (splVersionTooOld)
                                                UntrustfulAccount::collect(accountName, e, callEventTraces,
                                ...
                                if (CS.getNumArgOperands() > 3) {
                                    ...
                                    for (int i = 1; i < 4; i++) {
                                        ...
                                        // strip .key
                                        if (foundDot != string::npos) accountName_i = accountName_i.substr(0, foundDot);
                                        ...
                                        if (foundAnchor != string::npos) {
                                            // accountName_i = accountName_i.substr(0, accountName_i.find_last_of("."));
                                            // strip ctx.accounts.
                                        ...
                                        if (DEBUG_RUST_API)
                                            llvm::outs()
                                                << "accountsInvokedMap accountName_i: " << accountName_i << "\n";
                                    }
                                    // now check if source_token_account.key == destination_token_account.key
                                    if (CS.getTargetFunction()->getName().startswith(
                                        ...
                                        thread->tokenTransferFromToMap[pair] = e;
                                        // UntrustfulAccount::collect(e, callEventTraces, SVE::Type::ACCOUNT_DUPLICATE,
                                        // 9);
                                    }
                                }
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.Rent::from_account_info.") ||
                                       CS.getTargetFunction()->getName().startswith("sol.Clock::from_account_info.")) {
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                if (auto arg = dyn_cast<Argument>(value))
                                    valueName = funcArgTypesMap[func][arg->getArgNo()].first;
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("sysvar::instructions::")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("program::invoke")) {
                                auto value = CS.getArgOperand(0);
                                if (auto callValue = dyn_cast<CallBase>(value)) {
                                    CallSite CS2(callValue);
                                    if (CS2.getTargetFunction()->getName().startswith(
                                        ...
                                        if (accounts_vec.size() > 3) {
                                            ...
                                            if (DEBUG_RUST_API)
                                                llvm::outs()
                                                    << "sol.create_associated_token_account: " << associate_account
                                                    << "\n";
                                            auto e = graph->createReadEvent(ctx, inst, tid);
                            ...
                            } else if (CS.getTargetFunction()->getName().equals("sol.stake::instruction::split.4")) {
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                for (int i = 1; i <= 1; i++) {
                                    auto value = CS.getArgOperand(i);
                                    auto valueName = LangModel::findGlobalString(value);
                            ...
                            } else if (CS.getTargetFunction()->getName().equals("sol.stake::instruction::withdraw.5")) {
                                auto e = graph->createReadEvent(ctx, inst, tid);
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("system_instruction::transfer")) {
                                // TODO track parameter passing
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                auto found = valueName.find_last_of(".");
                                if (found != string::npos) valueName = valueName.substr(0, found);
                                // if this is parameter
                                if (valueName.empty()) {
                                    if (auto arg = dyn_cast<Argument>(value)) {
                                        valueName = funcArgTypesMap[func][arg->getArgNo()].first;
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("//sol.transfer_ctx")) {
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                if (DEBUG_RUST_API) llvm::outs() << "TODO sol.transfer_ctx: " << valueName << "\n";
                                //
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                thread->accountsInvokedMap["authority"] = e;

                            } else if (CS.getTargetFunction()->getName().startswith("sol.CpiContext::new.")) {
                                auto value = CS.getArgOperand(1);
                                auto valueName = LangModel::findGlobalString(value);
                                auto found = valueName.find("authority:");
                                // authority:self.authority.to_account_info()
                                if (found != string::npos) valueName = valueName.substr(found);
                                ...
                                if (DEBUG_RUST_API) llvm::outs() << "sol.CpiContext::new.: " << valueName << "\n";
                                //
                                auto e = graph->createReadEvent(ctx, inst, tid);
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.borrow_mut.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "borrow_mut: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                // llvm::outs() << "data: " << valueName << "\n";
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                ...
                                if (valueName.find(".data") != string::npos) {
                                    ...
                                    // llvm::outs()
                                    //     << "borrow_mut account: " << account << " valueName: " << valueName << "\n";
                                ...
                                if (!account_.empty()) account = account_;
                                if (!account.empty())
                                    if (!thread->isInAccountOrStructAccountMap(account))
                                        thread->accountsMap[account] = e;

                            } else if (CS.getTargetFunction()->getName().startswith("sol.serialize.")) {
                                // create an object of a class
                                if (DEBUG_RUST_API) llvm::outs() << "sol.serialize: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                auto account = valueName;
                                thread->borrowDataMutMap[account] = e;
                            } else if (CS.getTargetFunction()->getName().startswith("sol.try_borrow_mut_data.")) {
                                // create an object of a class
                                if (DEBUG_RUST_API) llvm::outs() << "sol.try_borrow_mut_data: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                auto account = valueName;
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.borrow.") ||
                                       CS.getTargetFunction()->getName().startswith("sol.try_borrow_data.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.borrow: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                // ctx.accounts.candy_machine
                                ...
                                if (valueName.find(".data") != string::npos) {
                                    bool isBorrow = true;
                                    if (valueName.find("borrow_mut") != string::npos) isBorrow = false;
                                    if (isBorrow)
                                    ...
                                    if (found1 != string::npos) {
                                        ...
                                        if (isBorrow)
                                            ...
                                            thread->borrowDataMutMap[account1] = e;
                                        if (DEBUG_RUST_API) llvm::outs() << "account1: " << account1 << "\n";
                                    }
                                ...
                                if (!account.empty()) {
                                    if (!thread->isInAccountOrStructAccountMap(account)) {
                                        ...
                                        // let fee_collector_account_b = ctx.remaining_accounts.get(1).unwrap();
                                                            ...
                                                            if (CS3.getTargetFunction()->getName().startswith(
                                                                ...
                                                                if (auto callValue4 = dyn_cast<CallBase>(value3)) {
                                                                    ...
                                                                    if (CS4.getTargetFunction()->getName().startswith(
                                                                        ...
                                                                        auto accountSig = valueName41.str() + ".get(" +
                                                                                          valueName42.str();
                                                                ...
                                                                // instruction_sysvar_account.to_account_info(); let
                                                                // instruction_sysvar =
                                        ...
                                        if (!remainingAccountChecked) thread->accountsMap[account] = e;
                                    }
                                }

                            } else if (CS.getTargetFunction()->getName().startswith("sol.require.!2")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.require.!: " << *inst << "\n";
                                ...
                                if (valueName.contains("::verify(")) {
                                    ...
                                    auto funcName =
                                        valueName.substr(0, found).str() + "." + std::to_string(value_vec.size());
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.assert.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "assert: " << *inst << "\n";
                                auto value = CS.getArgOperand(0);
                                auto valueName = LangModel::findGlobalString(value);
                                // llvm::outs() << "data: " << valueName << "\n";
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.assert_eq.") ||
                                       CS.getTargetFunction()->getName().startswith("sol.ne.2") ||
                                       CS.getTargetFunction()->getName().startswith("sol.eq.2") ||
                                       CS.getTargetFunction()->getName().contains("_keys_eq.") ||
                                       CS.getTargetFunction()->getName().contains("_keys_neq.") ||
                                       CS.getTargetFunction()->getName().contains("_keys_equal.") ||
                                       CS.getTargetFunction()->getName().contains("_keys_not_equal.")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.assert_owner.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "assert_owner: " << *inst << "\n";
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.assert_signer.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "assert_signer: " << *inst << "\n";
                                if (CS.getNumArgOperands() >= 1) {
                                    auto value1 = CS.getArgOperand(0);
                                    auto valueName1 = LangModel::findGlobalString(value1);
                                    auto account = stripAll(valueName1);
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.get.2")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.get.2: " << *inst << "\n";
                                auto value1 = CS.getArgOperand(0);
                                auto valueName1 = LangModel::findGlobalString(value1);
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.validate.!2")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.reload.1")) {
                                auto value1 = CS.getArgOperand(0);
                                auto valueName1 = LangModel::findGlobalString(value1);
                                ...
                                if (CS.getNumArgOperands() > 1) {
                                    ...
                                    if (valueName1.startswith("ctx.") && valueName1.endswith(".amount")) {
                                       ...
                                       CS.getTargetFunction()->getName().startswith("sol.<=")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.>=: " << *inst << "\n";
                                auto value1 = CS.getArgOperand(0);
                                auto value2 = CS.getArgOperand(1);
                                auto valueName1 = LangModel::findGlobalString(value1);
                            ...
                            } else if (CS.getTargetFunction()->getName().equals("sol.contains.2")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.contains.2: " << *inst << "\n";
                                if (func->getName().contains("::verify")) {
                                    ...
                                    if (DEBUG_RUST_API)
                                        llvm::outs()
                                            << "sol.contains.2 verified: " << valueName2 << " " << valueName1 << "\n";
                                ...
                                if (auto call_inst = dyn_cast<CallBase>(value)) {
                                    ...
                                    if (CS2.getTargetFunction()->getName().startswith("sol.borrow_mut.")) {
                                        if (DEBUG_RUST_API) llvm::outs() << "borrow_mut: " << *value << "\n";
                                        auto value2 = CS2.getArgOperand(0);

                                        auto valueName = LangModel::findGlobalString(value2);
                                        if (DEBUG_RUST_API) llvm::outs() << "data: " << valueName << "\n";
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                ...
                                if (isa<Argument>(value) || isa<Argument>(value2) || isArgument ||
                                    dyn_cast<CallBase>(value2)) {
                                    if (!isSafeType(func, value) && !isSafeType(func, value2)) {
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_ADD, 8);
                                    }
                                } else {
                                    if (valueName2.equals("1") && !isInLoop()) {
                                        if (!isSafeVariable(func, value) && !hasValueLessMoreThan(value, inst, true)) {
                                            auto e = graph->createReadEvent(ctx, inst, tid);
                                            UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_ADD, 8);
                                        }
                                    }
                                ...
                                if (isa<Argument>(value) || isa<Argument>(value2)) {
                                    if (!isSafeType(func, value) && !isSafeType(func, value2)) {
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_SUB, 8);
                                    }
                                } else {
                                    auto valueName2 = LangModel::findGlobalString(value2);
                                    if (valueName2.equals("1") && !isInLoop()) {
                                        if (!isSafeVariable(func, value) && !hasValueLessMoreThan(value, inst, false)) {
                                            auto e = graph->createReadEvent(ctx, inst, tid);
                                            UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_SUB, 8);
                                        }
                                    ...
                                    if (valueName.contains("rent")) {
                                        if (auto lamport_inst = dyn_cast<CallBase>(value1)) {
                                            ...
                                            if (CS2.getTargetFunction()->getName().startswith("sol.lamports.")) {
                                                auto e = graph->createReadEvent(ctx, inst, tid);
                                                UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_SUB,
                                                                         8);
                                            }
                                        }
                                    }
                                ...
                                if (isa<Argument>(value1) || isa<Argument>(value2)) {
                                    if (!isSafeType(func, value1) && !isSafeType(func, value2)) {
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_MUL, 6);
                                    }
                                }
                            } else if (CS.getTargetFunction()->getName().equals("sol./")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol./: " << *inst << "\n";
                                ...
                                if (isa<Argument>(value2)) {
                                    if (!isSafeType(func, value2)) {
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::OVERFLOW_DIV, 5);
                                    }
                                }
                                if (auto value = dyn_cast<CallBase>(CS.getArgOperand(0))) {
                                    ...
                                    if (CS2.getTargetFunction()->getName().equals("sol.*")) {
                                        ...
                                        if (valueName2.contains("total") && valueName2.contains("upply")) {
                                            ...
                                            SourceInfo srcInfo = getSourceLoc(inst);
                                            if (srcInfo.getSourceLine().find(" as u") == string::npos) {
                                                auto e = graph->createReadEvent(ctx, inst, tid);
                                                UnSafeOperation::collect(e, callEventTraces,
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.checked_div.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.checked_div: " << *inst << "\n";
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("//sol.checked_mul.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.checked_mul: " << *inst << "\n";
                                auto signature = "price_for_supply";
                                if (func->getName().contains(signature)) {
                                    for (auto idx = callEventTraces[tid].size() - 1; idx >= 0; idx--) {
                                        auto e = callEventTraces[tid][idx];
                                        if (auto call_inst = dyn_cast<CallBase>(e->getInst())) {
                                            CallSite CS2(call_inst);
                                            if (CS2.getTargetFunction()->getName().contains(signature) &&
                                                !CS2.getTargetFunction()->getName().contains(".anon")) {
                                                UnSafeOperation::collect(e, callEventTraces, SVE::Type::FLASHLOAN_RISK,
                                                                         9);
                                                break;
                                            }
                                        }
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.into.")) {
                                // sol.into.1
                            } else if (CS.getTargetFunction()->getName().contains("::from.1") ||
                                       CS.getTargetFunction()->getName().contains("::try_from.1")) {
                                //
                            } else if (CS.getTargetFunction()->getName().startswith("sol.to_u32.") ||
                                       CS.getTargetFunction()->getName().startswith("sol.to_u64.")) {
                                if (auto value = dyn_cast<CallBase>(CS.getArgOperand(0))) {
                                    CallSite CS2(value);
                                    ...
                                    if (targetName == "sol.*") {
                                        ...
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::CAST_TRUNCATE, 8);
                                    }
                                }
                            } else if (CS.getTargetFunction()->getName().equals("sol.typecast")) {
                                    ...
                                    for (auto pair : funcArgTypesMap[func]) {
                                        if (pair.first.contains(valueName1)) {
                                            ...
                                            if ((type1.startswith("u") || type1.startswith("i")) &&
                                                ...
                                                if (type1 == "usize") type1 = "u64";
                                                if (type2 == "usize") type2 = "u64";
                                                if (DEBUG_RUST_API)
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.to_imprecise.")) {
                                bool isPossibleRoundingError = true;
                                if (auto value = dyn_cast<CallBase>(CS.getArgOperand(0))) {
                                    CallSite CS2(value);
                                    auto targetName = CS2.getTargetFunction()->getName();
                                    // sol.floor. OR sol.ceil.
                                    if (DEBUG_RUST_API) llvm::outs() << "sol.to_imprecise from: " << targetName << "\n";
                                    isPossibleRoundingError = false;
                                }
                                if (isPossibleRoundingError && !thread->isOnceOnlyOwnerOnlyInstruction) {
                                    auto e = graph->createReadEvent(ctx, inst, tid);
                                    UnSafeOperation::collect(e, callEventTraces, SVE::Type::BIDIRECTIONAL_ROUNDING, 8);
                                }
                            } else if (CS.getTargetFunction()->getName().startswith("sol.try_round_")) {
                                // sol.try_round_u64.1
                                bool isPossibleRoundingError = true;
                                if (auto nextInst = inst->getNextNonDebugInstruction()) {
                                    if (auto nextInst_call = dyn_cast<CallBase>(nextInst)) {
                                        ...
                                        auto targetName = CS2.getTargetFunction()->getName();
                                        if (targetName.startswith("sol.max.") || targetName.startswith("sol.min."))
                                            isPossibleRoundingError = false;
                                        if (DEBUG_RUST_API)
                                            llvm::outs() << "sol.try_round_ from: " << targetName << "\n";
                                    }
                                }
                                if (isPossibleRoundingError && !thread->isOnceOnlyOwnerOnlyInstruction) {
                                    auto e = graph->createReadEvent(ctx, inst, tid);
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("::check_account_owner.2")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("::validate_owner.")) {
                                if (DEBUG_RUST_API) llvm::outs() << "validate_owner: " << *inst << "\n";
                                if (CS.getNumArgOperands() > 2) {
                                    auto value1 = CS.getArgOperand(0);  // program_id
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.require_eq.!")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("cmp_pubkeys.2")) {
                                auto value1 = CS.getArgOperand(0);  // mint_info.key
                                auto value2 = CS.getArgOperand(1);  // dest_account.base.mint
                                auto valueName1 = LangModel::findGlobalString(value1);
                                if (auto arg1 = dyn_cast<Argument>(value1))
                            ...
                            } else if (CS.getTargetFunction()->getName().startswith("sol.invoke_signed.") ||
                                       CS.getTargetFunction()->getName().startswith("sol.invoke.")) {
                                auto value = CS.getArgOperand(0);
                                // find all accounts invoked
                                if (!isa<CallBase>(value)) value = inst->getPrevNonDebugInstruction();
                                auto e = graph->createReadEvent(ctx, inst, tid);
                                ...
                                if (auto call_inst = dyn_cast<CallBase>(value)) {
                                    ...
                                    if (auto call_inst2 = dyn_cast<CallBase>(value2)) {
                                        ...
                                        if (CS3.getTargetFunction()->getName().contains("instruction::")) {
                                            ...
                                            for (auto value_ : value_vec) {
                                                ...
                                                if (foundClone != string::npos) value_ = value_.substr(0, foundClone);
                                                if (DEBUG_RUST_API)
                                    ...
                                    if (DEBUG_RUST_API)
                                        llvm::outs()
                                            << "sol.invoke: failed to find invoked accounts: " << *value << "\n";
                                }

                                // find program_id for sol.invoke_signed.
                                if (CS.getTargetFunction()->getName().startswith("sol.invoke_signed.")) {
                            ...
                            } else if (CS.getTargetFunction()->getName().contains("//initialized")) {
                                if (DEBUG_RUST_API) llvm::outs() << "sol.initialized: " << *inst << "\n";
                                thread->hasInitializedCheck = true;
                                ...
                                if (CS.getTargetFunction()->getName().contains(signature)) {
                                    auto value = CS.getArgOperand(0);
                                    auto valueName = LangModel::findGlobalString(value);
                                    if (valueName.contains(".accounts.") && valueName.contains("price")) {
                                        auto e = graph->createReadEvent(ctx, inst, tid);
                                        UnSafeOperation::collect(e, callEventTraces, SVE::Type::FLASHLOAN_RISK, 9);
                                    }
                                }
                                ...
                                if (CS.getTargetFunction()->getName().startswith("sol.check_") ||
                                    CS.getTargetFunction()->getName().startswith("sol.validate_")) {
                                    if (DEBUG_RUST_API) llvm::outs() << "sol.check_: " << *inst << "\n";
    ...
    // LOG_DEBUG("thread {} exit func : {}", tid, demangle(func->getName().str()));
    callStack.pop_back();
    ...
    if (DEBUG_CALL_STACK) {
        ...
        if (funcName.find("__cxx_global_") == string::npos && funcName.find("boost::") == string::npos &&
            ...
            call_stack_level--;
            llvm::outs() << DEBUG_STRING_SPACE + "thread" << tid << " pop " + funcName + " level: " << call_stack_level
                         << "\n";
        }
    }
}

void RaceDetectionPass::traverseFunctionWrapper(const aser::ctx *ctx, StaticThread *thread,
    ...
    if (bb.hasName() && bb.getName() == "sol.call" && bb.size() > 2 && call_size > 0) {
        int idx = call_size - 1;
        ...
        if (funcArgTypesMap.find(parentFunc) != funcArgTypesMap.end()) {
            ...
            if (pair.second.startswith("Context<")) {
                ...
                if (DEBUG_RUST_API) llvm::outs() << "ctxStructName: " << ctxStructName << "\n";
                for (auto &ii : bb) {
                    if (isa<CallBase>(&ii)) {
                        CallSite CS(&ii);
                        ...
                        if (!ctxStructName.empty() && fname.contains(ctxStructName)) {
                            f = targetF;
                            if (DEBUG_RUST_API) llvm::outs() << "  target: " << fname << "\n";
                            hasFoundTarget = true;
                            break;
                        }
                    }
        ...
        if (!hasFoundTarget) {
            ...
            if (DEBUG_RUST_API) llvm::outs() << "parentFuncName: " << parentFuncName << "\n";
            for (auto &ii : bb) {
                if (isa<CallBase>(&ii)) {
                    CallSite CS(&ii);
                    auto targetF = CS.getTargetFunction();
                    ...
                    if (found != string::npos) {
                        ...
                        if (DEBUG_RUST_API) llvm::outs() << "  packageName: " << packageName << "\n";
    ...
    // if (!directNode->getTargetFun()->isExtFunction())
    {
        ...
        {
            callEvent = graph->createCallEvent(ctx, inst, f->getName(), thread->getTID());
        ...
        if (forkevent) {
            // LOG_DEBUG("call inst: {}  - fork event id:  {}", *inst, forkevent->getID());
...
void RaceDetectionPass::initStructFunctions() {
    ...
    for (auto &function : functionList) {
        auto func = &function;
        if (!func->isDeclaration() && func->getName().startswith("sol.model.struct.")) {
            ...
            if (DEBUG_RUST_API) llvm::outs() << "func_struct_name: " << func_struct_name << "\n";
            std::vector<std::pair<llvm::StringRef, llvm::StringRef>> fieldTypes;
            for (auto &BB : *func) {
                for (auto &I : BB) {
                    if (isa<CallBase>(&I)) {
                        aser::CallSite CS(&I);
                        if (CS.getCalledFunction()->getName().startswith("sol.model.struct.field")) {
                            auto value1 = CS.getArgOperand(0);
                            auto field = LangModel::findGlobalString(value1);
                            auto value2 = CS.getArgOperand(1);
                            auto type = LangModel::findGlobalString(value2);
                            if (DEBUG_RUST_API) llvm::outs() << "field account: " << field << " type: " << type << "\n";
...
void RaceDetectionPass::detectUntrustfulAccounts(TID tid) {
    ...
    if (DEBUG_RUST_API) {
        ...
        for (auto [pair, e] : curThread->assertKeyEqualMap) {
            llvm::outs() << "assertKeyEqualMap: " << pair.first << " == " << pair.second << "\n";
        }
        ...
        for (auto [pair, e] : curThread->assertOwnerEqualMap) {
            llvm::outs() << "assertOwnerEqualMap: " << pair.first << " == " << pair.second << "\n";
        }
        for (auto [pair, e] : curThread->assertOtherEqualMap) {
            llvm::outs() << "assertOtherEqualMap: " << pair.first << " == " << pair.second << "\n";
        }
        for (auto [account, e] : curThread->accountsInvokedMap) {
            llvm::outs() << "accountsInvokedMap: " << account << "\n";
        }
        for (auto [pair, e] : assertKeyNotEqualMap) {
            llvm::outs() << "assertKeyNotEqualMap: " << pair.first << " != " << pair.second << "\n";
        }
    }
    // check unstake access control
    if (funcName.contains("unstake") && isAccessControlInstruction("stake") && !isAccessControlInstruction(funcName)) {
        auto e = callEventTraces[tid].front();
        UntrustfulAccount::collect(funcName, e, callEventTraces, SVE::Type::MISS_ACCESS_CONTROL_UNSTAKE, 9);
    }
    ...
    bool isPotentiallyOwnerOnly = funcName.contains("create_") || funcName.contains("new_");
    ...
    if (isInit) {
        ...
        for (auto ArgNo = 0; ArgNo < size; ArgNo++) {
            ...
            // llvm::outs() << "ArgNo: " << ArgNo << " name: " << pair.first << " type: " << pair.second << "\n";
            if (pair.second.contains("Pubkey")) potentialOwnerAccounts.insert(pair.first);
            for (auto [accountName, e] : curThread->accountsMap) {
                if (curThread->isAccountPDAInInstruction(accountName) ||
                    ...
                    if (DEBUG_RUST_API)
                        llvm::errs() << "==============ADDING globalStateAccounts: " << accountName << "============\n";
                }
            }
        }
    }
    bool isOwnerOnly = curThread->isOnceOnlyOwnerOnlyInstruction || curThread->isAccountSignerMultiSigPDA() ||
                       curThread->isAccountSignerVerifiedByPDAContains() ||
                       curThread->isPotentiallyOwnerOnlyInstruction(potentialOwnerAccounts, isInit);
    // isOwnerOnly = false;
    ...
    for (auto [accountName, e] : curThread->accountsMap) {
        ...
        if (isInit) {
            ...
            if (!curThread->hasInitializedCheck)
                if (curThread->isAccountBorrowDataMut(accountName) ||
                    ...
                    if (!curThread->isAccountDiscriminator(accountName) &&
                        ...
                        // if account key validated and function contains find_program_address
                        ...
                        if (!isPDAValidated) {
                            UntrustfulAccount::collect(accountName, e2, callEventTraces, SVE::Type::REINIT, 5);
                        }
                    }
                }
        } else {
            ...
            bool isUnvalidate = false;  // curThread->isAccountBorrowDataMut(accountName) ||
            // missing owner check
            // ctx.accounts.authority.key != &token.owner
            if (!isUnvalidate &&
                (curThread->isAccountBorrowData(accountName) && !curThread->isAccountBorrowDataMut(accountName)) &&
                !curThread->isAccountBorrowLamportsMut(accountName) && !isAnchorDataAccount(accountName) &&
                !isAccountPDA(accountName) && !curThread->isAccountKeyValidated(accountName) &&
                !curThread->isAccountInvoked(accountName) && !curThread->isAccountOwnerValidated(accountName) &&
                !curThread->isAccountUsedInSeed(accountName)) {
                // llvm::errs() << "==============VULNERABLE: MissingOwnerCheck!============\n";
                UntrustfulAccount::collect(accountName, e, callEventTraces, SVE::Type::MISS_OWNER, 10);
                isUnvalidate = true;
            }
            ...
            if (curThread->isTokenTransferDestination(accountName)) {
                if (!curThread->isAccountKeyValidated(accountName) &&
                    ...
                    auto accountNameSrc = curThread->getTokenTransferSourceAccount(accountName);
                    if (!accountNameSrc.empty() && !isAccountKeyNotEqual(accountNameSrc, accountName) &&
            ...
            if (accountName.contains("_ata") || accountName.contains("_associated_token_account")) {
                if (isAnchorTokenAccount(accountName) && !curThread->isAccountKeyValidated(accountName) &&
            ...
            if (LangModel::isAuthorityAccount(accountName)) {
                ...
                if (!isUnvalidate && !curThread->isAccountSigner(accountName) &&
                    !curThread->isAccountValidatedBySignerAccountInConstraint(accountName) &&
                    !isAccountPDA(accountName) && !curThread->isAccountInvokedAsAuthority(accountName) &&
                    !curThread->isAccountInvoked(accountName) && !curThread->isAccountBorrowDataMut(accountName)) {
                    if (!curThread->isAccountReferencedByAnyOtherAccountInHasOne(accountName) &&
                        !curThread->isAccountReferencedByAnyOtherAccountInConstraint(accountName) &&
                        !curThread->isAccountKeyValidatedSafe(accountName) && !isAnchorDataAccount(accountName)) {
                        bool isValidatedBySigner = false;
                        if (accountName.endswith("_info")) {
                            auto accountName_x = accountName.substr(0, accountName.find("_info"));
                            if (curThread->isAccountValidatedBySignerAccountInConstraint(accountName_x)) {
                                isValidatedBySigner = true;
                            }
                        }
                        // heuristic: skip checking signer on "manager_fee"
                        if (!isOwnerOnly && !isValidatedBySigner && !accountName.contains("pda") &&
                            !accountName.contains("mint") && !accountName.contains("new_") &&
                            !accountName.contains("from_") && !accountName.contains("to_") &&
                            !accountName.contains("_fee")) {
                            UntrustfulAccount::collect(accountName, e, callEventTraces, SVE::Type::MISS_SIGNER, 10);
                            isUnvalidate = true;
                        }
                ...
                if (!isUnvalidate && curThread->isAccountInvokedAsBurnAuthority(accountName) &&
                    !curThread->isAccountSigner(accountName)) {
                    if (!curThread->isAuthorizedAccountPDAAndHasSignedSeeds(accountName)) {
                        UntrustfulAccount::collect(accountName, e, callEventTraces,
                                                   SVE::Type::ACCOUNT_INCORRECT_AUTHORITY, 10);
                        isUnvalidate = true;
                    }
                }
                ...
                //&& !isAccountPDA(accountName) - this condition removed, because PDA can still be faked...
                if (!isUnvalidate && !isAnchorValidatedAccount(accountName) &&
                    !curThread->isAccountLamportsUpdated(accountName) &&
                    !curThread->isAccountKeyValidated(accountName) &&
                    (!curThread->isAccountOwnerValidated(accountName) && !curThread->isAccountSigner(accountName))) {
                    // llvm::errs() << "==============VULNERABLE: Account Unvalidated!============\n";
                    if (!isOwnerOnly) {
                        if (!curThread->isAccountPDAInInstruction(accountName) && !isGlobalStateAccount(accountName) &&
                            ...
                            if (accountName.contains("order")) {
                                ...
                                if (accountTypeContainsMoreThanOneMint(type)) skipOrderAccount = true;
                            } else if (accountName.contains("close")) {
                                if (curThread->startFunc->getName().contains("close")) skipCloseAccount = true;
                            } else  // if (accountName.contains("mint"))
                            {
                                auto type = curThread->getStructAccountType(accountName);
                                // && curThread->isMintAccountValidated(accountName)
                                if (type.contains("Mint")) skipMintAccount = true;
                                if (type.contains("TokenAccount") && accountName.contains("user_"))
                                    skipUserAccount = true;
                                else if ((accountName.contains("user_") || accountName.contains("multisig_") ||
                                          accountName.contains("market_")) &&
                                         curThread->isAccountHasOneContraint(accountName)) {
                                    skipUserAccount = true;
                                }

                                // if (type.contains("TokenAccount") && curThread->isAccountMintValidated(accountName))
                                // {
                                //     skipMintAccount = true;
                                //     if (!curThread->isAccountInvoked(accountName)) {
                                //         llvm::errs() << "tid: " << curThread->getTID()
                                //                      << " ==============isAccountInvoked NOT: " << accountName
                                //                      << "============\n";
                                //     }
                                // }
                                ...
                                if (accountName.endswith("_account_info")) {
                                    auto accountName_x = accountName.substr(0, accountName.find("_account_info"));
                                    if (curThread->isAccountKeyValidated(accountName_x)) {
                                        skipMintAccount = true;
                                    }
                                }
                            }
                            if (!isPotentiallyOwnerOnly && !skipOrderAccount && !skipCloseAccount && !skipMintAccount &&
                                !skipUserAccount &&  //! isPotentiallyOwnerOnly &&
                                !accountName.contains("new_") && !accountName.contains("payer") &&
                                !accountName.contains("delegat") && !accountName.contains("dest") &&
                                !accountName.contains("receiver") && !accountName.equals("to")) {
                                if (!curThread->isAccountReferencedBySignerAccountInConstraint(accountName) &&
                                    !curThread->isAccountValidatedBySignerAccountInConstraint(accountName) &&
                                    !curThread->isAccountReferencedByAnyOtherAccountInSeeds(accountName) &&
                            ...
                            if (!isGlobalStateAccount(accountName) && curThread->isAccountUsesSelfSeeds(accountName) &&
                                ...
                                !curThread->isAccountReferencedByAnyOtherAccountInHasOne(accountName) &&
                                !curThread->isAccountReferencedByAnyOtherAccountInConstraint(accountName)) {
                                // check if account is isolated..
                                if (DEBUG_RUST_API)
                                    llvm::errs() << "==============VULNERABLE: PDA Account Unvalidated!============\n";
                                UntrustfulAccount::collect(accountName, e, callEventTraces,
                                                           SVE::Type::ACCOUNT_UNVALIDATED_PDA, 10);
                                isUnvalidate = true;
                            }
                            ...
                            //     Unvalidated!============\n";
                            // UntrustfulAccount::collect(e, callEventTraces, SVE::Type::ACCOUNT_UNVALIDATED_OTHER,
                            // 5); isUnvalidate = true;
                        }
                    }
                }
            ...
            if (curThread->startFunc->getName().contains("close")) {
                // check close account
                if (curThread->isAccountClosed(accountName) && !curThread->isAccountInvoked(accountName) &&
                    !curThread->isAccountMemsetData(accountName)) {
                    // llvm::errs() << "==============VULNERABLE: Insecure Account
                    // Close!============\n";
                    UntrustfulAccount::collect(accountName, e, callEventTraces, SVE::Type::ACCOUNT_CLOSE, 5);
                }
            }
            ...
            if (!isOwnerOnly && accountName.contains("order")) {
                if (curThread->startFunc->getName().contains("init") ||
                    ...
                    potentialInitOrderRelatedAccountsMap[curThread->startFunc].insert(accountName);
                } else if (curThread->startFunc->getName().contains("cancel")) {
                    potentialCancelOrderRelatedAccountsMap[curThread->startFunc].insert(accountName);
                } else {
                    potentialExchangeOrderRelatedAccountsMap[curThread->startFunc].insert(accountName);
                }
            }
        }
    }

    if (DEBUG_RUST_API) llvm::outs() << "checking IDL backwards compatibility\n";
    // check backwards compatibility if IDL is available
    {
        for (auto pair : curThread->anchorStructFunctionFields) {
            ...
            if (result != 0) {
                ...
                if (result < 0)
                    UnSafeOperation::collect(e, callEventTraces, SVE::Type::ACCOUNT_IDL_INCOMPATIBLE_ADD, 7);
                else if (result > 0)
                    UnSafeOperation::collect(e, callEventTraces, SVE::Type::ACCOUNT_IDL_INCOMPATIBLE_MUT, 7);
            }
        }
        ...
        if (!curThread->isAccountCompatibleOrder(accountName1, accountName2)) {
            ...
            CosplayAccounts::collect(e1, e2, callEventTraces, SVE::Type::ACCOUNT_IDL_INCOMPATIBLE_ORDER, 7);
        }
    }

    if (DEBUG_RUST_API) llvm::outs() << "checking bump_seed_canonicalization_insecure\n";
    ...
    for (auto [pair, e] : curThread->accountsBumpMap) {
        ...
        if (!curThread->isAccountBumpValidated(bumpName) && !curThread->isSignerAccountUsedSeedsOfAccount0(account)) {
            // llvm::errs() << "==============VULNERABLE: bump seed canonicalization!============\n";
            if (!isOwnerOnly) UntrustfulAccount::collect(bumpName, e, callEventTraces, SVE::Type::BUMP_SEED, 9);
        }
    }
    if (DEBUG_RUST_API) llvm::outs() << "checking duplicate mutable accounts\n";
                ...
                for (size_t j = i + 1; j < size; j++) {
                    ...
                    if (type1 == type2) {
                        if (type1.contains("Account") &&
                            !isAccountKeyNotEqual(accountTypeVec[i].first, accountTypeVec[j].first)) {
                            // do not consider common types
                            if (!type1.contains("Pubkey") && !type1.contains("AccountInfo") &&
                                !type1.contains("Rent") && !type1.contains("Token") && !type1.contains("Mint") &&
                                ...
                                if (!curThread->isAccountsPDAInit(account1) &&
                                    !curThread->isAccountsPDAInit(account2) && !isAccountPDA(account1) &&
                                    !isAccountPDA(account1)) {
                                    if (DEBUG_RUST_API)
                                        llvm::errs()
                                    ...
                                    // llvm::errs() << "==============VULNERABLE: Duplicate Mutable
                                    // Account!============\n";
                                    ...
                                    if (curThread->isAccountDiscriminator(account1) &&
                                        ...
                                        isDifferent = true;
                                    else if (curThread->isDuplicateAccountChecked(account1, account2)) {
                                        isDifferent = true;
                                    }
                                    if (!isDifferent)
                                        CosplayAccounts::collect(e1, e2, callEventTraces, SVE::Type::ACCOUNT_DUPLICATE,
                                                                 9);
                                }
                            }
                        }
...
void RaceDetectionPass::detectAccountsCosplay(const aser::ctx *ctx, TID tid) {
    ...
    for (auto account : userProvidedInputStringPDAAccounts) {
        if (DEBUG_RUST_API)
            llvm::errs() << "==============userProvidedInputStringPDAAccounts account: " << account << "\n";
        auto seeds = accountsPDASeedsMap[account];
        if (DEBUG_RUST_API)
            for (auto seed : seeds) llvm::errs() << "==============accountsPDASeedsMap seed: " << seed << "\n";
        for (auto &[account2, seeds2] : accountsPDASeedsMap) {
            if (account != account2) {
                ...
                if (isCompatibleSeeds(first_seed, first_seed2) && isCompatibleSeeds(last_seed, last_seed2)) {
                    // report bug
                    // llvm::errs() << "==============userProvidedInputStringPDAAccounts account2: " << account2
                    //              << "\n";
                    // llvm::errs() << "==============accountsPDASeedsMap seed: " << seed << " seed2: " << seed2
                    //              << "\n";
                    auto e1 = accountsPDAMap[account];
                    auto e2 = accountsPDAMap[account2];
                    CosplayAccounts::collect(e1, e2, callEventTraces, SVE::Type::PDA_SEEDS_COLLISIONS, 7);
                }
            }
        }
    }
                    ...
                    if (size <= size2) {
                        ...
                        for (size_t i = 0; i < size; i++) {
                            ...
                            if (!type1.equals(type2) ||
                                (fieldTypes1[i].first.contains("crimi") || fieldTypes2[i].first.contains("crimi"))) {
                                mayCosplay = false;
                                break;
                            }
                            // llvm::errs() << "==============type1: " << type1 << "============\n";
                            // llvm::errs() << "==============type2: " << type2 << "============\n";
                            // must have a Pubkey field
                            if (type1.contains("ubkey")) hasPubkey = true;
                        }
                        if (mayCosplay && hasPubkey) {
                            ...
                            if (file1 == file2) {
                                // llvm::errs() << "==============VULNERABLE: Type Cosplay!============\n";
                                if (size == size2)
                                    CosplayAccounts::collect(e1, e2, callEventTraces, SVE::Type::COSPLAY_FULL, 6);
                                else
                                    CosplayAccounts::collect(e1, e2, callEventTraces, SVE::Type::COSPLAY_PARTIAL, 5);
                            }
                        }
                    }
                }
...
void RaceDetectionPass::detectRaceCondition(const aser::ctx *ctx, TID tid) {
    for (auto [funcCancel, cancelAccounts] : potentialCancelOrderRelatedAccountsMap) {
        if (DEBUG_RUST_API) llvm::errs() << "==============funcCancel: " << funcCancel->getName() << "!============\n";
        for (auto [funcInit, initAccounts] : potentialInitOrderRelatedAccountsMap) {
            if (DEBUG_RUST_API) llvm::errs() << "==============funcInit: " << funcInit->getName() << "!============\n";
            ...
            if (overlappingAccounts.size() > 0) {
                ...
                if (potentialPriceRaceCondition) {
                    ...
                    for (auto &[funcX, exchangeAccounts] : potentialExchangeOrderRelatedAccountsMap) {
                        ...
                        if (potentialRaceFunc) {
                            ...
                            if (!hasPrice) {
                                ...
                                if (auto callValue = dyn_cast<CallBase>(inst1)) {
                                    CallSite CS(callValue);
                                    inst1 = CS.getTargetFunction()->getEntryBlock().getFirstNonPHI();
                                }
                                // llvm::errs() << "==============inst1: " << *inst1 << "!============\n";
                                auto e1 = graph->createApiReadEvent(ctx, inst1, tid);
                                UnSafeOperation::collect(e1, callEventTraces, SVE::Type::ORDER_RACE_CONDITION, 9);
                            }
                        }
                    }
                }
...
StaticThread *RaceDetectionPass::forkNewThread(ForkEvent *forkEvent) {
    ...
    if (LangModel::isRustAPI(forkSite.getTargetFunction())) {
        auto threadEntry = pta->getDirectNode(forkEvent->getContext(), forkSite.getTargetFunction());
        if (!threadEntry) {
            llvm::outs() << "Failed to find function: " << forkSite.getTargetFunction()->getName() << "\n";
            return nullptr;
        }
        auto thread = new StaticThread(threadEntry, forkEvent->getInst(), forkEvent);
        forkEvent->setSpawnedThread(thread);
        llvm::outs() << "Found attack surface #" << thread->getTID() << ": " << forkSite.getTargetFunction()->getName()
                     << "\n";
        return thread;
    }
    ...
    auto cs = pta->getInDirectCallSite(forkEvent->getContext(), forkEvent->getInst());
...
bool RaceDetectionPass::runOnModule(llvm::Module &module) {
        ...
        for (auto node : *pta->getCallGraph()) {
            if (node->isIndirectCall()) {
                ...
                if (indirectCalls.empty()) {
                    ...
                    llvm::outs() << "Failed to resolve any indirect call in func: " << demangle(func->getName().str())
                                 << " at " << getSourceLoc(inst).sig() << "\n"
                                 << getSourceLoc(inst).getSnippet() << "\n";
                } else if (DEBUG_INDIRECT_CALL_ALL && indirectCalls.size() > 1) {
                    for (auto fun : indirectCalls) {
                        llvm::outs() << "Resolved indirect call: funcName=" << demangle(fun->getName().str()) << "\n";
                    }
                    llvm::outs() << "Hitting an indirect call (" << indirectCalls.size()
                                 << " resolved calls in total) in func: " << demangle(func->getName().str()) << " at "
                                 << getSourceLoc(inst).sig() << "\n"
                                 << getSourceLoc(inst).getSnippet() << "\n";
                }
            }
    ...
    while (!threadList.empty()) {
        ...
        LOG_DEBUG("Start travering thread {}, thread start = {}", curThread->getTID(),
                  curThread->getStartFunction()->getName());
        threadList.pop();
        ...
        if (tid != 0) {
            ...
            ptCreate = graph->createCallEvent(forkEvent->getContext(), forkEvent->getInst(),
            ...
            if (curThread->getEntryNode()) {
                threadCB = graph->createCallEvent(curThread->getEntryNode()->getContext(), forkEvent->getInst(),
                                                  curThread->getEntryNode(), tid);
            } else {
                threadCB = graph->createCallEvent(forkEvent->getContext(), forkEvent->getInst(),
        ...
        traverseFunction(curThread->getEntryNode() ? curThread->getEntryNode()->getContext()
    ...
    if (threadSet.size() == 1) {
        ...
        for (auto tid = 1; tid < threadSet.size(); tid++) detectUntrustfulAccounts(tid);
    }
...
static RegisterPass<RaceDetectionPass> RD("Race Detection",
