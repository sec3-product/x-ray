...
// convert the customized API into a form that coderrect can understand
...
void ThreadAPIConverter::convertFunPtrAsEntryThreadAPI(const ThreadAPIInfo &APIInfo) {
    Function *theAPI = APIInfo.getAPI();
    // reimplement the API to call coderrect special routine
    ...
    // now create coderrect.thread.create function
    ...
    // 3rd, create call to coderrect.thread.create
...
void rewriteCThreadCreateAPI(Module *M, Function *fun, const ThreadCreateAPI &api) {
    ...
    auto entryBB = BasicBlock::Create(M->getContext(), "aser.lock", fun);
