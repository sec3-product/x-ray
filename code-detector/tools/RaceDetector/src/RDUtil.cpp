...
bool aser::isNotParallelFunctions(std::vector<std::string> &st1, std::vector<std::string> &st2,
                                  std::map<std::string, std::string> &pairs) {
    for (const auto &pair : pairs) {
        const std::string &funNamePattern1 = pair.first;
        const std::string &funNamePattern2 = pair.second;

        // match any suffix
        std::string entry1 = (funNamePattern1.back() == '*' ? funNamePattern1 : funNamePattern1 + "*");
        std::string entry2 = (funNamePattern2.back() == '*' ? funNamePattern2 : funNamePattern2 + "*");

        // LOG_DEBUG("NotParallelFunctionPairs: name1={} name2={}", entry1, entry2);

        Expected<GlobPattern> pattern1 = llvm::GlobPattern::create(entry1);
        Expected<GlobPattern> pattern2 = llvm::GlobPattern::create(entry2);
        ...
        if (pattern1 && pattern2) {
            ...
            for (auto &str : st1) {
                // LOG_DEBUG("NotParallelFunctionPairs: str1={}", str);

                if (pattern1->match(str)) {
                    matched1 = true;
                    break;
                }
            }
            for (auto &str : st2) {
                // LOG_DEBUG("NotParallelFunctionPairs: str2={}", str);

                if (pattern2->match(str)) {
                    matched2 = true;
                    break;
                }
            }
            if (matched1 && matched2) {
                LOG_DEBUG("Matched NotParallelFunctionPairs: name1={} name2={}", funNamePattern1, funNamePattern2);
                return true;
            }

            // let's try st1 -> pattern2, str2 -> pattern1
    ...
    if (!pairs.empty()) LOG_DEBUG("NotParallelFunctionPairs no match");
