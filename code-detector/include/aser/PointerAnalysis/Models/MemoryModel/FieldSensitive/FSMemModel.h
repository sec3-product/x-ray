    ...
    inline ObjNode *allocStructArrayObj(const ctx *C, const llvm::Value *V, AllocKind T, llvm::Type *type,
        ...
        switch (kind) {
            ...
            case MemModelKind::CPP: {
                return static_cast<cpp::CppMemModel<ctx> *>(this)->template allocStructArrayObjImpl<PT>(C, V, T, type,
                                                                                                        DL);
            }
	    default:
                return nullptr;
...
//        }
                ...
                case llvm::Type::ArrayTyID: {
                    ...
                    if (auto structElem = llvm::dyn_cast<llvm::StructType>(elemTy)) {
                        ...
                        ObjNode *child = allocAnonObjRec<PT>(
                            C, DL, getUnboundedArrayTy(elemTy->getPointerElementType()), tag, typeTree);
                        if (child != nullptr) {
    ...
    void initializeGlobal(const llvm::GlobalVariable *gVar, const llvm::DataLayout &DL) {
        if (gVar->hasInitializer()) {
            ...
            if (gVar->isConstant()) {
                // the memory block is immutable, i.e., the content of the memory block can not be updated later.
