...
private:
    ...
    FSObject<ctx> *indexObject(const FSObject<ctx>* obj, const llvm::Instruction *idx,
            ...
            if (!USE_MEMLAYOUT_FILTERING) {
                ...
                if (offset.getSExtValue() >= 0) {
                    ...
                    if (!gep->getPointerOperandType()->getPointerElementType()->isIntegerTy(8)) {
                        ...
                        if (!isZeroOffsetTypeInRootType(fieldTy,
    ...
    // force the pOffset be initialized with the given object (this object can be a special object such as containers)
