    ...
    inline void interceptEntryFun(const llvm::Function *entry) {
        ...
        for (; it != ie; it++) {
            if (it->getType()->isPointerTy()) {
                auto objNode =
                    MMT::template allocateAnonObj<PT>(this->getMemModel(),                     // memory model
...
protected:
    ...
    [[nodiscard]]
    inline MapObject<KeyT, Self> *getOrAllocMapObj(const ctx *context, const void *tag) {
    ...
    inline InterceptResult overrideFunction(const ctx *callerCtx, const ctx *calleeCtx,
        ...
        return static_cast<SubClass *>(this)->interceptFunction(calleeCtx, callerCtx, F, callsite);
    }
    ...
    inline bool overrideCallSite(const CtxFunction<ctx> *caller, const CtxFunction<ctx> *callee,
        ...
        if (static_cast<SubClass *>(this)->isHeapAllocAPI(callee->getFunction(), callsite)) {
            // handle different heap allocation APIs.
            static_cast<SubClass *>(this)->interceptHeapAllocSite(caller, callee, callsite);
            return true;
        }

        return static_cast<SubClass *>(this)->interceptCallSite(caller, callee, originalTarget, callsite);
    }
...
struct LangModelTrait<LangModelBase<ctx, MemModel, PtsTy, SubClass>> {
    ...
    static inline void constructConsGraph(LangModelTy *model) { model->constructConsGraph(); }

    static inline CGNodeTy *indexObject(LangModelTy *model, ObjNodeTy *objNode, const llvm::Instruction *idx) {
        return model->indexObject(objNode, idx);
    }

    // get constructed constraint callgraph
    static inline const ConsGraphTy *getConsGraph(const LangModelTy *model) { return model->getConsGraph(); }

    // get constructed constraint callgraph
    static inline ConsGraphTy *getConsGraph(LangModelTy *model) { return model->getConsGraph(); }

    // get constructed call callgraph
    static inline const CallGraphTy *getCallGraph(LangModelTy *model) { return model->getCallGraph(); }

    // true if at least one indirect call site is updated.
    static inline bool updateFunPtrs(LangModelTy *model, const llvm::SparseBitVector<> &resolved) {
        return model->updateFunPtrs(resolved);
    }

    // get corresponding pointer nodes that represent v (in all different contexts)
    static inline void getNodesForValue(LangModelTy *model, const llvm::Value *v, std::set<CGNodeTy *> &result) {
        // langModel->getNodesForValue(v, result);
    }

    // get corresponding pointer nodes that represent v
    static inline void getPointedBy(LangModelTy *model, const ctx *context, const llvm::Value *v,
                                    std::set<Pointer<ctx>> &result) {}

    [[nodiscard]] static inline llvm::StringRef getEntryName(const LangModelTy *model) { return model->getEntryName(); }

    [[nodiscard]] static inline const llvm::Module *getLLVMModule(const LangModelTy *model) {
        return model->getLLVMModule();
    }

    [[nodiscard]] static inline const CallGraphNode<ctx> *getDirectNode(LangModelTy *model, const ctx *C,
                                                                        const llvm::Function *F) {
        return model->getDirectNode(C, F);
    }

    [[nodiscard]] static inline const CallGraphNode<ctx> *getDirectNodeOrNull(LangModelTy *model, const ctx *C,
                                                                              const llvm::Function *F) {
        return model->getDirectNodeOrNull(C, F);
    }

    [[nodiscard]] [
        [deprecated("use getInDirectCallSite(ctx, instruction) instead")]] static inline const CallGraphNode<ctx>
        *getInDirectNode(LangModelTy *model, const ctx *C, const llvm::Instruction *I) {
        return model->getInDirectNode(C, I);
    }

    [[nodiscard]] static inline const InDirectCallSite<ctx> *getInDirectCallSite(LangModelTy *model, const ctx *C,
                                                                                 const llvm::Instruction *I) {
        return model->getInDirectNode(C, I)->getTargetFunPtr();
    }

    static inline NodeID getSuperNodeIDForValue(LangModelTy *model, const ctx *C, const llvm::Value *V) {
    ...
    static inline PtrNode *getPtrNode(LangModelTy *model, const ctx *C, const llvm::Value *V) {
        return model->getPtrNode(C, V);
    }

    static inline bool isHeapAllocAPI(LangModelTy *model, const llvm::Function *fun) {
        return model->isHeapAllocAPI(fun);
    }

    // this is a special method only used for LockSetManager
    // To handle empty pts on lock object
    // FIXME: this method should be removed, create a wrapper class for lock object,
    // and assign an unique fake identifier for those empty pts lock ptr
    static inline ObjNodeTy *allocSpecialAnonObj(LangModelTy *model, const llvm::Instruction *I, const llvm::Value *V) {
        auto objNode = MMT::template allocateHeapObj<PT>(model->getMemModel(),  // memory model
                                                         CT::getInitialCtx(),   // initial context
                                                         I,
