...
template <typename V> struct can_output_to_llvm_ostream_impl {
  template <typename T = V,
            FMT_ENABLE_IF(!is_string<T>::value &&
                          !is_char<T>::value)> // template parameters here to
                                               // enable SFINAE
  static auto test(T &&t, llvm::raw_ostream &os)
      -> decltype(os << t, void(), std::true_type{});

  static auto test(...) -> std::false_type;
  using type =
      decltype(test(std::declval<V>(), std::declval<llvm::raw_ostream &>()));
};
...
// template <typename V>
// constexpr bool can_output_to_llvm_ostream_v =
// can_output_to_llvm_ostream<V>::value;

// Formats an object of type T that has an overloaded ostream operator<<.
template <typename T, typename Char>
struct fallback_formatter<T, Char,
                          enable_if_t<can_output_to_llvm_ostream<T>::value>>
...
// GCC 8 and earlier cannot handle std::back_insert_iterator<SpecialObject> with
