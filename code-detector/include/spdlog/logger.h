...
public:
  ...
  void log(source_loc loc, level::level_enum lvl, string_view_t fmt,
           const Args &...args) {
...
#error SPDLOG_WCHAR_TO_UTF8_SUPPORT only supported on windows
  ...
  template <typename... Args>
  void log(source_loc loc, level::level_enum lvl, wstring_view_t fmt,
           const Args &...args) {
    bool log_enabled = should_log(lvl);
    bool traceback_enabled = tracer_.enabled();
    if (!log_enabled && !traceback_enabled) {
...
protected:
  ...
  spdlog::level_t level_{
      level::trace}; // ASER: we use static way to select log level
