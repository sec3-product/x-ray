...
//     string output = "((4 * (sext i32 (({%tid,+,1}<%.preheader> * %54) + {0,+,1}<%53>) to i64))<nsw> + %7)";
//     string expected_output = "4 * %tid * %54 + %7";
...
TEST(SVFTest, test_scev_parser2) {
    SCEV_TEMP_RESULT = "((4 * (sext i32 (({%tid,+,1}<%.preheader> * %54) + {0,+,1}<%53>) to i64))<nsw> + %7)";
    llvm::outs() << "before: " << SCEV_TEMP_RESULT << "\n";
    int output = test_scev_parser(SCEV_TEMP_RESULT.c_str());
    llvm::outs() << "after: " << SCEV_TEMP_RESULT << "\n";
    ASSERT_EQ(0, output);
...
TEST(SVFTest, test_scev_parser3) {
    SCEV_TEMP_RESULT = "((4 * (sext i32 (-10 + (2 * {%tid,+,1}<%.lr.ph>)) to i64))<nsw> + (2 * {%tid,+,1}<%.lr.ph>))";
    llvm::outs() << "before: " << SCEV_TEMP_RESULT << "\n";
    int output = test_scev_parser(SCEV_TEMP_RESULT.c_str());
    llvm::outs() << "after: " << SCEV_TEMP_RESULT << "\n";
    ASSERT_EQ(0, output);
