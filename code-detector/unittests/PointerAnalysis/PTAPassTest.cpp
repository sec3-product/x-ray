...
cl::opt<std::string> TestIR(cl::Positional,
...
template <typename Solver> class PTAVerificationPass : public llvm::ModulePass {
public:
  using ctx = NoCtx;

  static char ID;
  PTAVerificationPass() : llvm::ModulePass(ID) {
    static_assert(std::is_same<typename Solver::ctx, NoCtx>::value &&
                  "Only support context insensitive");
  }
...
template <typename PTA> char PTAVerificationPass<PTA>::ID = 0;
...
static llvm::RegisterPass<PointerAnalysisPass<Solver>>
