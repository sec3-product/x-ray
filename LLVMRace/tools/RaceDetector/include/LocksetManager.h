...
private:
    ...
    // ```
    // however, since our PTA is conservative, usually the lockset off l1 won't mssing o2
    // therefore, the vector is a better option here
    ...
    std::map<std::vector<const ObjTy *>, std::set<std::string>> lockSetStringsCache;
    ...
    bool hasCommonLocks(const MemAccessEvent *e1, const std::vector<const ObjTy *> &lockset1, const MemAccessEvent *e2,
...
public:
    ...
    void releaseLock(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr);
    void acquireLock(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr);
    ...
    void dumpLockset() {
        ...
        for (auto &[lockset, lockStrings] : lockSetStringsCache) {
            for (auto o : lockset) {
                ...
                for (auto s : lockStrings) llvm::outs() << "        name: " << s << "\n";
            }
        }
    ...
    std::string findLockStr(const llvm::Instruction *I, const llvm::Value *lockPtr);
    void getLockSet(const ctx *context, const llvm::Instruction *I, const llvm::Value *lockPtr,
    ...
    LocksetManager::ID getRetrofitLockSetID(std::vector<const ObjTy *> pts, LocksetManager::ID id);
    std::vector<const ObjTy *> getLockSet(LocksetManager::ID id);

    bool hasDifferentPointsToSet(const ctx *ctx1, const llvm::Value *lockPtr1, const ctx *ctx2,
                                 const llvm::Value *lockPtr2) {
