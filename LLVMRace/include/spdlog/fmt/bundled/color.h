...
struct ansi_color_escape {
    ...
    FMT_CONSTEXPR ansi_color_escape(emphasis em) FMT_NOEXCEPT {
        ...
        if (em_bits & static_cast<uint8_t>(emphasis::strikethrough)) em_codes[3] = 9;
...
template <typename Char>
std::basic_string<Char> vformat(const text_style& ts, basic_string_view<Char> format_str,
    ...
    if (ts.has_foreground()) {
        has_style = true;
        ansi_color_escape<Char> escape = make_foreground_color<Char>(ts.get_foreground());
        buffer.append(escape.begin(), escape.end());
    }
    if (ts.has_background()) {
        has_style = true;
        ansi_color_escape<Char> escape = make_background_color<Char>(ts.get_background());
        buffer.append(escape.begin(), escape.end());
    }
...
inline std::basic_string<Char> format(const text_style& ts, const S& format_str, const Args&... args) {
    return internal::vformat(ts, to_string_view(format_str), {internal::make_args_checked(format_str, args...)});
}
