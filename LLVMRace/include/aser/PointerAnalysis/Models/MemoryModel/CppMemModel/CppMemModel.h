...
// TODO: there are a lot of things to do to model C++'s memory model accurately besides vtable
// e.g., runtime type information + class hirachy, so we put it into a CppMemModel
// this will make it easier for future extension when we need to handle more languages such as Rust/Fortran

#include "aser/PointerAnalysis/Models/MemoryModel/CppMemModel/RewriteModeledAPIPass.h"
...
private:
    ...
    static bool isSpecialTypeImpl(const llvm::Type *T) {
        ...
        // is type equivalience accurate enough? i.e., How often is it when user use the same type but not for vtable
        // should be rare, the vtable ptr type is "i32 (...) **"
        // FIXME: if there might be user defined type that is identical to vtable pointer,
        // identify vtable pointer using more accurate predicate
        ...
        switch (calledAPI.getAPIKind()) {
            case VectorAPI::APIKind::PUSH_BACK:
                if (calledAPI.getVecElemType()->isPointerTy()) {
                    ...
                    auto elem =
                        getPtrNode(C, calledAPI.getArgOperand(1));  // second argument is a pointer to the element.
    ...
    inline ObjNode *allocStructArrayObjImpl(const ctx *C, const llvm::Value *V, AllocKind T, llvm::Type *type,
        ...
        if (T != AllocKind::Anonymous) {
            if (auto GV = llvm::dyn_cast<llvm::GlobalVariable>(V)) {
                if (isVTableVariable(GV) && !GV->isDeclaration()) {
                    // if this is a vtable, handle it specially (do not collapse the array when building the memory
                    // layout)
        ...
        auto block =
            static_cast<AggregateMemBlock<ctx> *>(super allocMemBlock<AggregateMemBlock<ctx>>(C, V, T, layout));
    ...
    inline InterceptResult interceptFunction(const llvm::Function *F, const llvm::Instruction *callSite) {
        // does not matter as they function body should be deleted by RewriteModeledAPIPass
